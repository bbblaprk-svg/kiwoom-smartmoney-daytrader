...
REMOTE_DOCKERFILE="QUANT_NOVA_3.3.6_R493_MARKET_RELATIVE_SAFE_FIX3_20260822.Dockerfile"
EXPECTED_DOCKERFILE_SHA="9d7966e6dbd9ec95c581c9b027aa6dd3ac2f5f39651cef20f489b4f4a241b60d"
...
say "1A/9 PREFLIGHT EMBEDDED SOURCE PAYLOAD (NO CONTAINER CHANGE)"
PYBIN="$(command -v python3 || command -v python || true)"
[ -n "$PYBIN" ] || { echo "FAIL: host python3/python is required for payload preflight"; exit 1; }
"$PYBIN" - "$DOCKERFILE" <<'PY' | tee -a "$LOG"
import base64,gzip,hashlib,io,pathlib,re,sys,tarfile,tempfile,subprocess,os
df=pathlib.Path(sys.argv[1]).read_text()
parts=re.findall(r"RUN printf '%s' '([^']*)' >> /tmp/nova-source\.tar\.gz\.b64",df)
if not parts:
    raise SystemExit("PREFLIGHT_FAIL: no embedded payload chunks")
payload=''.join(parts)
try:
    raw=base64.b64decode(payload,validate=True)
except Exception as e:
    raise SystemExit(f"PREFLIGHT_FAIL: base64 decode: {e}")
if raw[:2] != b'\x1f\x8b':
    raise SystemExit("PREFLIGHT_FAIL: payload is not gzip")
with tempfile.TemporaryDirectory(prefix='nova-r493-payload-') as td:
    try:
        with tarfile.open(fileobj=io.BytesIO(raw),mode='r:gz') as tf:
            names=set(tf.getnames())
            required={'app/main.py','scripts/master_audit.py','scripts/r493_market_relative_acceptance.py','SOURCE_MANIFEST.sha256','requirements.txt'}
            missing=sorted(required-names)
            if missing:
                raise SystemExit("PREFLIGHT_FAIL: missing "+','.join(missing))
            tf.extractall(td)
    except Exception as e:
        raise SystemExit(f"PREFLIGHT_FAIL: tar/gzip integrity: {e}")
    cp=subprocess.run(['sha256sum','-c','SOURCE_MANIFEST.sha256'],cwd=td,stdout=subprocess.DEVNULL,stderr=subprocess.STDOUT)
    if cp.returncode:
        raise SystemExit("PREFLIGHT_FAIL: SOURCE_MANIFEST mismatch")
    key_files=['app/broker/market_index.py','app/addons/market_relative.py','app/service.py','app/addons/decision_panels.py']
    for rel in key_files:
        cp=subprocess.run([sys.executable,'-m','py_compile',rel],cwd=td,stdout=subprocess.DEVNULL,stderr=subprocess.STDOUT)
        if cp.returncode:
            raise SystemExit('PREFLIGHT_FAIL: py_compile '+rel)
    mit=(pathlib.Path(td)/'app/broker/market_index.py').read_text()
    for needle in ("API_ID = 'ka20001'","API_PATH = '/api/dostk/sect'","payload.get('upl')","payload.get('lst')"):
        if needle not in mit:
            raise SystemExit('PREFLIGHT_FAIL: market-index contract missing '+needle)
print(f"PREFLIGHT_PAYLOAD=PASS chunks={len(parts)} b64_bytes={len(payload)} gzip_bytes={len(raw)} gzip_sha256={hashlib.sha256(raw).hexdigest()} key_py_compile=PASS market_contract=PASS")
PY
...
say "3/9 RESOURCE PREFLIGHT, THEN STOP OLD TEMPORARILY TO FREE 1GB HOST AND BUILD"
...
echo "post_stop_available_mb=$AVAIL_MB free_kb=$FREE_KB" | tee -a "$LOG"
...
[ "$(dc image inspect "$IMAGE" --format '{{index .Config.Labels "io.quantnova.packaging_revision"}}')" = "FIX3_FULL_AUDIT_CUTOVER_SAFE_OFFICIAL_BREADTH" ]
...
# Keep the pre-deploy container STOPPED through cutover.  Restarting it here can
# retain host/network port ownership and race the new container start.
say "5/9 ATOMIC CUTOVER; PRE-DEPLOY CONTAINER PRESERVED STOPPED"
if [ "$(dc inspect "$APP" --format '{{.State.Running}}')" = "true" ]; then dc stop -t 5 "$APP" >/dev/null; OLD_STOPPED=1; fi
dc rename "$APP" "$BACKUP"
OLD_RENAMED=1
STATE_MUTATED=1
[ "$(dc inspect "$BACKUP" --format '{{.State.Running}}')" = "false" ] || { echo "FAIL: backup container unexpectedly running before cutover"; exit 1; }
...
dc image inspect "$IMAGE" --format 'UI_PATCH={{index .Config.Labels "io.quantnova.ui_patch"}} R492={{index .Config.Labels "io.quantnova.r492_extension"}} R493_INDEX={{index .Config.Labels "io.quantnova.r493_market_index"}} R493_REST={{index .Config.Labels "io.quantnova.r493_market_rest_calls_per_refresh"}} R493_WS={{index .Config.Labels "io.quantnova.r493_new_ws_subscription_types"}} PACKAGING={{index .Config.Labels "io.quantnova.packaging_revision"}}' | tee -a "$LOG"
