...
EXPECTED_DOCKERFILE_SHA="3e79847b87c35e8bb5ce4926ed46d482e70cc76544a5aa65534912a229b93a36"
EXPECTED_VERSION="NOVA-3.3.4-MARKET-WIDE-ROTATION-VERIFY"
IMAGE="quant-nova:3.3.4-r48-final-local"
STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP="${APP}-pre-r48-${STAMP}"
FAILED="${APP}-failed-r48-${STAMP}"
CANDIDATE="${APP}-candidate-r48-${STAMP}"
WORK="$(mktemp -d /tmp/nova-r48-final.XXXXXX)"
DOCKERFILE="$WORK/Dockerfile"
ENVFILE="$WORK/current.env"
CAND_DATA="$WORK/candidate-data"
LOG="/tmp/nova-r48-final-deploy-${STAMP}.log"
OLD_WAS_RUNNING=0
OLD_STOPPED=0
OLD_RENAMED=0
NEW_STARTED=0
...
recover_previous_if_needed(){
  ...
  latest="$(dc ps -a --format '{{.Names}}' | grep -E "^${APP}-pre-r48-[0-9]{8}-[0-9]{6}$" | sort | tail -n 1 || true)"
  if [ -n "$latest" ]; then
...
[ "$(dc image inspect "$IMAGE" --format '{{index .Config.Labels "io.quantnova.ui_patch"}}')" = "R48_BOARD_INDEPENDENCE_ENTRY_FLOW_SPLIT" ]
[ "$(dc image inspect "$IMAGE" --format '{{index .Config.Labels "io.quantnova.board_independence"}}')" = "PREIGNITION_EARLY_EDGE__PREBUY_ENTRY_PROXIMITY__NXT_EARLY_SMART_FLOW_2PLUS" ]
