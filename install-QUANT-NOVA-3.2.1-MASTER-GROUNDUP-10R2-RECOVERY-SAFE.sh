...
APP_BK="${APP}-backup-g10r2-${STAMP}"
GUARD_BK="${GUARD}-backup-g10r2-${STAMP}"
TMP="$(mktemp -d)"
...
rollback(){
  ...
  echo "[AUTO-ROLLBACK] G10R2 deployment incomplete" >&2
...
for _ in range(4):
    ...
    assert j['version']=='NOVA-3.2.1-MASTER-GROUNDUP-10R',j.get('version')
...
rm -rf "$TMP"
echo
echo "===== QUANT NOVA 3.2.1 G10R2 RECOVERY-SAFE DEPLOY PASS ====="
