...
ZIP_NAME="kiwoom-smartmoney-daytrader-v3.1.19-FINAL.zip"
SOURCE_DIR_NAME="kiwoom-smartmoney-daytrader-v3.1.19"
EXPECTED_ZIP_SHA256="00f128948250b0f959d217dc5f1a4fd45aaae6ce30c93f4f51e08b0feedf398d"
IMAGE_NAME="kiwoom-smartmoney:v3.1.19"
...
rollback() {
  ...
  printf '\n[자동 복구] v3.1.19 적용 실패 — 기존 앱으로 되돌립니다.\n' >&2
...
if [[ "$ACTUAL_ZIP_SHA256" != "$EXPECTED_ZIP_SHA256" ]]; then
  printf "[오류] GitHub ZIP이 최신 v3.1.19 수정본이 아닙니다.\n예상 SHA256: %s\n실제 SHA256: %s\n" "$EXPECTED_ZIP_SHA256" "$ACTUAL_ZIP_SHA256" >&2
...
# v3.1.19 변경의 중심은 제한적 성과학습과 경량 10종목 확대이며 Kiwoom 수집경로는 유지한다.
...
(
  ...
  [[ "$PACKAGE_VERSION" == "3.1.19" ]] || fail "ZIP 내부 package.json 버전이 3.1.19이 아닙니다: ${PACKAGE_VERSION:-미확인}"
)
...
LABEL org.opencontainers.image.version="3.1.19"
RUN apk add --no-cache unzip
WORKDIR /app
ENV NEXT_TELEMETRY_DISABLED=1
COPY kiwoom-smartmoney-daytrader-v3.1.19-FINAL.zip /tmp/app.zip
RUN mkdir -p /tmp/source \
    && unzip -q /tmp/app.zip -d /tmp/source \
    && cp -a /tmp/source/kiwoom-smartmoney-daytrader-v3.1.19/. /app/ \
    && rm -rf /tmp/app.zip /tmp/source
RUN sha256sum -c SOURCE_MANIFEST.sha256
RUN npm config set registry https://registry.npmjs.org/ \
    && npm install --no-audit --no-fund
RUN npm run check \
    && LOAD_TEST_SYMBOLS=30 LOAD_TEST_TICKS=30000 npm run check:load \
    && npm run build
...
say "v3.1.19 Docker 이미지 빌드 — 전체검사·30종목 부하검사·Next 빌드 포함"
...
[[ "$RUNNING_VERSION" == "3.1.19" ]] || fail "실행 컨테이너 버전이 3.1.19이 아닙니다: $RUNNING_VERSION"
...
printf '\n============================================================\n'
printf 'v3.1.19 ADAPTIVE LEARNING + 30 STOCKS 업데이트 완료\n'
printf '앱 주소   : https://%s\n' "$DOMAIN"
printf '상태 확인 : https://%s/api/health\n' "$DOMAIN"
printf '감시 구조 : 고정4 + 사용자집중8 + 경량18 = 최대 30종목\n'
printf '핵심목적  : 상승초입 EARLY → 관찰강화도 → 급등 사전포착 → 엄격한 BUY 선별\n'
printf '매도잡음  : 일반 감시종목 RISK/EARLY_SELL/SELL 표시·푸시 제거\n'
printf '보유관리  : 실제 보유종목의 설정 손절가 도달만 1회 알림\n'
printf '알림정책  : 확정 BUY + 실제 보유 STOP만 푸시\n'
printf '추천기능  : 일일 8+18 / 장중 강한종목 8+18 / 수동보호 / 자동적용 ON/OFF 유지\n'
printf '성과학습  : EARLY/BUY 실제성과로 자동추천만 최대 ±10점 보정 · BUY 안전게이트 고정\n'
printf '관찰강화  : 횟수 자체 0점 · 점수/체결/스마트머니 연속개선 때만 +4/+8\n'
printf '화면안정  : 1.5초 묶음표시 + 종합기록부 기본 발생중 + 지난관찰 분리\n'
printf '============================================================\n'
printf '현재 건강상태: '
cat "$TMP_ROOT/health.json"
printf '\n\n최근 앱 로그:\n'
sudo docker logs --tail 60 "$APP_CONTAINER" || true
printf '\nSafari/PWA를 완전히 닫았다가 다시 열고 화면 상단 v3.1.19을 확인하세요.\n'
printf 'subscribed 값은 현재 실제 등록 종목 수이며 최대 30입니다.\n'
