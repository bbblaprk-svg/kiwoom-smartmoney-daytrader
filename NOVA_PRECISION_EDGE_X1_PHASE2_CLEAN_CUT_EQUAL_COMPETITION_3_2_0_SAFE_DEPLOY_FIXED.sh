#!/usr/bin/env bash
# NOVA PRECISION EDGE X1 PHASE2 · CLEAN CUT EQUAL COMPETITION · 3.2.0 FIXED-MANIFEST
# Safe build/canary/cutover with automatic rollback. No runtime source patching.
set -Eeuo pipefail

PRODUCT='NOVA PRECISION EDGE X1 PHASE2 · CLEAN CUT EQUAL COMPETITION · 3.2.0'
VERSION='NOVA-PRECISION-EDGE-X1-PHASE2-CLEAN-CUT-EQUAL-COMPETITION-3.2.0'
CURRENT_CONTAINER="${NOVA_CURRENT_CONTAINER:-quant-nova}"
DOCKERFILE_NAME='NOVA_PRECISION_EDGE_X1_PHASE2_CLEAN_CUT_EQUAL_COMPETITION_3_2_0_FIXED.Dockerfile'
EXPECTED_DOCKERFILE_SHA256='381c180f7df09b191aefc9eb0449543e362586897fd2e92cc6934a5ba2054027'
...
IMAGE_TAG="quant-nova:clean-cut-equal-3-2-0-${STAMP}"
BACKUP_CONTAINER="quant-nova-backup-before-clean-cut-equal-3-2-0-${STAMP}"
CANARY_CONTAINER="quant-nova-canary-clean-cut-equal-3-2-0-${STAMP}"
BUILD_CONTEXT=''
CUTOVER_STARTED=0
...
BUILD_CONTEXT="$(mktemp -d -t nova-equal-news-315-build.XXXXXX)"
...
[[ "$(label io.quantnova.clean_cut)" == 'TRUE' ]] || fail 'clean cut contract missing'
[[ "$(label io.quantnova.live_candidate_boot)" == 'EMPTY_EACH_TRADING_DAY' ]] || fail 'live candidate boot must be empty'
[[ "$(label io.quantnova.live_candidate_source)" == 'CURRENT_DAY_BROKER_RANK_SOURCE_ONLY' ]] || fail 'live source must be current broker evidence only'
[[ "$(label io.quantnova.prior_close_candidate_materialization)" == 'ZERO' ]] || fail 'prior close candidate materialization must be zero'
[[ "$(label io.quantnova.historical_signal_candidate_materialization)" == 'ZERO' ]] || fail 'historical signal candidate materialization must be zero'
[[ "$(label io.quantnova.formal_signal_pin_privilege)" == 'ZERO' ]] || fail 'formal signal pin privilege must be zero'
[[ "$(label io.quantnova.display_snapshot_backfill)" == 'ZERO' ]] || fail 'display snapshot backfill must be zero'
[[ "$(label io.quantnova.core_fresh_anchor_reservation)" == 'ZERO' ]] || fail 'legacy slot reservation must be zero'
...
[[ "$(label io.quantnova.position_restore)" == 'BUY_COOLDOWN_ONLY_POSITION_ONLY_NO_DISCOVERY_RANK' ]] || fail 'position restore isolation mismatch'
...
assert edge.get('version')=='NOVA-PRECISION-EDGE-X1-PHASE2-CLEAN-CUT-EQUAL-COMPETITION-3.2.0', edge.get('version')
...
say "CLEAN_CUT=TRUE"
say "LIVE_CANDIDATE_BOOT=EMPTY_EACH_TRADING_DAY"
say "LIVE_CANDIDATE_SOURCE=CURRENT_DAY_BROKER_RANK_SOURCE_ONLY"
...
say "DISPLAY_SNAPSHOT_BACKFILL=ZERO"
say "CORE_FRESH_ANCHOR_RESERVATION=ZERO"
say "SPARSE_BOARD=SIGNAL_WAIT_NOT_OLD_SYMBOL"
say "POSITION_RESTORE=BUY_COOLDOWN_ONLY_NO_DISCOVERY_RANK"
