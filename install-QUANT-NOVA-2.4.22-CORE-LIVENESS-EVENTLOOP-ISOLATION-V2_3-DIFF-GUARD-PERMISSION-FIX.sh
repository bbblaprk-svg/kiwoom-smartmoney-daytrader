...
REV="QUANT-NOVA-2.4.22-CORE-LIVENESS-EVENTLOOP-ISOLATION-V2.3-DIFF-GUARD-PERMISSION-FIX"
APP="quant-nova"
...
echo "PERMISSION_POLICY=ROOT_BACKUP+USER_OWNED_WORKING_COPY"
...
# docker cp writes files as root on this host. The patcher runs as the
# current ubuntu user, so make only the working copy readable/writable by it.
sudo chown "$(id -u):$(id -g)" "$NEW_MAIN"
chmod 600 "$NEW_MAIN"
test -r "$NEW_MAIN" && test -w "$NEW_MAIN"
echo "WORKING_MAIN_PERMISSION=PASS owner=$(stat -c '%U:%G' "$NEW_MAIN") mode=$(stat -c '%a' "$NEW_MAIN")"
...
echo "=== PATCH WORKING COPY ==="
python3 "$PATCHER" "$NEW_MAIN"
python3 -m py_compile "$NEW_MAIN"

echo "=== DIFF GUARD PERMISSION CHECK ==="
echo "OLD_MAIN_OWNER=$(stat -c '%U:%G %a' "$OLD_MAIN")"
echo "NEW_MAIN_OWNER=$(stat -c '%U:%G %a' "$NEW_MAIN")"
sudo test -r "$OLD_MAIN"
test -r "$NEW_MAIN"
echo "DIFF_GUARD_PERMISSION=PASS"

echo "=== DIFF GUARD ==="
# OLD_MAIN is intentionally root-owned/protected. Run only this read-only
# comparison as root instead of weakening backup permissions.
sudo python3 - "$OLD_MAIN" "$NEW_MAIN" <<'PY'
import ast,sys
from pathlib import Path
