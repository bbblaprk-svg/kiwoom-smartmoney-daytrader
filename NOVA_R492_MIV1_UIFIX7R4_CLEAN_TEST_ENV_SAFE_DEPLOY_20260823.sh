...
DEPLOY_SCRIPT_REV="UIFIX7R4_CLEAN_TEST_ENV_ISOLATION_FIX"
BASELINE_VERSION="NOVA-3.3.5-R492-MARKET-INDEX-VERIFY1"
IMAGE="quant-nova:3.3.5-r492-miv1-uifix7r2-sparse-io-local"
STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP="${APP}-pre-uifix7r4-${STAMP}"
CANDIDATE="${APP}-candidate-uifix7r4-${STAMP}"
WORK="$(mktemp -d /tmp/nova-r492-miv1-uifix7r4-cleantest.XXXXXX)"
DOCKERFILE="$WORK/Dockerfile"
ENVFILE="$WORK/current.env"
CAND_DATA="$WORK/candidate-data"
LOG="/tmp/nova-r492-miv1-uifix7r4-cleantest-deploy-${STAMP}.log"
LOCKFILE="/tmp/nova-r492-miv1-uifix7r4-cleantest-deploy.lock"
OLD_WAS_RUNNING=0
OLD_STOPPED=0
OLD_RENAMED=0
NEW_STARTED=0
...
exec 9>"$LOCKFILE"
if ! flock -n 9; then
  echo "INFO: R492 MIV1 UIFIX7R4 clean-test-env deploy is already running. Do not start it twice."
  exit 0
fi
...
say "4A/9 UNIT TESTS — CLEAN ONE-SHOT IMAGE ENV, NO PRODUCTION ENV / NO SHARED DATA"
echo "UNIT_TEST_ENV=CLEAN_IMAGE_DEFAULTS NO_ENVFILE=1 NETWORK=none DATA=/tmp/nova-test-data PYTHONPATH=/app" | tee -a "$LOG"
# Do not run the unit suite inside the candidate container. That container intentionally inherits
# production env values and owns the candidate data mount; both can alter Settings/test state.
# Unit tests verify the image/source contract and therefore run in a fresh one-shot container with
# only explicit neutral test variables, no broker network, and a disposable /tmp data directory.
dc run --rm --network none --memory 384m --memory-swap 384m --pids-limit 256 \
  ...
  -e NOVA_DATA_DIR=/tmp/nova-test-data -e NOVA_LEGACY_DATA_DIR=/tmp/nova-test-data \
  --workdir /app --entrypoint sh "$IMAGE" -c \
  "mkdir -p /tmp/nova-test-data && exec python -m unittest discover -s tests -p 'test_*.py' -q" 2>&1 | tee -a "$LOG"
