#!/usr/bin/env bash
# Compatible with PHASE2 CHART FIX9 CLOSE5 3.0.5 safe-cutover backup names.
set -Eeuo pipefail

CURRENT_CONTAINER="${NOVA_CURRENT_CONTAINER:-quant-nova}"
STAMP="$(date -u +%Y%m%d-%H%M%S)"
FAILED_CONTAINER="quant-nova-failed-precision-edge-x1-phase2-chart-fix9-close5-${STAMP}"
...
if [[ -z "${BACKUP_CONTAINER}" ]]; then
  BACKUP_CONTAINER="$(
    docker ps -a --format '{{.Names}}' |
      awk '/^quant-nova-backup-precision-edge-x1-phase2-chart-fix9-close5-[0-9]{8}-[0-9]{6}$/{print}' |
      sort -r | head -n 1
  )"
fi
[[ "${BACKUP_CONTAINER}" =~ ^quant-nova-backup-precision-edge-x1-phase2-chart-fix9-close5-[0-9]{8}-[0-9]{6}$ ]] ||
...
say "FAILED_PHASE2_CHART_FIX9_CLOSE5_CONTAINER=${FAILED_CONTAINER}"
say 'The failed PHASE2 CHART FIX9 CLOSE5 container was preserved for inspection and can be removed later by exact name.'
