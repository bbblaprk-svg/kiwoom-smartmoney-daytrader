...
ZIP_NAME="kiwoom-unified-edge-v3.3.5-NEWS156-OFFICIAL-TRADE-0A-FID9081.zip"
SOURCE_DIR_NAME="kiwoom-unified-edge-v3.3.5"
EXPECTED_ZIP_SHA256="13cefb42bf1bc0445fdf75ea866ae3b39fca4b5033b65a61dcfeefe7a73a2d34"
OLD_APP_DIR="$HOME/kiwoom-smartmoney-daytrader"
NEW_APP_DIR="$HOME/kiwoom-unified-edge"
TRADER_IMAGE="kiwoom-unified:v3.3.5"
...
rollback(){
  local ec=$?
  [[ $ec -eq 0 ]] && return
  printf '\n[자동복구] v3.3.5 OFFICIAL TRADE 0A 적용 실패 — 기존 컨테이너로 복구합니다.\n' >&2
...
say "v3.3.5 OFFICIAL TRADE 0A + FID9081 + News 1.5.6 파일 받기"
...
[[ "$TRADER_VERSION" == "3.3.5" ]] || fail "trader 버전 불일치: $TRADER_VERSION"
[[ "$NEWS_VERSION" == "1.5.6" ]] || fail "news 버전 불일치: $NEWS_VERSION"
grep -Fq "DOMESTIC_TRADE_0A" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "키움 공식 bare 종목코드 0A 주식체결 REG 누락"
grep -Fq "KIWOOM_OFFICIAL_DOMESTIC_BARE_REG_V4" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "공식 국내 WebSocket bare-code 등록모드 누락"
...
grep -Fq "DUPLICATE_SKIPPED" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "중복 0A 체결 억제 누락"
...
grep -Fq "REAL_0A_FID9081_ONLY" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "0A 실체결 FID9081 거래소 진실정책 누락"
! grep -Fq "DOMESTIC_TRADE_0B" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "구형 0B 체결 REG 잔존"
! grep -Fq "entry.type === '0B'" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "구형 0B tradeLike 판정 잔존"
grep -Fq 'pick.venue === 'NXT' ? `${pick.code}_NX` : pick.code' "$ROOT/trader/worker/kiwoomRealtime.js" || fail "REST NXT _NX 조회 경로 소실"
...
[[ "$RUNNING_VERSION" == "3.3.5" ]] || fail "실행 버전 불일치: $RUNNING_VERSION"

say "실제 수신 검증 — 공식 0A DOMESTIC REG ACK + FID9081 firstTick/fresh 관찰 (최대 90초)"
RUNTIME_OK=0
NXT_UNCONFIRMED=0
...
if [[ $RUNTIME_OK -ne 1 ]]; then
  ...
  fail "공식 국내 0A WebSocket REG ACK/연결 검증 실패. 기존 컨테이너로 자동 롤백합니다."
fi
