...
ZIP_NAME="kiwoom-unified-edge-v3.3.3-NEWS156-REALTIME-FOUNDATION-NXT-SUFFIX-ROUTE.zip"
SOURCE_DIR_NAME="kiwoom-unified-edge-v3.3.3"
EXPECTED_ZIP_SHA256="d3ea89708cf6c03648da19eae1e420e0c4c514f3dbf66e9a702d2d119a9d4440"
OLD_APP_DIR="$HOME/kiwoom-smartmoney-daytrader"
NEW_APP_DIR="$HOME/kiwoom-unified-edge"
TRADER_IMAGE="kiwoom-unified:v3.3.3"
...
rollback(){
  local ec=$?
  [[ $ec -eq 0 ]] && return
  printf '\n[자동복구] v3.3.3 NXT SUFFIX ROUTE 적용 실패 — 기존 컨테이너로 복구합니다.\n' >&2
...
say "v3.3.3 REALTIME FOUNDATION + NXT SUFFIX ROUTE + News 1.5.6 파일 받기"
...
[[ "$TRADER_VERSION" == "3.3.3" ]] || fail "trader 버전 불일치: $TRADER_VERSION"
[[ "$NEWS_VERSION" == "1.5.6" ]] || fail "news 버전 불일치: $NEWS_VERSION"
grep -Fq "DOMESTIC_TRADE_BARE_0B" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "KRX bare 6자리 0B 핵심 REG 누락"
grep -Fq 'NXT_CORE_${code}_0B' "$ROOT/trader/worker/kiwoomRealtime.js" || fail "NXT 종목별 suffix 0B CORE REG 누락"
grep -Fq '${code}_NX' "$ROOT/trader/worker/kiwoomRealtime.js" || fail "NXT _NX 거래소별 종목코드 REG 누락"
grep -Fq "DOMESTIC_BARE_KRX_PLUS_NXT_SUFFIX_0B_V3" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "v3.3.3 NXT suffix 등록모드 누락"
! grep -Eq "stex_tp[[:space:]]*:" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "국내 WebSocket에 미국주식용 stex_tp item-map 코드가 남아 있습니다"
! grep -Fq "NXT_TRADE_EXCHANGE_MAP_0B" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "구형 NXT exchange-map 등록 잔존"
...
[[ "$RUNNING_VERSION" == "3.3.3" ]] || fail "실행 버전 불일치: $RUNNING_VERSION"

say "실제 수신 검증 — NXT _NX REG ACK + FID9081-confirmed firstTick/fresh 확인 (최대 90초)"
RUNTIME_OK=0
...
nxtreg=r.get('NXT_SUFFIX') or {}
ack=int(dom.get('regAccepted') or 0)
nxt_ack=int(nxtreg.get('regAccepted') or 0)
rows=[f'DOMESTIC:ack={ack}', f'NXT_SUFFIX:ack={nxt_ack}']
...
for v in exp:
    ...
    vok=first>0 and fresh>0 and conf
...
if [[ "$ACTIVE_NOW" == "1" && $RUNTIME_OK -ne 1 ]]; then
  ...
  fail "활성 거래소의 실제 FID9081-confirmed firstTick/fresh 검증 실패. 기존 컨테이너로 자동 롤백합니다."
fi
