...
ZIP_NAME="kiwoom-unified-edge-v3.2.7-NEWS151-PIPELINEGUARD.zip"
SOURCE_DIR_NAME="kiwoom-unified-edge-v3.2.7"
EXPECTED_ZIP_SHA256="fc17c290cc345b7a77802c61c927cc2ed726e63b9f50b7438bcef69caf27604a"
OLD_APP_DIR="$HOME/kiwoom-smartmoney-daytrader"
NEW_APP_DIR="$HOME/kiwoom-unified-edge"
TRADER_IMAGE="kiwoom-unified:v3.2.7"
...
rollback() {
  ...
  printf '\n[자동 복구] v3.2.7 PIPELINE GUARD 적용 실패 — 기존 앱으로 복구합니다.\n' >&2
...
say "v3.2.7 PIPELINE GUARD 파일 받기"
...
# v3.2.7 compatibility check: 기존 trader와 동일하게 APP_ACCESS_TOKEN 최소 길이를 8자로 통일한다.
...
[[ "$TRADER_VERSION" == "3.2.7" ]] || fail "trader package 버전이 3.2.7이 아닙니다: $TRADER_VERSION"
...
# v3.2.7 추천용 시장발굴 + KRX/NXT dual-feed + pipeline guard. Kiwoom 실시간 WebSocket/TR/FID 수신 설정은 변경하지 않는다.
...
# v3.2.7 PIPELINE GUARD dual-feed 장애방어 기본값. 기존 사용자가 명시한 값은 보존한다.
...
# v3.2.7에서는 15분 전체 스냅샷 환경값을 사용하지 않는다. 오래된 값은 제거해 혼선을 방지한다.
...
say "v3.2.7 PIPELINE GUARD 공개앱 실행"
sudo docker run -d \
  --name "$APP_CONTAINER" \
  --restart unless-stopped \
  --network "$DOCKER_NETWORK" \
...
printf '\n============================================================\n'
printf 'KIWOOM UNIFIED EDGE v3.2.7 PIPELINE GUARD 설치 완료\n'
printf '공개 앱     : https://%s\n' "$DOMAIN"
printf '공개 컨테이너: kiwoom-app v3.2.7\n'
...
printf '\nSafari/PWA를 완전히 닫았다가 다시 열고 상단 v3.2.7 PIPELINE GUARD와 통합레이더 탭을 확인하세요.\n'
