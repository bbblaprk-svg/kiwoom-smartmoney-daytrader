...
ZIP_NAME="kiwoom-smartmoney-daytrader-v3.1.7-FINAL.zip"
SOURCE_DIR_NAME="kiwoom-smartmoney-daytrader-v3.1.7"
IMAGE_NAME="kiwoom-smartmoney:v3.1.7"
...
rollback() {
  ...
  printf '\n[자동 복구] v3.1.7 FINAL 적용 실패 — 기존 앱으로 되돌립니다.\n' >&2
...
say "v3.1.7 FINAL 파일 받기"
git clone --depth 1 "$REPO_URL" "$TMP_ROOT/repo"
cd "$TMP_ROOT/repo"
[[ -f "$ZIP_NAME" ]] || fail "GitHub 최상위에 $ZIP_NAME 파일이 없습니다."
EXPECTED_ZIP_SHA256="591ae123701de41f33c2a452c7298847a1ad3bc301c8eb923c145b5c107bdb7c"
...
COPY kiwoom-smartmoney-daytrader-v3.1.7-FINAL.zip /tmp/app.zip
RUN mkdir -p /tmp/source \
    && unzip -q /tmp/app.zip -d /tmp/source \
    && cp -a /tmp/source/kiwoom-smartmoney-daytrader-v3.1.7/. /app/ \
...
say "v3.1.7 FINAL Docker 이미지 빌드 — 검사·부하검사·Next 빌드 포함"
...
printf '\nSafari 탭을 완전히 닫았다가 다시 열고 v3.1.7, 신호 한눈에, 종합기록부를 확인하세요.\n'
