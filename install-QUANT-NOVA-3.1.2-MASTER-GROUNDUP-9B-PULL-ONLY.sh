...
VERSION="3.1.2"
REV="MASTER-GROUNDUP-9B-PULL-ONLY"
APP_VERSION="NOVA-3.1.2-MASTER-GROUNDUP-9B"
IMAGE="ghcr.io/bbblaprk-svg/kiwoom-smartmoney-daytrader:nova-3.1.2-master-groundup-9b"
EXPECTED_SOURCE_ZIP_SHA256="24f8523ac5eecd733b86b5c48368c499ba4064e364dda4aad03371d69863e878"
...
DEPLOY_DIR="$APP_DIR/deployments/nova3120-master-groundup9b-$STAMP"
BACKUP="quant-nova-backup-master-groundup9b-$STAMP"
CANDIDATE="quant-nova-candidate-master-groundup9b-$STAMP"
...
rollback(){
  ...
  echo "[AUTO-ROLLBACK] G9B 미완료 — 새 컨테이너 제거 및 기존 immutable 상태 복원" >&2
  ...
  if [[ $ACTIVE_DATA_MUTATED -eq 1 && -d "$BASE_DATA/nova30" ]]; then
    ...
    echo "[AUTO-ROLLBACK] G9B NOVA30 state restored; failed attempt preserved at $FAILED_DATA" >&2
  fi
...
sudo docker exec "$CANDIDATE" python -c "import json,urllib.request;j=json.load(urllib.request.urlopen('http://127.0.0.1:8000/api/readyz',timeout=2));assert j.get('ok') and j.get('version')=='NOVA-3.1.2-MASTER-GROUNDUP-9B',j;print('CANDIDATE_READYZ=PASS',j)"
...
curl -fsS --max-time 3 "http://127.0.0.1:${HOST_PORT}/api/livez" | python3 -c 'import json,sys;j=json.load(sys.stdin);assert j.get("ok") and j.get("version")=="NOVA-3.1.2-MASTER-GROUNDUP-9B",j;print("ACTIVE_LIVEZ=PASS",j)'
...
sudo docker exec "$CONTAINER" python -c "import json,urllib.request;j=json.load(urllib.request.urlopen('http://127.0.0.1:8000/api/readyz',timeout=2));assert j.get('ok') and j.get('version')=='NOVA-3.1.2-MASTER-GROUNDUP-9B',j;print('ACTIVE_READYZ=PASS',j)"
...
if [[ -d "$GUARD_STATIC" ]] && sudo docker inspect nova-http-guard >/dev/null 2>&1; then
  mkdir -p "$GUARD_BK" "$TMP/g9b-static"
  ...
  for f in index.html nova.js nova.css manifest.webmanifest sw.js; do
    sudo docker cp "$CONTAINER:/app/static/$f" "$TMP/g9b-static/$f"
  done
  grep -Fq "h['X-App-Token']=token" "$TMP/g9b-static/nova.js" || die "G9B source-level X-App-Token UI 없음"
  grep -Fq "h['Authorization']='Bearer '+token" "$TMP/g9b-static/nova.js" || die "G9B source-level Authorization UI 없음"
  grep -Fq "startLivePoll" "$TMP/g9b-static/nova.js" || die "G9B permanent UI poll 없음"
  ! grep -Eq '__NOVA_INLINE_AUTH_V3__|__NOVA_ALWAYS_ON_POLL_R7__|NOVA_R6|NOVA_R7' "$TMP/g9b-static/nova.js" || die "debug JS lineage 발견"
  ...
  for f in index.html nova.js nova.css manifest.webmanifest sw.js; do
    sudo cp "$TMP/g9b-static/$f" "$GUARD_STATIC/.$f.nova312.tmp"
...
PYGUARD
  ...
  curl -ksS --max-time 5 "https://3-38-25-20.nip.io/static/nova.js?g9b=$STAMP" -o "$PUBLIC_JS"
  ...
  curl -ksS --max-time 5 "https://3-38-25-20.nip.io/api/auth/check?g9b0=$STAMP" -o "$A0"
  if [[ -n "$AUTH_TOKEN" ]]; then
    A3="$TMP/auth3.json"; A4="$TMP/auth4.json"
    curl -ksS --max-time 5 -H "X-App-Token: $AUTH_TOKEN" "https://3-38-25-20.nip.io/api/auth/check?g9b1=$STAMP" -o "$A1"
    curl -ksS --max-time 5 "https://3-38-25-20.nip.io/api/auth/check?g9b2=$STAMP" -o "$A2"
    curl -ksS --max-time 5 -H "Authorization: Bearer $AUTH_TOKEN" "https://3-38-25-20.nip.io/api/auth/check?g9b3=$STAMP" -o "$A3"
    curl -ksS --max-time 5 "https://3-38-25-20.nip.io/api/auth/check?g9b4=$STAMP" -o "$A4"
