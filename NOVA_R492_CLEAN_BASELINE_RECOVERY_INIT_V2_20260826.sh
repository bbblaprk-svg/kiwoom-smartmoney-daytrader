...
CUTOVER_STARTED=1
dc run -d --init --name "$APP" --restart "$RESTART" --env-file "$ENVFILE" \
...
say "6. SINGLE-INSTANCE + INIT + HEALTH + PROCESS ACCEPTANCE"
INIT_ENABLED="$(dc inspect "$APP" --format '{{.HostConfig.Init}}' 2>/dev/null || true)"
echo "DOCKER_INIT=$INIT_ENABLED" | tee -a "$LOG"
[ "$INIT_ENABLED" = "true" ] || { echo "FAIL: Docker --init is not enabled"; exit 9; }
...
sleep 35
HOST_Z_RECHECK="$(zombies)"
echo "HOST_ZOMBIES_RECHECK_35S=$HOST_Z_RECHECK" | tee -a "$LOG"
[ "$HOST_Z_RECHECK" -le 2 ] || { echo "FAIL: zombies re-accumulated after --init: $HOST_Z_RECHECK"; exit 10; }
echo "RESULT=SUCCESS BASELINE_SHA=$BASE_SHA SINGLE_NOVA=1 DOCKER_INIT=1 BACKUP_STOPPED=$BACKUP LOG=$LOG" | tee -a "$LOG"
