#!/usr/bin/env bash
set -Eeuo pipefail
REV="QUANT-NOVA-2.4.22-UI-TRUTH-CONSOLIDATION-V2.1-CADDY-SYNC"
APP="quant-nova"
CADDY="kiwoom-caddy"
STAMP="$(date +%Y%m%d%H%M%S)"
WORK="/tmp/nova-v21-caddy-$STAMP"
mkdir -p "$WORK"
chmod 700 "$WORK"

echo "=== $REV START ==="
echo "PURPOSE=APPLY_V2_UI_AND_SYNC_ACTUAL_CADDY_STATIC_PATH"
...
EMBEDDED_V2
chmod +x "$WORK/apply-v2.sh"

echo "=== APPLY V2 INSIDE QUANT-NOVA ==="
bash "$WORK/apply-v2.sh"
MAIN_AFTER_V2="$(sudo docker exec "$APP" sha256sum /app/app/main.py | awk '{print $1}')"
[[ "$MAIN_BEFORE" == "$MAIN_AFTER_V2" ]] || { echo "MAIN_PY_CHANGED_AFTER_V2=FAIL" >&2; exit 1; }
echo "MAIN_PY_AFTER_V2=UNCHANGED"
sudo docker exec "$APP" sh -c "grep -q 'NOVA_UI_TRUTH_CONSOLIDATION_V2_START' /app/static/nova.js"
echo "QUANT_NOVA_V2_MARKER=PASS"

CADDYFILE="$WORK/Caddyfile"
sudo docker exec "$CADDY" sh -c 'cat /etc/caddy/Caddyfile 2>/dev/null || cat /config/caddy/Caddyfile 2>/dev/null || true' > "$CADDYFILE"
[[ -s "$CADDYFILE" ]] || { echo "CADDYFILE_DISCOVERY=FAIL" >&2; exit 1; }
echo "=== CADDYFILE ==="
cat "$CADDYFILE"
echo "=== CADDY MOUNTS ==="
sudo docker inspect "$CADDY" --format '{{range .Mounts}}{{println .Source "->" .Destination}}{{end}}'
...
mapfile -t ROOTS < <(awk '/^[[:space:]]*#/ {next} {for(i=1;i<=NF;i++){if($i=="root"){if($(i+1)=="*" && $(i+2)!=""){print $(i+2)} else if($(i+1)!=""){print $(i+1)}}}}' "$CADDYFILE" | sed 's/[{;]$//' | sort -u)
SYNCED=0
for ROOT in "${ROOTS[@]:-}"; do
  [[ -n "$ROOT" ]] || continue
  echo "CADDY_ROOT_CANDIDATE=$ROOT"
  if sudo docker exec "$CADDY" sh -c "test -d '$ROOT'"; then
    sudo docker exec "$CADDY" sh -c "mkdir -p '$ROOT/static'"
    sudo docker exec "$CADDY" sh -c "test -f '$ROOT/index.html' && cp '$ROOT/index.html' '$ROOT/index.html.before-$STAMP' || true"
    sudo docker exec "$CADDY" sh -c "test -f '$ROOT/static/nova.js' && cp '$ROOT/static/nova.js' '$ROOT/static/nova.js.before-$STAMP' || true"
    sudo docker cp "$WORK/index.html" "$CADDY:$ROOT/index.html"
    sudo docker cp "$WORK/nova.js" "$CADDY:$ROOT/static/nova.js"
    if sudo docker exec "$CADDY" sh -c "grep -q 'NOVA_UI_TRUTH_CONSOLIDATION_V2_START' '$ROOT/static/nova.js'"; then
      echo "CADDY_STATIC_SYNC=PASS root=$ROOT"
      SYNCED=1
    fi
  fi
done
  ...
  for ROOT in "${ROOTS[@]:-}"; do
    case "$ROOT" in
      "$DST"|"$DST"/*)
        ...
        HOSTROOT="$SRC$REL"
        if [[ -d "$HOSTROOT" ]]; then
          echo "HOST_STATIC_ROOT=$HOSTROOT"
          sudo mkdir -p "$HOSTROOT/static"
          [[ -f "$HOSTROOT/index.html" ]] && sudo cp "$HOSTROOT/index.html" "$HOSTROOT/index.html.before-$STAMP" || true
          [[ -f "$HOSTROOT/static/nova.js" ]] && sudo cp "$HOSTROOT/static/nova.js" "$HOSTROOT/static/nova.js.before-$STAMP" || true
          sudo cp "$WORK/index.html" "$HOSTROOT/index.html"
          sudo cp "$WORK/nova.js" "$HOSTROOT/static/nova.js"
          grep -q 'NOVA_UI_TRUTH_CONSOLIDATION_V2_START' "$HOSTROOT/static/nova.js" && echo "HOST_STATIC_SYNC=PASS root=$HOSTROOT"
        fi
        ;;
    esac
  done
done < <(sudo docker inspect "$CADDY" --format '{{range .Mounts}}{{println .Source "\t" .Destination}}{{end}}')

if [[ "$SYNCED" -eq 0 ]]; then
  echo "CADDY_STATIC_ROOT_NOT_FOUND=YES"
  echo "NO_PROXY_REWRITE_PERFORMED=SAFETY"
fi
...
PUBLIC_MARKER=0
grep -q 'NOVA_UI_TRUTH_CONSOLIDATION_V2_START' "$PUB_JS" && PUBLIC_MARKER=1 || true
echo "PUBLIC_V2_MARKER=$PUBLIC_MARKER"
PUBLIC_VER="$(grep -oE 'nova\.js\?v=[^"'"'"']+' "$PUB_HTML" | tail -1 || true)"
echo "PUBLIC_HTML_JS_VERSION=${PUBLIC_VER:-NOT_FOUND}"

MAIN_FINAL="$(sudo docker exec "$APP" sha256sum /app/app/main.py | awk '{print $1}')"
[[ "$MAIN_BEFORE" == "$MAIN_FINAL" ]] || { echo "FINAL_MAIN_PY_HASH_GATE=FAIL" >&2; exit 1; }
echo "FINAL_MAIN_PY_HASH_GATE=PASS"

for ep in /api/health /api/nova /api/opening-shakeout-reversal /api/close-picks; do
  C="$(curl -sS --max-time 4 -o /dev/null -w '%{http_code}' "http://127.0.0.1:3200$ep" || true)"
  echo "$ep=$C"
done

if [[ "$PUBLIC_MARKER" -ne 1 ]]; then
  echo "PUBLIC_UI_FIX=NOT_YET_SERVING_FROM_DETECTED_STATIC_ROOT"
  echo "DATA_LOGIC=UNCHANGED"
  echo "MAIN_PY=UNCHANGED"
  exit 2
fi

echo "PUBLIC_UI_FIX=PASS"
echo "CLOSEPICKS_08:50_HIDE=ENABLED"
echo "OPENING_SHAKEOUT_TABLE=ENABLED"
