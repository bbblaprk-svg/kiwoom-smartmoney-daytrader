...
DEPLOY_SCRIPT_REV="UIFIX7R2_BUILD_DETERMINISTIC_PLUS_STRICT_PERF_GATE"
BASELINE_VERSION="NOVA-3.3.5-R492-MARKET-INDEX-VERIFY1"
IMAGE="quant-nova:3.3.5-r492-miv1-uifix7r2-sparse-io-local"
STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP="${APP}-pre-uifix7r2-${STAMP}"
CANDIDATE="${APP}-candidate-uifix7r2-${STAMP}"
WORK="$(mktemp -d /tmp/nova-r492-miv1-uifix7r2-sparse-io.XXXXXX)"
DOCKERFILE="$WORK/Dockerfile"
ENVFILE="$WORK/current.env"
CAND_DATA="$WORK/candidate-data"
LOG="/tmp/nova-r492-miv1-uifix7r2-sparse-io-deploy-${STAMP}.log"
LOCKFILE="/tmp/nova-r492-miv1-uifix7r2-sparse-io-deploy.lock"
OLD_WAS_RUNNING=0
OLD_STOPPED=0
OLD_RENAMED=0
NEW_STARTED=0
...
exec 9>"$LOCKFILE"
if ! flock -n 9; then
  echo "INFO: R492 MIV1 UIFIX7R2 SPARSE IO deploy is already running. Do not start it twice."
  exit 0
fi
...
strict_rotation_image_gate(){
  ...
  echo "PERF_POLICY=UNCHANGED_TEST d5951f873d9f97e2d4f7c1c50bb1e02f3c20e5d1ac4556de872c57670a7586a9 · REQUIRE=2_CONSECUTIVE_PASS · candidate_web_server=OFF · BUILD_PHASE_TIMING_TEST=DEFERRED_TO_THIS_GATE" | tee -a "$LOG"
