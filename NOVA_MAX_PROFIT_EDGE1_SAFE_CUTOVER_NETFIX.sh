...
EXPECTED_DOCKERFILE_SHA256="65fd8ea9aa3f8acaada557964d75af69461bbf3a5966a1e2e5de09c1f98cec0e"
...
if dc inspect "$APP" >/dev/null 2>&1; then
  ...
  chmod 600 "$ENVFILE"
  mapfile -t NETWORKS < <(dc inspect "$APP" --format '{{range $name, $_ := .NetworkSettings.Networks}}{{println $name}}{{end}}' | sed '/^[[:space:]]*$/d')
  NETWORK_ARGS=()
  if [[ "${#NETWORKS[@]}" -gt 0 && -n "${NETWORKS[0]}" ]]; then
    NETWORK_ARGS=( --network "${NETWORKS[0]}" )
  fi
  ...
  OLD_RENAMED=1
  dc run -d --name "$APP" --restart "$RESTART" "${NETWORK_ARGS[@]}" --env-file "$ENVFILE" \
    "${RESOURCE_ARGS[@]}" "${LOG_ARGS[@]}" "${MOUNT_ARGS[@]}" "${PORT_ARGS[@]}" "$IMAGE" >/dev/null
  NEW_STARTED=1
  if [[ "${#NETWORKS[@]}" -gt 1 ]]; then
    for network in "${NETWORKS[@]:1}"; do
      [[ -n "$network" ]] && dc network connect "$network" "$APP" >/dev/null
    done
  fi
else
