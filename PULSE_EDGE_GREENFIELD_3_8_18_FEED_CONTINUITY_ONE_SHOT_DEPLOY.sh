...
RELEASE="3.8.18_FEED_CONTINUITY"
...
SRC_NAME="PULSE_EDGE_GREENFIELD_3_8_18_FEED_CONTINUITY_SOURCE.tar.gz"
EXPECTED_SRC_SHA="6e212c6990832b313eec461b734a982e32473e97b2f473f82b815018f835afff"
EXPECTED_DF_SHA="786c6fad43a03f287840eeafb0d5f20afddf4755900e8f9c9156e9f810a14f67"
MODE="${1:-deploy}"
SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)"
RUN_ID="$(date +%Y%m%d-%H%M%S)-$$"
RUN_DIR="$BASE/deployments/$RUN_ID"
CANARY_DATA="$RUN_DIR/canary-data"
IMG="pulse-edge:gf3818-hidden-$RUN_ID"
CANARY="pulse-edge-gf3818-canary-$RUN_ID"
LIVE="pulse-edge-gf3818-live-$RUN_ID"
OLD=""
OLD_RUNNING=false
OLD_RESTART="no"
CUTOVER=0
...
fixed={
'PULSE_RELEASE':'3.8.18_FEED_CONTINUITY','PULSE_FEED_ENABLED':'true','PULSE_DATA_DIR':'/app/data/pulse_edge',
...
import json,sys
q=json.load(open(sys.argv[1])); assert q['release']=='3.8.18_FEED_CONTINUITY' and q['ready'] is True and q['feed_enabled'] is False
PY
  fi
...
import json,sys
q=json.load(open(sys.argv[1])); assert q.get('release')=='3.8.18_FEED_CONTINUITY' and isinstance(q.get('rows'),list),q
...
import json,sys
h=json.load(open(sys.argv[1])); assert h.get('runtime_safety_release')=='3.8.18_FEED_CONTINUITY'
PY
public=0
