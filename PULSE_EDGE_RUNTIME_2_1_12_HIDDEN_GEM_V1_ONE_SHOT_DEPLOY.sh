#!/usr/bin/env bash
set -Eeuo pipefail
# PULSE EDGE 2.1.12 HIDDEN GEM V1 — replacement decision engine + preserved runtime recovery.
# Stable feed/runtime/CPU/route recovery are preserved; current ranking/decision authority is HIDDEN_GEM_V1.
# Docker-network canary only; no host-port/userland-proxy dependency. Run with bash, never source this file.
...
RELEASE=2.1.12_HIDDEN_GEM_V1
EXPECTED_DF_SHA=aa495ceb987e6a05c72a7d29b5708a7a609f9c871d78a83d6b089d2a8b54ea17
EXPECTED_SRC_SHA=d4ded4a558b57570725266e0db63385da96e251ed5c45f49580babd1e1af23f9
SRC_NAME=PULSE_EDGE_CORE_2_1_3_RUNTIME_2_1_12_HIDDEN_GEM_V1_SOURCE.tar.gz
...
detach(){
  ...
  if docker inspect "$1" --format '{{json .NetworkSettings.Networks}}' |
    ...
    docker network disconnect -f "$NET" "$1" >/dev/null || return 1
  fi
}
...
container_net_ip(){
  docker inspect -f "{{with index .NetworkSettings.Networks \"$NET\"}}{{.IPAddress}}{{end}}" "$1" 2>/dev/null || true
}

direct_health(){
  local c="$1" ip i
  running "$c" || return 1
  ip="$(container_net_ip "$c")"
  [ -n "$ip" ] || return 1
  for i in $(seq 1 12); do
    if curl -fsS --connect-timeout 2 --max-time 4 "http://$ip:8000/health" >/dev/null 2>&1; then return 0; fi
    sleep 1
  done
  return 1
}
...
verify_data_mount(){
  local c="$1" src
  src="$(docker inspect -f '{{range .Mounts}}{{if eq .Destination "/app/data"}}{{.Source}}{{end}}{{end}}' "$c" 2>/dev/null || true)"
  [ -n "$src" ] && [ "$(readlink -f "$src")" = "$(readlink -f "$DATA_DIR")" ]
}

managed_run_id(){
  local c="$1" r
  r="$(docker inspect -f '{{index .Config.Labels "io.pulse-edge.run"}}' "$c" 2>/dev/null || true)"
  if [ -n "$r" ] && [ "$r" != '<no value>' ]; then printf '%s\n' "$r"; return 0; fi
  case "$c" in
    pulse-edge-live-runtime-2.1.11.10-*) printf '%s\n' "${c#pulse-edge-live-runtime-2.1.11.10-}" ;;
    pulse-edge-canary-runtime-2.1.11.10-*) printf '%s\n' "${c#pulse-edge-canary-runtime-2.1.11.10-}" ;;
    pulse-edge-live-runtime-2.1.11.9-*) printf '%s\n' "${c#pulse-edge-live-runtime-2.1.11.9-}" ;;
    pulse-edge-canary-runtime-2.1.11.9-*) printf '%s\n' "${c#pulse-edge-canary-runtime-2.1.11.9-}" ;;
    *) return 1 ;;
  esac
}

reconcile_route_holders(){
  local c role project rid release preferred
  local -a holders_now=()
  while IFS= read -r c; do
    [ -n "$c" ] || continue
    if ! running "$c"; then
      log "DETACH_STOPPED_STALE_ALIAS=$c"
      detach "$c" || die "could not detach stopped stale route owner: $c"
      continue
    fi
    project="$(docker inspect -f '{{index .Config.Labels "io.pulse-edge.project"}}' "$c" 2>/dev/null || true)"
    role="$(docker inspect -f '{{index .Config.Labels "io.pulse-edge.role"}}' "$c" 2>/dev/null || true)"
    rid="$(managed_run_id "$c" 2>/dev/null || true)"
    # A canary must never own the stable route. Preserve its container, but stop/detach it.
    if [ "$project" = PULSE_EDGE ] && [ "$role" = canary ]; then
      log "STOP_STALE_CANARY_ROUTE_OWNER=$c"
      stop_verified "$c" || die "could not stop stale canary route owner: $c"
      detach "$c" || die "could not detach stale canary route owner: $c"
      continue
    fi
    # Any managed 2.1.11.9/2.1.11.10 live name with a known run directory but no deploy-passed marker is objectively a failed/orphan run.
    # Do not depend on labels: early R2/R3 containers may not carry the newer project/role labels, yet Docker can resurrect them.
    if [[ "$c" = pulse-edge-live-runtime-2.1.11.9-* || "$c" = pulse-edge-live-runtime-2.1.11.10-* ]] && [ -n "$rid" ] && [ -d "$BASE/deployments/$rid" ] && [ ! -f "$BASE/deployments/$rid/deploy-passed" ]; then
      log "STOP_FAILED_MANAGED_ROUTE_OWNER=$c run=$rid"
      stop_verified "$c" || die "could not stop failed prior deployment route owner: $c"
      detach "$c" || die "could not detach failed prior deployment route owner: $c"
      continue
    fi
    # Current-label path for managed lives not covered by the historical-name guard above.
    if [ "$project" = PULSE_EDGE ] && [ "$role" = live ] && [ -n "$rid" ] && [ -d "$BASE/deployments/$rid" ] && [ ! -f "$BASE/deployments/$rid/deploy-passed" ]; then
      log "STOP_FAILED_DEPLOY_ROUTE_OWNER=$c run=$rid"
      stop_verified "$c" || die "could not stop failed prior deployment route owner: $c"
      detach "$c" || die "could not detach failed prior deployment route owner: $c"
      continue
    fi
  ...
  # Second pass: deterministic arbitration for historical failed managed lives whose
  # deployment directory/labels may be incomplete. This exact stable baseline is already
  # the configured production fallback for this installation; use it only if it is RUNNING,
  # directly healthy on kiwoom-net, and mounted to the explicit PULSE data directory.
  mapfile -t holders_now < <(alias_holders)
  if [ "${#holders_now[@]}" -gt 1 ]; then
    preferred="${PULSE_DEPLOY_PREFERRED_BASELINE:-pulse-edge-live-ui-2.1.9-preopen-visible}"
    if printf '%s\n' "${holders_now[@]}" | grep -Fxq "$preferred"; then
      running "$preferred" || die "preferred baseline is not running: $preferred"
      verify_data_mount "$preferred" || die "preferred baseline does not use explicit PULSE data: $preferred"
      direct_health "$preferred" || die "preferred baseline direct health failed; refusing route arbitration: $preferred"
      log "PREFERRED_BASELINE_CONFIRMED=$preferred"
      for c in "${holders_now[@]}"; do
        [ "$c" = "$preferred" ] && continue
        project="$(docker inspect -f '{{index .Config.Labels "io.pulse-edge.project"}}' "$c" 2>/dev/null || true)"
        role="$(docker inspect -f '{{index .Config.Labels "io.pulse-edge.role"}}' "$c" 2>/dev/null || true)"
        release="$(docker inspect -f '{{index .Config.Labels "io.pulse-edge.release"}}' "$c" 2>/dev/null || true)"
        case "$c" in
          pulse-edge-live-runtime-2.1.11.9-*|pulse-edge-canary-runtime-2.1.11.9-*|pulse-edge-live-runtime-2.1.11.10-*|pulse-edge-canary-runtime-2.1.11.10-*)
            log "STOP_PRIOR_2119_ROUTE_OWNER=$c"
            stop_verified "$c" || die "could not stop prior 2.1.11.9 route owner: $c"
            detach "$c" || die "could not detach prior 2.1.11.9 route owner: $c"
            ;;
          *)
            if [ "$project" = PULSE_EDGE ] && { [ "$role" = canary ] || { [ "$role" = live ] && [ "$release" = "$RELEASE" ]; }; }; then
              log "STOP_PRIOR_MANAGED_ROUTE_OWNER=$c"
              stop_verified "$c" || die "could not stop prior managed route owner: $c"
              detach "$c" || die "could not detach prior managed route owner: $c"
            else
              die "ambiguous running route owner remains beside verified baseline: $c"
            fi
            ;;
        esac
      done
    ...
    for root in roots:
        for p in sorted(root.glob(pattern)):
            if p.is_file() and hashlib.sha256(p.read_bytes()).hexdigest()==expected:
                found.append(p)
    if not found: raise SystemExit(f'{label}: matching 2.1.12 HIDDEN_GEM_V1 file missing or modified; put all 3 files together')
...
# Reconcile only objectively stale PULSE route owners. Never delete containers/images/data.
# This handles the exact failure mode where Docker restart resurrects a previously failed 2.1.11.9 live container.
reconcile_route_holders
mapfile -t holders < <(alias_holders)
[ "${#holders[@]}" -le 1 ] || die "multiple legitimate RUNNING pulse-edge alias owners remain after stale-run reconciliation; refusing to guess: ${holders[*]}"
...
IMG="pulse-edge:runtime-2.1.12-hidden-gem-v1-$RUN_ID"
CANARY="pulse-edge-canary-runtime-2.1.12-hidden-gem-v1-$RUN_ID"
LIVE="pulse-edge-live-runtime-2.1.12-hidden-gem-v1-$RUN_ID"
save_state
...
docker run -d --name "$CANARY" --restart=no \
  --network "$NET" \
  --label io.pulse-edge.project=PULSE_EDGE --label io.pulse-edge.role=canary --label "io.pulse-edge.run=$RUN_ID" --label "io.pulse-edge.release=$RELEASE" \
...
docker run -d --name "$LIVE" --restart=unless-stopped \
  --label io.pulse-edge.project=PULSE_EDGE --label io.pulse-edge.role=live --label "io.pulse-edge.run=$RUN_ID" --label "io.pulse-edge.release=$RELEASE" \
