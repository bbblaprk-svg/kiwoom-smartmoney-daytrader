...
ZIP_NAME="kiwoom-unified-edge-v3.2.16-NEWS156-PRE-IGNITION-VERIFIED-REALTIME.zip"
SOURCE_DIR_NAME="kiwoom-unified-edge-v3.2.16"
EXPECTED_ZIP_SHA256="ca81fbbdaec72ffd57108848449bf60655b52e496f77caee64fd26530a80de62"
OLD_APP_DIR="$HOME/kiwoom-smartmoney-daytrader"
NEW_APP_DIR="$HOME/kiwoom-unified-edge"
TRADER_IMAGE="kiwoom-unified:v3.2.16"
...
rollback() {
  local ec=$?
  [[ $ec -eq 0 ]] && return
  printf '\n[자동 복구] v3.2.16 PRE-IGNITION VERIFIED REALTIME 적용 실패 — 기존 앱으로 복구합니다.\n' >&2
...
say "v3.2.16 PRE-IGNITION VERIFIED REALTIME + News v1.5.6 파일 받기"
...
[[ "$TRADER_VERSION" == "3.2.16" ]] || fail "trader package 버전 불일치: $TRADER_VERSION"
...
grep -Fq "const FOCUS_TYPES = ['0A', '0C', '0E', '0u'];" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "공식 FOCUS realtime 타입 불일치"
...
# 실시간 판단 정책은 과거 .env 값보다 v3.2.16 정책을 우선한다.
...
say "Trader v3.2.16 PRE-IGNITION VERIFIED REALTIME 실행"
...
# REG은 비동기이므로 잠시 기다리고, KRX/NXT degraded는 실패 처리한다.
REG_CHECK=""
for _ in $(seq 1 30); do
  REG_CHECK="$(sudo docker exec "$APP_CONTAINER" node -e "fetch('http://127.0.0.1:3000/api/health').then(r=>r.json()).then(h=>{const v=h.venueRegistration||{};const vals=Object.values(v).map(String);const pending=vals.some(x=>x==='pending');const fatal=vals.some(x=>x.startsWith('degraded'));console.log(JSON.stringify({pending,fatal,registration:v}));process.exit(fatal?2:pending?1:0)}).catch(()=>process.exit(1))" 2>/dev/null || true)"
  [[ "$REG_CHECK" == *'"fatal":true'* ]] && fail "실시간 REG degraded: $REG_CHECK"
...
(async()=>{
  ...
  console.log(JSON.stringify({status:finalStatus,failed,krxActive,nxtActive,krxFresh:k,nxtFresh:n,routeStats:routes,feedReady:last?.feedReady,venueFeeds:last?.venueFeeds,socketFrameAgeMs:last?.socketFrameAgeMs,venueRecovery:last?.venueRecovery||null}));
...
printf '\n============================================================\n'
printf 'KIWOOM UNIFIED EDGE v3.2.16 PRE-IGNITION VERIFIED REALTIME 설치 완료\n'
printf '공개 앱      : https://%s\n' "$DOMAIN"
printf 'Trader       : kiwoom-unified:v3.2.16\n'
printf 'News         : v1.5.6 ROTATION PICKS / discovery-only realtime bridge\n'
printf '현재가 기준  : 활성장 Kiwoom WebSocket 0A ONLY\n'
printf '판단 기준    : 체결0A·호가잔량0C·프로그램0w·거래원0F 실시간만 동적판정\n'
...
printf '\nSafari/PWA를 완전히 닫았다가 다시 열고 상단 v3.2.16를 확인하세요. 장중에는 카드의 실시간 0A 수신시각도 함께 확인하세요.\n'
