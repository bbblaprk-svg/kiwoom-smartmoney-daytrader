...
SRC_NAME="PULSE_EDGE_GREENFIELD_3_8_17_NOW_REPAIR_SOURCE.tar.gz"
EXPECTED_SRC_SHA="c88b3fd535ccc9539d9dc97de2c32d708c19477964c49310d9c8a382f8b72e5e"
EXPECTED_DF_SHA="dd999a0913aa970a3be68e2dcf289824309dae1ec0b8ce0c3e6dd090d1e05af1"
MODE="${1:-deploy}"
SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)"
RUN_ID="$(date +%Y%m%d-%H%M%S)-$$"
RUN_DIR="$BASE/deployments/$RUN_ID"
CANARY_DATA="$RUN_DIR/canary-data"
IMG="pulse-edge:gf3817-hidden-$RUN_ID"
CANARY="pulse-edge-gf3817-canary-$RUN_ID"
LIVE="pulse-edge-gf3817-live-$RUN_ID"
OLD=""
OLD_RUNNING=false
OLD_RESTART="no"
CUTOVER=0
...
fixed={
'PULSE_RELEASE':'3.8.17_NOW_REPAIR','PULSE_FEED_ENABLED':'true','PULSE_DATA_DIR':'/app/data/pulse_edge',
...
import json,sys
q=json.load(open(sys.argv[1])); assert q['release']=='3.8.17_NOW_REPAIR' and q['ready'] is True and q['feed_enabled'] is False
PY
  fi
...
import json,sys
q=json.load(open(sys.argv[1])); assert q.get('release')=='3.8.17_NOW_REPAIR' and isinstance(q.get('rows'),list),q
...
import json,sys
h=json.load(open(sys.argv[1])); assert h.get('runtime_safety_release')=='3.8.17_NOW_REPAIR'
PY
public=0
