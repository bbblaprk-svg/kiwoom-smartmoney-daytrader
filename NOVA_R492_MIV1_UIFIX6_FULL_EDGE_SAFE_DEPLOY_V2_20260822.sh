...
DEPLOY_SCRIPT_REV="V2_TEST_ENV_ISOLATION_FIX"
...
trap - ERR INT TERM EXIT
cleanup
echo "RESULT=SUCCESS VERSION=$EXPECTED_VERSION UIFIX=$EXPECTED_UIFIX_LABEL DEPLOY_REV=$DEPLOY_SCRIPT_REV CURRENT=$APP ROLLBACK_CONTAINER=$BACKUP" | tee -a "$LOG"
