...
ZIP_NAME="kiwoom-smartmoney-daytrader-v3.1.1.zip"
IMAGE_NAME="kiwoom-smartmoney:v3.1.1"
...
rollback() {
  ...
  printf '\n[자동 복구] v3.1.1 적용 실패 — 기존 앱으로 되돌립니다.\n' >&2
...
cat > "$TMP_ROOT/Dockerfile.v311" <<'DOCKER'
FROM node:22-alpine AS build
RUN apk add --no-cache unzip
WORKDIR /app
ENV NEXT_TELEMETRY_DISABLED=1
COPY kiwoom-smartmoney-daytrader-v3.1.1.zip /tmp/app.zip
RUN mkdir -p /tmp/source \
    && unzip -q /tmp/app.zip -d /tmp/source \
    && cp -a /tmp/source/kiwoom-smartmoney-daytrader-v3.1.1/. /app/ \
...
say "v3.1.1 Docker 이미지 빌드 — 5~15분 걸릴 수 있습니다"
sudo docker build --pull -f "$TMP_ROOT/Dockerfile.v311" -t "$IMAGE_NAME" .
...
printf '\nSafari에서 탭을 완전히 닫았다가 다시 열고, 상단에 v3.1.1 표시를 확인하세요.\n'
