#!/usr/bin/env bash
# Revision: PHASE2-CHART-FIX9-3_0
set -Eeuo pipefail

PRODUCT='NOVA PRECISION EDGE X1 PHASE2 · CHART FIX9'
VERSION='NOVA-PRECISION-EDGE-X1-PHASE2-CHART-FIX9-3.0.0'
CURRENT_CONTAINER="${NOVA_CURRENT_CONTAINER:-quant-nova}"
DOCKERFILE_NAME='QUANT_NOVA_PRECISION_EDGE_X1_PHASE2_CHART_FIX9.Dockerfile'
EXPECTED_DOCKERFILE_SHA256='da408eb0618ad5f8a756cdd9643f10e86749659265d24ba552c7677772ffd28d'
...
IMAGE_TAG="quant-nova:precision-edge-x1-phase2-chart-fix9-${STAMP}"
BACKUP_CONTAINER="quant-nova-backup-precision-edge-x1-phase2-chart-fix9-${STAMP}"
CANARY_CONTAINER="quant-nova-canary-precision-edge-x1-phase2-chart-fix9-${STAMP}"
BUILD_CONTEXT=''
CUTOVER_STARTED=0
...
BUILD_CONTEXT="$(mktemp -d -t nova-precision-x1-phase2-chart-fix9-build.XXXXXX)"
...
assert edge.get("version")=="NOVA-PRECISION-EDGE-X1-PHASE2-CHART-FIX9-3.0.0",edge
...
print("PHASE2_CHART_FIX9_OFFLINE_CANARY=PASS")
'
...
assert j.get("ok") and j.get("version")=="NOVA-PRECISION-EDGE-X1-PHASE2-CHART-FIX9-3.0.0",j
...
print("PRECISION_CHART_FIX9_API_CONTRACT=PASS PHASE="+str(j.get("phase")))
'
...
done < <(docker images 'quant-nova' --format '{{.Repository}}:{{.Tag}}' | awk '/^quant-nova:precision-edge-x1-phase2-chart-fix9-[0-9]{8}-[0-9]{6}$/')
...
say "MANUAL_ROLLBACK=./NOVA_PRECISION_EDGE_X1_PHASE2_CHART_FIX9_MANUAL_ROLLBACK.sh ${BACKUP_CONTAINER}"
