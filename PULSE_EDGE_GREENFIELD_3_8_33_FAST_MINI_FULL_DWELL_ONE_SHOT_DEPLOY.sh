...
RELEASE="3.8.33_FAST_MINI_FULL_DWELL"
...
SRC_NAME="PULSE_EDGE_GREENFIELD_3_8_33_FAST_MINI_FULL_DWELL_SOURCE_FINAL.tar.gz"
EXPECTED_SRC_SHA="08e568afbeb9541a0bfb46c77d2591d65b6cf79a2f8faeaae43b5777050a7d2c"
MODE="${1:-deploy}"
SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)"
RUN_ID="$(date +%Y%m%d-%H%M%S)-$$"
RUN_DIR="$BASE/deployments/$RUN_ID"
CANARY_DATA="$RUN_DIR/canary-data"
IMG="pulse-edge:gf3833-hidden-$RUN_ID"
CANARY="pulse-edge-gf3833-canary-$RUN_ID"
LIVE="pulse-edge-gf3833-live-$RUN_ID"
OLD=""
OLD_RUNNING=false
OLD_RESTART="no"
CUTOVER=0
...
# Dockerfile contract is validated semantically instead of by brittle whole-file SHA.
# This still blocks wrong/old Dockerfiles while tolerating harmless upload/line-ending changes.
grep -Fq 'io.pulse-edge.runtime="3.8.33_FAST_MINI_FULL_DWELL"' "$SCRIPT_DIR/Dockerfile" || die "Dockerfile release contract mismatch"
grep -Fq 'COPY PULSE_EDGE_GREENFIELD_3_8_33_FAST_MINI_FULL_DWELL_SOURCE_FINAL.tar.gz /tmp/source.tar.gz' "$SCRIPT_DIR/Dockerfile" || die "Dockerfile SOURCE contract mismatch"
grep -Fq 'python -m deploy.offline_tests' "$SCRIPT_DIR/Dockerfile" || die "Dockerfile offline-test contract missing"
grep -Fq 'CMD ["python","-m","uvicorn","pulse_edge.main:app"' "$SCRIPT_DIR/Dockerfile" || die "Dockerfile runtime command contract mismatch"
log "DOCKERFILE_CONTRACT_PASS sha=$(sha256sum "$SCRIPT_DIR/Dockerfile" | awk '{print $1}')"
...
fixed={
'PULSE_RELEASE':'3.8.33_FAST_MINI_FULL_DWELL','PULSE_FEED_ENABLED':'true','PULSE_DATA_DIR':'/app/data/pulse_edge',
...
'PULSE_PUSH_ENABLED':'true','PULSE_WS_DYNAMIC_MAX_ITEMS':'34','PULSE_WS_BACKGROUND_RESERVE_ITEMS':'14','PULSE_WS_QUOTE_FOCUS_ITEMS':'4','PULSE_PRIORITY_IDLE_SEC':'480','PULSE_DISCOVERY_NEW_SEC':'180','PULSE_DISCOVERY_FRESH_SLOTS':'8','PULSE_LATENCY_SAMPLE_SIZE':'300','PULSE_EVENT_LOOP_PROBE_SEC':'0.5','PULSE_RECONNECT_SIGNAL_QUIET_SEC':'15','PULSE_PERFORMANCE_UPDATE_SEC':'30','PULSE_PROBE_REST_CONFIRM_MIN_SCORE':'60','PULSE_PROBE_REST_CONFIRM_COOLDOWN_SEC':'20','PULSE_URGENT_BASELINE_MIN_SCORE':'60','PULSE_URGENT_BASELINE_COOLDOWN_SEC':'20','PULSE_QUOTE_NEWCOMER_SLOTS':'2','PULSE_QUOTE_ROTATION_SEC':'15','PULSE_QUOTE_MIN_FOCUS_SEC':'30','PULSE_QUOTE_FOCUS_SWITCH_MARGIN':'8','PULSE_ZERO_TRADE_MINUTE_COVERAGE_SAMPLES':'45','PULSE_BID_ABSORPTION_MIN_SCORE':'60','PULSE_BID_ABSORPTION_WINDOW_SEC':'90','PULSE_BID_ABSORPTION_ABS_FLOOR_KRW':'5000000','PULSE_BID_ABSORPTION_SELL_RATIO_MIN':'0.20','PULSE_EARLY_ABSORPTION_MIN_OBSERVED_SEC':'60','PULSE_EARLY_HIDDEN_GEM_MIN_SCORE':'60','PULSE_EARLY_HIDDEN_GEM_CHANGE_MIN':'-1.5','PULSE_EARLY_HIDDEN_GEM_CHANGE_MAX':'2.5','PULSE_MODEL_A2_ABSORB_RATIO_MIN':'0.75',
'PULSE_RANK_COMMIT_SEC':'30','PULSE_BACKGROUND_ROTATE_SEC':'3','PULSE_PROBE_TRACE_MIN_SCORE':'35','PULSE_PROBE_VELOCITY_MIN_SCORE':'8','PULSE_PROBE_ACCELERATION_MIN_SCORE':'3','PULSE_PROBE_MINI_SCORE':'22','PULSE_PROBE_MINI_VELOCITY_MIN_SCORE':'3','PULSE_PROBE_MINI_DWELL_SEC':'15','PULSE_PROBE_MID_DWELL_SEC':'45','PULSE_PROBE_HIGH_DWELL_SEC':'90','PULSE_PROBE_RETAINED_BACKGROUND_MAX':'4','PULSE_FAST_SCAN_MIN_FRESH_SLOTS':'10','PULSE_SCAN_INTERVAL_SEC':'20','PULSE_INVESTOR_REFRESH_SEC':'20','PULSE_DAILY_CONTEXT_REFRESH_SEC':'120','PULSE_DAILY_CONTEXT_SYMBOLS':'20','PULSE_COILED_REFRESH_SEC':'60','PULSE_COILED_SEED_LIMIT':'30','PULSE_COILED_TOP_LIMIT':'5','PULSE_BASELINE_BOOTSTRAP_INTERVAL_SEC':'60','PULSE_BASELINE_BOOTSTRAP_SYMBOLS_PER_CYCLE':'2','PULSE_SUPPLY_TIGHT_MIN_CONDITIONS':'8','PULSE_SUPPLY_EXHAUSTION_MIN_SIGNALS':'7','PULSE_IGNITION_TRIGGER_MIN_COUNT':'2'}
...
import json,sys
q=json.load(open(sys.argv[1])); assert q['release']=='3.8.33_FAST_MINI_FULL_DWELL' and q['ready'] is True and q['feed_enabled'] is False
PY
  fi
...
import json,sys
q=json.load(open(sys.argv[1])); assert q.get('release')=='3.8.33_FAST_MINI_FULL_DWELL' and isinstance(q.get('rows'),list),q
...
import json,sys
h=json.load(open(sys.argv[1])); assert h.get('runtime_safety_release')=='3.8.33_FAST_MINI_FULL_DWELL'
PY
...
import json,sys
h=json.load(open(sys.argv[1])); assert h.get('runtime_safety_release')=='3.8.33_FAST_MINI_FULL_DWELL',h
