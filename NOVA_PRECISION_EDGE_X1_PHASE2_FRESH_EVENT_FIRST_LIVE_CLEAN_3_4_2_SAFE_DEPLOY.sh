#!/usr/bin/env bash
# NOVA PRECISION EDGE X1 PHASE2 · FRESH EVENT FIRST LIVE CLEAN · 3.4.2 · CURRENT-DAY ACCELERATION ONLY · CLOSED LIVE ROWS ZERO
# Safe build/canary/cutover with automatic rollback. No runtime source patching.
set -Eeuo pipefail

PRODUCT='NOVA PRECISION EDGE X1 PHASE2 · FRESH EVENT FIRST LIVE CLEAN · 3.4.2'
VERSION='NOVA-PRECISION-EDGE-X1-PHASE2-FRESH-EVENT-FIRST-LIVE-CLEAN-3.4.2'
CURRENT_CONTAINER="${NOVA_CURRENT_CONTAINER:-quant-nova}"
DOCKERFILE_NAME='NOVA_PRECISION_EDGE_X1_PHASE2_FRESH_EVENT_FIRST_LIVE_CLEAN_3_4_2.Dockerfile'
EXPECTED_DOCKERFILE_SHA256='0e16c5a9f47897468400cfe4b4a53bc8ff54edea1f3d4ac0cc5d7d8e13b729ad'
...
IMAGE_TAG="quant-nova:fresh-event-live-clean-3-4-2-${STAMP}"
BACKUP_CONTAINER="quant-nova-backup-before-fresh-event-live-clean-3-4-2-${STAMP}"
CANARY_CONTAINER="quant-nova-canary-fresh-event-live-clean-3-4-2-${STAMP}"
BUILD_CONTEXT=''
CUTOVER_STARTED=0
...
BUILD_CONTEXT="$(mktemp -d -t nova-fresh-event-live-clean-342-build.XXXXXX)"
...
[[ "$(label io.quantnova.discovery_model)" == 'FRESH_EVENT_FIRST' ]] || fail 'fresh acceleration model missing'
[[ "$(label io.quantnova.live_candidate_boot)" == 'EMPTY_EACH_PROCESS_AND_TRADING_DAY' ]] || fail 'live candidate boot mismatch'
[[ "$(label io.quantnova.live_candidate_source)" == 'CURRENT_DAY_ACCELERATION_EVENTS_ONLY' ]] || fail 'live source mismatch'
[[ "$(label io.quantnova.acceleration_seed_sources)" == 'BID_SURGE_RESIDUAL_RATIO_SURGE_VOLUME_SURGE' ]] || fail 'acceleration seed mismatch'
[[ "$(label io.quantnova.single_static_appearance_live)" == 'ZERO' ]] || fail 'single static appearance must not enter LIVE'
[[ "$(label io.quantnova.acceleration_confirmation)" == 'TWO_SOURCES_45S_OR_POSITIVE_RANK_RATE_ACCEL' ]] || fail 'acceleration confirmation mismatch'
[[ "$(label io.quantnova.acceleration_live_ttl)" == '120S' ]] || fail 'acceleration live TTL mismatch'
[[ "$(label io.quantnova.absolute_leader_sources)" == 'OBSERVATION_ONLY_ZERO_TTL_ZERO_SCORE_ZERO_MEMBERSHIP' ]] || fail 'absolute leader sources must be context only'
[[ "$(label io.quantnova.nxt_seed_sources)" == 'VOLUME_SURGE_BID_SURGE_RESIDUAL_RATIO_SURGE_ONLY' ]] || fail 'NXT seed source mismatch'
# Rank-priority semantics are enforced by fresh_event_341 acceptance during image build.
# The image contract checks the actual label that exists in the Dockerfile.
[[ "$(label io.quantnova.rank_delivery)" == 'CURRENT_CONFIRMED_EVENT_FIXED10_30S_COMMIT' ]] || fail 'rank delivery mismatch'
# Fresh-event expiry is represented by the existing acceleration_live_ttl label.
[[ "$(label io.quantnova.acceleration_live_ttl)" == '120S' ]] || fail 'fresh-event ttl mismatch'
[[ "$(label io.quantnova.incumbent_score_cushion)" == 'ZERO' ]] || fail 'incumbent cushion not zero'
[[ "$(label io.quantnova.prior_close_to_candidate)" == 'ZERO' ]] || fail 'prior close candidate materialization must be zero'
[[ "$(label io.quantnova.history_to_candidate)" == 'ZERO' ]] || fail 'history candidate materialization must be zero'
[[ "$(label io.quantnova.performance_to_candidate)" == 'ZERO' ]] || fail 'performance candidate materialization must be zero'
[[ "$(label io.quantnova.unknown_ws_to_candidate)" == 'ZERO' ]] || fail 'unknown WS candidate materialization must be zero'
[[ "$(label io.quantnova.snapshot_to_live_membership)" == 'ZERO' ]] || fail 'snapshot live membership must be zero'
[[ "$(label io.quantnova.closed_live_tables)" == 'EMPTY_HISTORY_SEPARATE' ]] || fail 'closed live tables must be empty'
[[ "$(label io.quantnova.legacy_frozen_reconstruction)" == 'REMOVED' ]] || fail 'legacy frozen reconstruction not removed'
[[ "$(label io.quantnova.old_board_handoff)" == 'HISTORY_META_ONLY_ZERO_ROWS' ]] || fail 'old board handoff must be history-only'
[[ "$(label io.quantnova.chart_edge_closed_rows)" == 'ZERO' ]] || fail 'closed Chart Edge rows must be zero'
[[ "$(label io.quantnova.buy_signal_membership_privilege)" == 'ZERO' ]] || fail 'BUY membership privilege must be zero'
[[ "$(label io.quantnova.sparse_board_policy)" == 'SIGNAL_WAIT_NOT_OLD_SYMBOL' ]] || fail 'sparse board policy mismatch'
[[ "$(label io.quantnova.rank_delivery)" == 'CURRENT_CONFIRMED_EVENT_FIXED10_30S_COMMIT' ]] || fail 'rank delivery mismatch'
[[ "$(label io.quantnova.data_source)" == 'EXISTING_KIWOOM_API_ONLY' ]] || fail 'broker data-source contract mismatch'
...
assert edge.get('version')=='NOVA-PRECISION-EDGE-X1-PHASE2-FRESH-EVENT-FIRST-LIVE-CLEAN-3.4.2', edge.get('version')
...
# OFFLINE canary is a closed session: every actionable LIVE board must be empty.
assert all(not (edge.get('boards') or {}).get(k) for k in ('preignition','prebuy','nxt','pullback')), edge.get('boards')
ce=edge.get('chart_edge') or {}
assert not ce.get('primary') and not ce.get('next'), ce
ne=ce.get('nxtEdge') or {}
assert not ne.get('rows') and not ((ne.get('closeBet') or {}).get('rows')), ne
cp=edge.get('close_preserve') or {}
assert cp.get('mode')=='SESSION_CLOSED_HISTORY_ONLY' and cp.get('legacy_frozen_reconstruction') is False, cp
...
# 3.4.2: never write LIVE board membership into cutover handoff.
print('OFFICIAL_BOARD_HANDOFF=DISABLED_HISTORY_ONLY',end=' ')
try:
    news=get('/api/news-early-ignition')
...
assert c.get('new_broker_rest_calls')==0 and c.get('new_ws_subscription_types')==0, c
assert c.get('automatic_order') is False, c
if str(edge.get('phase'))=='SESSION CLOSED':
    assert all(not (edge.get('boards') or {}).get(k) for k in ('preignition','prebuy','nxt','pullback')), edge.get('boards')
    ce=edge.get('chart_edge') or {}; ne=ce.get('nxtEdge') or {}
    assert not ce.get('primary') and not ce.get('next'), ce
    assert not ne.get('rows') and not ((ne.get('closeBet') or {}).get('rows')), ne
    cp=edge.get('close_preserve') or {}
    assert cp.get('mode')=='SESSION_CLOSED_HISTORY_ONLY' and cp.get('official_rows')==0, cp
nc=news.get('contracts') or {}
assert news.get('ok') and nc.get('table_rows')==10, news
...
say "DISCOVERY_MODEL=FRESH_EVENT_FIRST"
say "LIVE_CANDIDATE_BOOT=EMPTY_EACH_PROCESS_AND_TRADING_DAY"
say "LIVE_CANDIDATE_SOURCE=CURRENT_DAY_ACCELERATION_EVENTS_ONLY"
say "ACCELERATION_SEED_SOURCES=BID_SURGE_RESIDUAL_RATIO_SURGE_VOLUME_SURGE"
say "ABSOLUTE_LEADER_SOURCES=OBSERVATION_ONLY_ZERO_TTL_ZERO_SCORE_ZERO_MEMBERSHIP"
say "NXT_SEED_SOURCES=VOLUME_SURGE_BID_SURGE_RESIDUAL_RATIO_SURGE_ONLY"
say "RANK_PRIORITY=CONFIRMED_EVENT_THEN_ACCEL_RECENCY_THEN_RANK_VELOCITY"
say "INCUMBENT_SCORE_CUSHION=ZERO"
say "STAGNATION_POLICY=480S_NO_ACCEL_DROP"
say "PRIOR_CLOSE_CANDIDATE_MATERIALIZATION=ZERO"
say "HISTORY_TO_CANDIDATE=ZERO"
say "PERFORMANCE_TO_CANDIDATE=ZERO"
say "UNKNOWN_WS_TO_CANDIDATE=ZERO"
say "SNAPSHOT_TO_LIVE_MEMBERSHIP=ZERO"
say "CLOSED_LIVE_TABLES=EMPTY_HISTORY_SEPARATE"
say "LEGACY_FROZEN_RECONSTRUCTION=REMOVED"
say "OLD_BOARD_HANDOFF=HISTORY_META_ONLY_ZERO_ROWS"
say "CHART_EDGE_CLOSED_ROWS=ZERO"
say "BUY_SIGNAL_MEMBERSHIP_PRIVILEGE=ZERO"
say "SPARSE_BOARD=SIGNAL_WAIT_NOT_OLD_SYMBOL"
