...
ZIP_NAME="kiwoom-unified-edge-v3.2.22-NEWS156-KRX-LOCK-NXT-ISOLATED.zip"
SOURCE_DIR_NAME="kiwoom-unified-edge-v3.2.22"
EXPECTED_ZIP_SHA256="f66c7a464ad8b8aa139372b75c0be26b52c68ea7b6b7f850caccff54b5215ceb"
OLD_APP_DIR="$HOME/kiwoom-smartmoney-daytrader"
NEW_APP_DIR="$HOME/kiwoom-unified-edge"
TRADER_IMAGE="kiwoom-unified:v3.2.22"
...
rollback(){
  local ec=$?
  [[ $ec -eq 0 ]] && return
  printf '\n[자동복구] v3.2.22 KRX LOCK + NXT ISOLATED 적용 실패 — 기존 컨테이너로 복구합니다.\n' >&2
...
say "v3.2.22 KRX LOCK + NXT ISOLATED + News 1.5.6 파일 받기"
...
[[ "$TRADER_VERSION" == "3.2.22" ]] || fail "trader 버전 불일치: $TRADER_VERSION"
[[ "$NEWS_VERSION" == "1.5.6" ]] || fail "news 버전 불일치: $NEWS_VERSION"
grep -Fq "KRX_CORE_BARE_0B" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "검증된 KRX bare 0B 고정 루트 누락"
grep -Fq "startNxtProbeSocket" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "NXT 격리 WebSocket probe 누락"
grep -Fq "onNxtProbeMessage" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "NXT 격리 수신 처리 누락"
grep -Fq "stex_tp:'NXT'" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "NXT 이름형 probe 누락"
grep -Fq "stex_tp:'2'" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "NXT 숫자형 fallback probe 누락"
grep -Fq "venue!=='NXT'" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "FID9081 NXT 진실 게이트 누락"
! grep -Fq "values['9081']||values['337']" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "FID337 거래소 오판정 구버전 잔존"
grep -Fq "KRX_BARE_LOCK_NXT_ISOLATED_PROBE_FID9081_V3" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "KRX/NXT 격리 모드 누락"
...
[[ "$RUNNING_VERSION" == "3.2.22" ]] || fail "실행 버전 불일치: $RUNNING_VERSION"

say "실제 firstTick/fresh 검증 — KRX primary와 NXT isolated를 분리 확인 (최대 90초)"
RUNTIME_OK=0
...
probe=h.get('nxtProbe') or {}
rows=[]; ok=True
for v in exp:
    x=r.get(v) or {}; first=int(x.get('firstTick') or 0); fresh=int(x.get('fresh') or 0); conf=bool(x.get('venueConfirmed'))
    if v=='KRX':
        ack=int(x.get('regAccepted') or 0); vok=ack>0 and first>0 and fresh>0 and conf
        rows.append(f'KRX:ack={ack},first={first},fresh={fresh},confirmed={conf}')
    else:
        reg=probe.get('lastReg') or {}; ack=bool(reg.get('ok')); vok=ack and first>0 and fresh>0 and conf
        rows.append(f'NXT:probeAck={ack},first={first},fresh={fresh},confirmed={conf},mode={probe.get("mode")}')
    ok = ok and vok
...
if [[ "$ACTIVE_NOW" == "1" && $RUNTIME_OK -ne 1 ]]; then
  ...
  fail "활성 거래소의 실제 firstTick/fresh 검증 실패. 기존 컨테이너로 자동 롤백합니다."
fi
