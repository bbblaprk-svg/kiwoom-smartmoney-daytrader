#!/usr/bin/env bash
# Revision: PHASE2-CHART-FIX9-CLOSE5-3_0_5
set -Eeuo pipefail

PRODUCT='NOVA PRECISION EDGE X1 PHASE2 · CHART FIX9 CLOSE5'
VERSION='NOVA-PRECISION-EDGE-X1-PHASE2-CHART-FIX9-CLOSE5-3.0.5'
CURRENT_CONTAINER="${NOVA_CURRENT_CONTAINER:-quant-nova}"
DOCKERFILE_NAME='QUANT_NOVA_PRECISION_EDGE_X1_PHASE2_CHART_FIX9_CLOSE5.Dockerfile'
EXPECTED_DOCKERFILE_SHA256='889eaef2d6a26cc60bae83471fe7daaebf230b57215041a1d739adb7aa1c5cb3'
...
IMAGE_TAG="quant-nova:precision-edge-x1-phase2-chart-fix9-close5-${STAMP}"
BACKUP_CONTAINER="quant-nova-backup-precision-edge-x1-phase2-chart-fix9-close5-${STAMP}"
CANARY_CONTAINER="quant-nova-canary-precision-edge-x1-phase2-chart-fix9-close5-${STAMP}"
BUILD_CONTEXT=''
CUTOVER_STARTED=0
...
BUILD_CONTEXT="$(mktemp -d -t nova-precision-x1-phase2-chart-fix9-close5-build.XXXXXX)"
...
assert edge.get("version")=="NOVA-PRECISION-EDGE-X1-PHASE2-CHART-FIX9-CLOSE5-3.0.5",edge
...
print("PHASE2_CHART_FIX9_CLOSE5_DUAL_CLOSE_STORE_RESTART_CANARY=PASS")
'
...
assert j.get("ok") and j.get("version")=="NOVA-PRECISION-EDGE-X1-PHASE2-CHART-FIX9-CLOSE5-3.0.5",j
...
assert lc.get("maxSignalCycles")==1000 and lc.get("maxStageEvents")==5000,lc
assert lc.get("outcomeWriteFlushSec")==12 and not lc.get("repeatedPacketsCreateEvents"),lc
print("PRECISION_CHART_FIX9_CLOSE5_API_CONTRACT=PASS PHASE="+str(j.get("phase")))
'
...
done < <(docker images 'quant-nova' --format '{{.Repository}}:{{.Tag}}' | awk '/^quant-nova:precision-edge-x1-phase2-chart-fix9-close5-[0-9]{8}-[0-9]{6}$/')
...
say "MANUAL_ROLLBACK=./NOVA_PRECISION_EDGE_X1_PHASE2_CHART_FIX9_CLOSE5_MANUAL_ROLLBACK.sh ${BACKUP_CONTAINER}"
