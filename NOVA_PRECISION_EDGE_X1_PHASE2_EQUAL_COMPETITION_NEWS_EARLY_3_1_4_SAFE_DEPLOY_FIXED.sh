#!/usr/bin/env bash
# NOVA PRECISION EDGE X1 PHASE2 · EQUAL COMPETITION + NEWS EARLY · 3.1.4
# Safe build/canary/cutover with automatic rollback. No runtime source patching.
set -Eeuo pipefail

PRODUCT='NOVA PRECISION EDGE X1 PHASE2 · EQUAL COMPETITION + NEWS EARLY · 3.1.4'
VERSION='NOVA-PRECISION-EDGE-X1-PHASE2-EQUAL-COMPETITION-NEWS-EARLY-3.1.4'
CURRENT_CONTAINER="${NOVA_CURRENT_CONTAINER:-quant-nova}"
DOCKERFILE_NAME='NOVA_PRECISION_EDGE_X1_PHASE2_EQUAL_COMPETITION_NEWS_EARLY_3_1_4_FIXED.Dockerfile'
EXPECTED_DOCKERFILE_SHA256='aaa2c61ffa2ab4cf978bbe1ea7e4466183e49d3dcff77a6bca9e53ea21663809'
...
IMAGE_TAG="quant-nova:equal-news-3-1-4-${STAMP}"
BACKUP_CONTAINER="quant-nova-backup-before-equal-news-3-1-4-${STAMP}"
CANARY_CONTAINER="quant-nova-canary-equal-news-3-1-4-${STAMP}"
BUILD_CONTEXT=''
CUTOVER_STARTED=0
...
BUILD_CONTEXT="$(mktemp -d -t nova-equal-news-314-build.XXXXXX)"
...
[[ "$(label io.quantnova.nxt_preview)" == 'PRE1540_DISPLAY_ONLY_OFFICIAL_ENTRY_HARD_GATED' ]] || fail 'NXT preview contract mismatch'
[[ "$(label io.quantnova.nxt_baseline_window)" == '1540_1830_KST_NO_ENTRY' ]] || fail 'NXT baseline window mismatch'
[[ "$(label io.quantnova.nxt_close_window)" == '1830_1955_KST_LOCK_AT_1955' ]] || fail 'NXT close window mismatch'
[[ "$(label io.quantnova.signal_occurrence)" == 'CANONICAL_10M_REFRACTORY_IGNITION_EPISODES_NOT_STAGE_EVENTS' ]] || fail 'signal occurrence contract mismatch'
[[ "$(label io.quantnova.signal_visibility)" == 'MERGE_LIVE_WITH_SAME_DAY_LAST_VALID_FIXED10_DISPLAY_ONLY' ]] || fail 'signal visibility contract mismatch'
[[ "$(label io.quantnova.intraday_snapshot)" == 'SAME_DAY_PRE_PREBUY_PULLBACK_MERGE_LIVE_FIRST_DISPLAY_ONLY' ]] || fail 'intraday snapshot contract mismatch'
...
assert edge.get('version')=='NOVA-PRECISION-EDGE-X1-PHASE2-EQUAL-COMPETITION-NEWS-EARLY-3.1.4', edge.get('version')
...
say "NXT_TIME_GATE=PRE1540_DISPLAY_ONLY__1540_1830_NO_ENTRY__1830_1955_CLOSE"
say "SIGNAL_OCCURRENCE=10M_REFRACTORY_IGNITION_EPISODES"
say "SIGNAL_DISPLAY_CONTINUITY=MERGE_LIVE_WITH_SAME_DAY_LAST_VALID_FIXED10_DISPLAY_ONLY"
say "CADDY_NETWORK_INHERIT=ENABLED"
say "PREBUY_ROUTING=SOURCE_BOARD_PRESERVED"
say "INTRADAY_FIXED10_CONTINUITY=LIVE_FIRST_PLUS_SAME_DAY_SNAPSHOT"
