...
ZIP_NAME="kiwoom-smartmoney-daytrader-v3.1.11-FINAL.zip"
SOURCE_DIR_NAME="kiwoom-smartmoney-daytrader-v3.1.11"
EXPECTED_ZIP_SHA256="537fc99f5cf11a401045ec75b365600220dd9514fec90b80cd257809d8870dc0"
IMAGE_NAME="kiwoom-smartmoney:v3.1.11"
...
rollback() {
  ...
  printf '\n[자동 복구] v3.1.11 FINAL 적용 실패 — 기존 앱으로 되돌립니다.\n' >&2
...
# 키움 REST/WebSocket 주소·TR·FID 관련 환경값은 기존 v3.1.10 값을 그대로 보존한다.
# 이번 판은 판정/배점과 감시 상한만 변경한다.
...
COPY kiwoom-smartmoney-daytrader-v3.1.11-FINAL.zip /tmp/app.zip
RUN mkdir -p /tmp/source \
    && unzip -q /tmp/app.zip -d /tmp/source \
    && cp -a /tmp/source/kiwoom-smartmoney-daytrader-v3.1.11/. /app/ \
...
say "v3.1.11 FINAL Docker 이미지 빌드 — 전체검사·20종목 부하검사·Next 빌드 포함"
...
[[ "$RUNNING_VERSION" == "3.1.11" ]] || fail "실행 컨테이너 버전이 3.1.11이 아닙니다: $RUNNING_VERSION"
...
printf '\n============================================================\n'
printf 'v3.1.11 FINAL 업데이트 완료\n'
printf '앱 주소   : https://%s\n' "$DOMAIN"
printf '상태 확인 : https://%s/api/health\n' "$DOMAIN"
printf '감시 구조 : 고정4 + 사용자집중8 + 경량8 = 20종목\n'
printf '============================================================\n'
printf '현재 건강상태: '
cat "$TMP_ROOT/health.json"
printf '\n\n최근 앱 로그:\n'
sudo docker logs --tail 60 "$APP_CONTAINER" || true
printf '\nSafari 탭을 완전히 닫았다가 다시 열고 v3.1.11, 급등 사전포착, 기존점수 비교, 20종목 한도를 확인하세요.\n'
