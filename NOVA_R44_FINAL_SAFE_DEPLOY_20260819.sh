...
EXPECTED_DOCKERFILE_SHA="f52e27c7e5f0bda4d943d1cdafe951b4fd9ecb2774eeb8518d11a6cae388de01"
EXPECTED_VERSION="NOVA-3.3.4-MARKET-WIDE-ROTATION-VERIFY"
IMAGE="quant-nova:3.3.4-r44-final-local"
STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP="${APP}-pre-r44-${STAMP}"
FAILED="${APP}-failed-r44-${STAMP}"
CANDIDATE="${APP}-candidate-r44-${STAMP}"
WORK="$(mktemp -d /tmp/nova-r44-final.XXXXXX)"
DOCKERFILE="$WORK/Dockerfile"
ENVFILE="$WORK/current.env"
CAND_DATA="$WORK/candidate-data"
LOG="/tmp/nova-r44-final-deploy-${STAMP}.log"
OLD_WAS_RUNNING=0
OLD_STOPPED=0
OLD_RENAMED=0
NEW_STARTED=0
...
fetch_dockerfile(){
  local remote="QUANT_NOVA_3.3.4_R44_OPENING_HIDE_LARGECAP_LIVE_ONLY_FINAL_20260819.Dockerfile"
...
say "1/8 FETCH R44 FINAL DOCKERFILE + SHA LOCK"
fetch_dockerfile
...
[ "$(dc image inspect "$IMAGE" --format '{{index .Config.Labels "io.quantnova.ui_patch"}}')" = "R44_OPENING_HIDE_1000_LARGECAP_LIVE_ONLY" ]
...
OLD_RENAMED=1
RUN_ARGS=(run -d --name "$APP" --restart "$RESTART" --network "${NETWORKS[0]}" --env-file "$ENVFILE")
