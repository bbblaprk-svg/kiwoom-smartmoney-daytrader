#!/usr/bin/env bash
# Revision: PHASE2-CHART-FIX9-CLOSE3-3_0_3
set -Eeuo pipefail

PRODUCT='NOVA PRECISION EDGE X1 PHASE2 · CHART FIX9 CLOSE3'
VERSION='NOVA-PRECISION-EDGE-X1-PHASE2-CHART-FIX9-CLOSE3-3.0.3'
CURRENT_CONTAINER="${NOVA_CURRENT_CONTAINER:-quant-nova}"
DOCKERFILE_NAME='QUANT_NOVA_PRECISION_EDGE_X1_PHASE2_CHART_FIX9_CLOSE3.Dockerfile'
EXPECTED_DOCKERFILE_SHA256='3ff3e5eb2cdfa39f0c9e0bd288e0a4783ab06fbcc602475d9db19781ac558158'
...
IMAGE_TAG="quant-nova:precision-edge-x1-phase2-chart-fix9-close3-${STAMP}"
BACKUP_CONTAINER="quant-nova-backup-precision-edge-x1-phase2-chart-fix9-close3-${STAMP}"
CANARY_CONTAINER="quant-nova-canary-precision-edge-x1-phase2-chart-fix9-close3-${STAMP}"
BUILD_CONTEXT=''
CUTOVER_STARTED=0
...
BUILD_CONTEXT="$(mktemp -d -t nova-precision-x1-phase2-chart-fix9-close3-build.XXXXXX)"
...
assert edge.get("version")=="NOVA-PRECISION-EDGE-X1-PHASE2-CHART-FIX9-CLOSE3-3.0.3",edge
...
print("PHASE2_CHART_FIX9_CLOSE3_OFFLINE_CANARY=PASS")
'
...
assert j.get("ok") and j.get("version")=="NOVA-PRECISION-EDGE-X1-PHASE2-CHART-FIX9-CLOSE3-3.0.3",j
...
assert lc.get("maxSignalCycles")==1000 and lc.get("maxStageEvents")==5000,lc
assert lc.get("outcomeWriteFlushSec")==12 and not lc.get("repeatedPacketsCreateEvents"),lc
print("PRECISION_CHART_FIX9_CLOSE3_API_CONTRACT=PASS PHASE="+str(j.get("phase")))
'
...
done < <(docker images 'quant-nova' --format '{{.Repository}}:{{.Tag}}' | awk '/^quant-nova:precision-edge-x1-phase2-chart-fix9-close3-[0-9]{8}-[0-9]{6}$/')
...
say "MANUAL_ROLLBACK=./NOVA_PRECISION_EDGE_X1_PHASE2_CHART_FIX9_CLOSE3_MANUAL_ROLLBACK.sh ${BACKUP_CONTAINER}"
