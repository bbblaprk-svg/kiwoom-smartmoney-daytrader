...
ZIP_NAME="kiwoom-unified-edge-v3.3.0-NEWS156-REALTIME-FOUNDATION.zip"
SOURCE_DIR_NAME="kiwoom-unified-edge-v3.3.0"
EXPECTED_ZIP_SHA256="1e50fd681ce1850bc0e1e89eb98e43386efcee49bbb416a0e82b773fa806f941"
OLD_APP_DIR="$HOME/kiwoom-smartmoney-daytrader"
NEW_APP_DIR="$HOME/kiwoom-unified-edge"
TRADER_IMAGE="kiwoom-unified:v3.3.0"
...
rollback(){
  local ec=$?
  [[ $ec -eq 0 ]] && return
  printf '\n[자동복구] v3.3.0 REALTIME FOUNDATION 적용 실패 — 기존 컨테이너로 복구합니다.\n' >&2
...
say "v3.3.0 REALTIME FOUNDATION + News 1.5.6 파일 받기"
...
[[ "$TRADER_VERSION" == "3.3.0" ]] || fail "trader 버전 불일치: $TRADER_VERSION"
[[ "$NEWS_VERSION" == "1.5.6" ]] || fail "news 버전 불일치: $NEWS_VERSION"
grep -Fq "DOMESTIC_TRADE_BARE_0B" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "공식 bare 6자리 0B 핵심 REG 누락"
! grep -Fq "startNxtProbeSocket" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "구형 NXT 별도 probe socket 잔존"
! grep -Fq "stex_tp" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "국내 WebSocket stex_tp 변칙 등록 잔존"
...
[[ "$RUNNING_VERSION" == "3.3.0" ]] || fail "실행 버전 불일치: $RUNNING_VERSION"

say "실제 수신 검증 — REG ACK가 아니라 FID9081-confirmed firstTick/fresh 확인 (최대 90초)"
RUNTIME_OK=0
