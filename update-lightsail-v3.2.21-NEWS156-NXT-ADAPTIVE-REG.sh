...
ZIP_NAME="kiwoom-unified-edge-v3.2.21-NEWS156-NXT-ADAPTIVE-REG.zip"
SOURCE_DIR_NAME="kiwoom-unified-edge-v3.2.21"
EXPECTED_ZIP_SHA256="845c7812e0fdc754a1133a246f004cea34231424a9c24b2851aa919ef10f345e"
OLD_APP_DIR="$HOME/kiwoom-smartmoney-daytrader"
NEW_APP_DIR="$HOME/kiwoom-unified-edge"
TRADER_IMAGE="kiwoom-unified:v3.2.21"
...
rollback(){
  local ec=$?
  [[ $ec -eq 0 ]] && return
  printf '\n[자동복구] v3.2.21 NXT ADAPTIVE REG 적용 실패 — 기존 컨테이너로 복구합니다.\n' >&2
...
say "v3.2.21 NXT ADAPTIVE REG + News 1.5.6 파일 받기"
...
[[ "$TRADER_VERSION" == "3.2.21" ]] || fail "trader 버전 불일치: $TRADER_VERSION"
[[ "$NEWS_VERSION" == "1.5.6" ]] || fail "news 버전 불일치: $NEWS_VERSION"
grep -Fq "NXT_CORE_0B" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "NXT 전용 0B CORE 등록 누락"
grep -Fq "KRX_CORE_0B" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "KRX 전용 0B CORE 등록 누락"
grep -Fq "stex_tp: stex" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "WebSocket 거래소 map 등록 누락"
grep -Fq "venue === 'NXT' ? 'NXT'" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "NXT 명시형 stex_tp 기본 등록 누락"
grep -Fq "NXT_CORE_0B_FALLBACK_NUMERIC" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "NXT 숫자형 호환 probe 누락"
! grep -Fq "values['9081']||values['337']" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "FID337을 거래소 진실값으로 사용하는 구버전 로직 잔존"
grep -Fq "VENUE_NAME_PRIMARY_NXT_NUMERIC_FALLBACK_FID9081_V2" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "거래소 분리 등록 모드 누락"
grep -Fq "FID9081_VENUE_TRUTH" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "FID9081 거래소 진실 게이트 누락"
grep -Fq "NXT_ADAPTIVE_REG_CHECK PASS" "$ROOT/trader/scripts/nxt-dedicated-reg-check.js" || fail "NXT 전용 등록 검사 누락"
...
[[ "$RUNNING_VERSION" == "3.2.21" ]] || fail "실행 버전 불일치: $RUNNING_VERSION"

say "거래소별 REG ACK + FID9081 firstTick/fresh 검증 (최대 90초)"
RUNTIME_OK=0
for _ in $(seq 1 30); do
  OUT="$(curl -fsS --max-time 3 http://127.0.0.1:3000/api/health 2>/dev/null || true)"
  if [[ -n "$OUT" ]]; then
    printf '%s' "$OUT" > "$TMP_ROOT/health.json"
    CHECK="$(python3 -c 'import json,sys; h=json.load(open(sys.argv[1])); r=h.get("registrationStats") or {}; exp=h.get("expectedVenues") or []; rows=[]; ok=True;
for v in exp:
 x=r.get(v) or {}; a=int(x.get("regAccepted") or 0); f=int(x.get("firstTick") or 0); q=int(x.get("fresh") or 0); c=bool(x.get("venueConfirmed")); rows.append(f"{v}:ack={a},first={f},fresh={q},confirmed={c}"); ok=ok and a>0 and f>0 and q>0 and c
print(("OK" if ok else "WAIT"), "expected="+",".join(exp), *rows, "status="+str(h.get("dataStatus")))' "$TMP_ROOT/health.json")"
    echo "$CHECK"
    [[ "$CHECK" == OK* ]] && { RUNTIME_OK=1; break; }
  fi
  sleep 3
done
ACTIVE_NOW="$(python3 -c 'from datetime import datetime,timezone,timedelta; k=datetime.now(timezone.utc)+timedelta(hours=9); sec=k.hour*3600+k.minute*60+k.second; wd=k.weekday()<5; krx=9*3600<=sec<=15*3600+30*60; nxt=(8*3600<=sec<=8*3600+50*60) or (9*3600+30<=sec<=15*3600+20*60) or (15*3600+40*60<=sec<=20*3600); print("1" if wd and (krx or nxt) else "0")')"
if [[ "$ACTIVE_NOW" == "1" && $RUNTIME_OK -ne 1 ]]; then
  cat "$TMP_ROOT/health.json" || true
  echo "=== REALTIME DEBUG ==="
  curl -s "http://127.0.0.1:3000/api/realtime-debug?limit=10" || true
  sudo docker logs --tail 220 "$APP_CONTAINER" || true
  fail "활성 거래소의 전용 REG ACK→FID9081 firstTick→fresh 검증 실패. 자동 롤백합니다."
fi
...
curl -s http://127.0.0.1:3000/api/health | python3 -c 'import sys,json; h=json.load(sys.stdin); print("version=",h.get("version")); print("status=",h.get("status")); print("dataStatus=",h.get("dataStatus")); print("registrationStats=",h.get("registrationStats")); print("feedReady=",h.get("feedReady")); print("venueFeeds=",h.get("venueFeeds"))'
