...
ZIP_NAME="kiwoom-smartmoney-daytrader-v3.1.15-FINAL.zip"
SOURCE_DIR_NAME="kiwoom-smartmoney-daytrader-v3.1.15"
EXPECTED_ZIP_SHA256="13801343a49dc653c50c18cb1e5651f6e9fb6019cc038706643fb2e94f276758"
IMAGE_NAME="kiwoom-smartmoney:v3.1.15"
...
rollback() {
  ...
  printf '\n[자동 복구] v3.1.15 적용 실패 — 기존 앱으로 되돌립니다.\n' >&2
...
# 키움 REST/WebSocket 주소·TR·FID 및 기존 인증값은 그대로 보존한다.
# v3.1.15는 v3.1.14의 화면/알림 안정화를 보존하고, 장중 강한종목 수동갱신 + 별도 자동적용 ON/OFF를 추가한다. 키움 데이터 수신 경로는 보존한다.
...
COPY kiwoom-smartmoney-daytrader-v3.1.15-FINAL.zip /tmp/app.zip
RUN mkdir -p /tmp/source \
    && unzip -q /tmp/app.zip -d /tmp/source \
    && cp -a /tmp/source/kiwoom-smartmoney-daytrader-v3.1.15/. /app/ \
...
say "v3.1.15 Docker 이미지 빌드 — 전체검사·20종목 부하검사·Next 빌드 포함"
...
[[ "$RUNNING_VERSION" == "3.1.15" ]] || fail "실행 컨테이너 버전이 3.1.15이 아닙니다: $RUNNING_VERSION"

if grep -q '"status":"connected"' "$TMP_ROOT/health.json"; then
  say "키움 연결상태 connected 확인"
else
  printf '[주의] 앱 서버/버전은 정상 기동했지만 키움 연결은 아직 connected가 아닙니다. 점검시간/인증/네트워크를 /api/health에서 확인하세요.\n' >&2
fi
...
printf '\n============================================================\n'
printf 'v3.1.15 업데이트 완료\n'
printf '앱 주소   : https://%s\n' "$DOMAIN"
printf '상태 확인 : https://%s/api/health\n' "$DOMAIN"
printf '감시 구조 : 고정4 + 사용자집중8 + 경량8 = 최대 20종목\n'
printf '기록부    : 중요도순/최신발생순 + EARLY→BUY→RISK→EARLY_SELL→SELL 타임라인\n'
printf '장중추천  : 버튼형 강한종목 집중8/경량8 + 60초 재조회 제한 + 별도 자동적용 ON/OFF\n'
printf '일일추천  : 평일 20:15 집중8/경량8 자동추천 + 수동보호 + 별도 자동적용 ON/OFF\n'
printf '화면/알림 : 1.5초 안정표시 + EARLY/RISK 푸시 억제 + 푸시 완전해제\n'
printf '============================================================\n'
printf '현재 건강상태: '
cat "$TMP_ROOT/health.json"
printf '\n\n최근 앱 로그:\n'
sudo docker logs --tail 60 "$APP_CONTAINER" || true
printf '\nSafari/PWA를 완전히 닫았다가 다시 열고 v3.1.15 종목관리의 장중 강한종목/장중 자동적용 버튼을 확인하세요.\n'
printf 'subscribed 값은 현재 실제 등록 종목 수이며, 20종목을 등록/적용한 뒤 최대 20까지 늘어납니다.\n'
