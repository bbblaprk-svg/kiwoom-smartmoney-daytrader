...
EXPECTED_VERSION="NOVA-3.3.5-R492-MAX-PROFIT-EDGE1-FEED4"
EXPECTED_DOCKERFILE_SHA256="226c236d410d7cd224a028535e3a2c803a48e755b02e67388514938c1bac4235"
EXPECTED_EMBEDDED_SOURCE_SHA256="a9f45c9e1027b9fdb4a8b5fe5690cc8cf0ee74a7722fa087225dad8db05a403c"
EXPECTED_FEED4_LABEL="PARTIAL_RESYNC_WARN_HOT50_ENTRY8_LARGECAP10_NXT8_SCOUT10"
DEFAULT_DOCKERFILE="./QUANT_NOVA_R492_MAX_PROFIT_EDGE1_FEED4.Dockerfile"
if [[ $# -ge 1 ]]; then
  DOCKERFILE="$1"
...
IMAGE="${2:-${NOVA_NEW_IMAGE:-quant-nova:max-profit-edge1-feed4}}"
...
panels=get('/api/decision-panels'); nxt=get('/api/nxt-after-edge'); health=get('/api/realtime-health'); truth=get('/api/feed-truth')
...
assert str(contract.get('feed4_candidate_supply') or '').startswith('EXISTING_HOT_CAPACITY_ONLY:'),contract
assert contract.get('feed4_pre_supply') and 'ENTRY_V18 unchanged' in contract.get('feed4_pre_supply'),contract
truth_contract=truth.get('contracts') or {}
assert truth.get('mode')=='DISPLAY_ONLY',truth
assert truth_contract.get('partial_resync_policy')=='WARN_WHEN_READY_KEYS_EXIST_FAIL_ONLY_WHEN_ZERO_READY',truth_contract
assert truth_contract.get('venue_width_basis')=='ACTIVE_WS_TARGETS_FRESH_OVER_TRACKED',truth_contract
assert 'tracked_codes' in (truth.get('venues') or {}).get('KRX',{}),truth.get('venues')
...
print(json.dumps({'ok':True,'version':expected,'tables':6,'rows':10,'rank_commit_sec':30,
                  'nxt_close':'19:30-19:58','automatic_order':False,'feed4':True},ensure_ascii=False))
PY
}
rollback(){
  local rc=$?
...
[[ "$ACTUAL_DOCKERFILE_SHA256" == "$EXPECTED_DOCKERFILE_SHA256" ]] || {
  echo "FAIL: Dockerfile SHA256 불일치" >&2
  echo " expected=$EXPECTED_DOCKERFILE_SHA256" >&2
  echo " actual=$ACTUAL_DOCKERFILE_SHA256" >&2
  exit 2
}
...
LABEL_SOURCE="$(dc image inspect "$IMAGE" --format '{{index .Config.Labels "io.quantnova.embedded_source_sha256"}}')"
LABEL_FEED4="$(dc image inspect "$IMAGE" --format '{{index .Config.Labels "io.quantnova.feed4_supply"}}')"
[[ "$LABEL_VERSION" == "$EXPECTED_VERSION" ]]
[[ "$LABEL_SOURCE" == "$EXPECTED_EMBEDDED_SOURCE_SHA256" ]]
[[ "$LABEL_FEED4" == "$EXPECTED_FEED4_LABEL" ]]
...
if dc inspect "$APP" >/dev/null 2>&1; then
  ...
  while IFS='=' read -r KEY VALUE; do
    KEY="$(xargs <<<"$KEY")"; VALUE="$(xargs <<<"$VALUE")"
