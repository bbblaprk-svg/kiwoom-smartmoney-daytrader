# PULSE EDGE runtime 2.1.11.9 SCOUT_FUNNEL_CPU_SAFE
...
RUN python -c 'import hashlib,pathlib,tarfile; files=sorted(pathlib.Path("/tmp/pulse-source").glob("*.gz")); matched=[p for p in files if hashlib.sha256(p.read_bytes()).hexdigest()=="0d6c34169012c0db539148f9c950abccd0b9b84afa836177386dda0606e957f5"]; assert matched, "PULSE 2.1.11.9 source missing or SHA256 mismatch"; t=tarfile.open(matched[0],"r:gz"); t.extractall("/app",filter="data"); t.close(); assert pathlib.Path("/app/requirements-runtime.txt").is_file(); assert pathlib.Path("/app/pulse_edge/main.py").is_file(); assert pathlib.Path("/app/pulse_edge/web/pulse.js").is_file()'
