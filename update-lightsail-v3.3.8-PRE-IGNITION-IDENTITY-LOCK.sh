...
ZIP_NAME="kiwoom-unified-edge-v3.3.8-PRE-IGNITION-IDENTITY-LOCK.zip"
SOURCE_DIR_NAME="kiwoom-unified-edge-v3.3.8"
EXPECTED_ZIP_SHA256="b1eecb923042ce8be0b31af6bffd7dea001071c310d9b5e575649cbe76af55f0"
...
say "ZIP/manifest/identity/registration layer 검증"
...
grep -Fq "const packageVersion = require('../../package.json').version;" "$ROOT/trader/pages/api/health.js" || fail "health version이 package.json에 바인딩되지 않음"
grep -Fq "version: packageVersion" "$ROOT/trader/pages/api/health.js" || fail "health version stale literal 잔존"
grep -Fq "const packageVersion = require('../../package.json').version;" "$ROOT/trader/pages/api/realtime-debug.js" || fail "realtime-debug version이 package.json에 바인딩되지 않음"
grep -Fq "version:packageVersion" "$ROOT/trader/pages/api/realtime-debug.js" || fail "realtime-debug stale literal 잔존"
! grep -Fq "3.3.7-nxt-dual-reg-0b-fid9081-news156" "$ROOT/trader/pages/api/health.js" || fail "health 구버전 identity 잔존"
! grep -Fq "3.3.1-realtime-foundation-close-freshness-news156" "$ROOT/trader/pages/api/realtime-debug.js" || fail "realtime-debug 구버전 identity 잔존"
...
HEALTH_IDENTITY="$(python3 - "$TMP_ROOT/health.json" <<'PYID'
import json,sys
h=json.load(open(sys.argv[1]))
print(h.get('version') or '')
PYID
)"
[[ "$HEALTH_IDENTITY" == "3.3.8" ]] || fail "health 실행 identity 불일치: $HEALTH_IDENTITY"
DEBUG_IDENTITY="$(curl -fsS --max-time 3 'http://127.0.0.1:3000/api/realtime-debug?limit=1' | python3 -c 'import sys,json; print(json.load(sys.stdin).get("version") or "")')"
[[ "$DEBUG_IDENTITY" == "3.3.8" ]] || fail "realtime-debug 실행 identity 불일치: $DEBUG_IDENTITY"
