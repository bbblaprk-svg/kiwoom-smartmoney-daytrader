...
EXPECTED_DOCKERFILE_SHA="dd38899a7dffa4ef76cedc3429d95f5e02e50d598f171e9c2741d01fb7674736"
EXPECTED_SOURCE_SHA="161c7b84cad5642752da2bfa893d702fab62bfd9c9db48f9101bf4594e7fbc09"
...
BACKUP="${APP}-pre-fresh-rotation-${STAMP}"
FAILED="${APP}-failed-fresh-rotation-${STAMP}"
CANDIDATE="${APP}-fresh-rotation-candidate-${STAMP}"
WORK="$(mktemp -d /tmp/nova-fresh-rotation-deploy.XXXXXX)"
DOCKERFILE="$WORK/Dockerfile"
ENVFILE="$WORK/current.env"
CAND_DATA="$WORK/candidate-data"
LOG="/tmp/nova-fresh-rotation-deploy-${STAMP}.log"
...
rollback(){
  ...
  trap - ERR INT TERM
  say "AUTO ROLLBACK -> EXACT CURRENT FIXED20 BASELINE"
...
with tempfile.TemporaryDirectory(prefix='nova-lmpre-preflight-') as td:
    ...
    protected={
      ...
      'app/broker/discovery.py':'69afe77951d6986eb221114aa2e3cd31455b7267e63e5622db6ec654e1cc977f',
...
[ "$CURRENT_VERSION" = "$EXPECTED_VERSION" ] || { echo "FAIL: current version is not FIXED20 MIV1 lineage" | tee -a "$LOG"; false; }
...
[ "$CURRENT_INDEX_SHA" = "$BASE_INDEX_SHA" ] || { echo "FAIL: current index.html is not exact FIXED20 baseline" | tee -a "$LOG"; false; }
...
say "3/8 LOW-RAM BUILD — CURRENT FIXED20 BASELINE PRESERVED"
...
[ "$(dc image inspect "$IMAGE" --format '{{index .Config.Labels "io.quantnova.fresh_rotation"}}')" = "HARD_BUY_COOLDOWN_RECENT15__SOFT_HISTORY__FRESH_RESERVE30__LMPRE_SCOUT180" ]
...
[ "$(image_label "$APP" io.quantnova.fresh_rotation)" = "HARD_BUY_COOLDOWN_RECENT15__SOFT_HISTORY__FRESH_RESERVE30__LMPRE_SCOUT180" ]
runtime_gate "$APP"
