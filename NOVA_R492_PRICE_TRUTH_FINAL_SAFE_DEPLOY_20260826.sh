...
DOCKERFILE_NAME="Dockerfile_PRICE_TRUTH_FINAL_20260826"
DEFAULT_URL="https://raw.githubusercontent.com/bbblaprk-svg/kiwoom-smartmoney-daytrader/main/${DOCKERFILE_NAME}"
DOCKERFILE_SOURCE="${1:-${NOVA_DOCKERFILE_SOURCE:-}}"
IMAGE="${NOVA_PRICE_TRUTH_IMAGE:-quant-nova:r492-price-truth-final-20260826}"
STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP="${APP}-pre-pricetruth-final-${STAMP}"
FAILED="${APP}-failed-pricetruth-final-${STAMP}"
WORK="$(mktemp -d /tmp/nova-r492-pricetruth-final.XXXXXX)"
ENVFILE="$WORK/current.env"
BUILDCTX="$WORK/buildctx"
LOG="/tmp/nova-r492-pricetruth-final-${STAMP}.log"
NEW_STARTED=0
OLD_RENAMED=0
ROLLBACK_DONE=0
...
wait_health(){
  ...
  for _ in $(seq 1 60); do
    ...
    [ "$status" = "running" ] && sleep 3 || sleep 3
  done
  return 1
}
protected_snapshot(){
...
fetch_file(){
  local src="$1" dest="$2"
  if [[ "$src" =~ ^https?:// ]]; then
    curl -fL --retry 5 --retry-delay 2 --connect-timeout 10 "$src" -o "$dest"
  else
    cp -- "$src" "$dest"
  fi
}
recover_if_missing(){
  ...
  local candidate
  candidate="$(dc ps -a --format '{{.Names}}' | grep -E "^${APP}-pre-(freshrot|pricetruth|pricetruth-final)-[0-9]{8}-[0-9]{6}$" | sort -r | head -n1 || true)"
...
say "1. PRECHECK — UNIQUE FINAL DOCKERFILE + CURRENT CONTAINER"
...
elif [ -f "./${DOCKERFILE_NAME}" ]; then
  cp -- "./${DOCKERFILE_NAME}" "$BUILDCTX/Dockerfile"
else
  fetch_file "${NOVA_DOCKERFILE_URL:-$DEFAULT_URL}" "$BUILDCTX/Dockerfile"
fi
ACTUAL="$(sha256sum "$BUILDCTX/Dockerfile" | awk '{print $1}')"
[ "$ACTUAL" = "$EXPECTED_DOCKER_SHA" ] || { echo "FAIL: FINAL Dockerfile SHA mismatch actual=$ACTUAL expected=$EXPECTED_DOCKER_SHA"; exit 3; }
...
echo "FINAL_DOCKERFILE=$DOCKERFILE_NAME SHA256=$ACTUAL" | tee -a "$LOG"

if image_contract_ok; then
  say "2. REUSE EXACT PRICE-TRUTH FINAL IMAGE"
  echo "IMAGE_REUSE=YES IMAGE=$IMAGE" | tee -a "$LOG"
else
  say "2. BUILD PRICE-TRUTH FINAL IMAGE"
...
trap - ERR INT TERM
say "6. SUCCESS"
echo "RESULT=SUCCESS VERSION=$EXPECTED_VERSION CURRENT=$APP ROLLBACK_CONTAINER=$BACKUP FINAL_DOCKERFILE=$DOCKERFILE_NAME PRICE_TRUTH=ACTIVE_VENUE_MAXAGE15 DATA_SUPPLY=RESTORED_BASELINE_20260825_143621 NEW_WS_TYPE=0" | tee -a "$LOG"
cleanup
