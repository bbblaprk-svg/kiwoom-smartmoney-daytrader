...
RELEASE="3.8.20_SCORE_DIAGNOSTICS"
...
SRC_NAME="PULSE_EDGE_GREENFIELD_3_8_20_SCORE_DIAGNOSTICS_SOURCE.tar.gz"
EXPECTED_SRC_SHA="db42682bf091794c334d5caf78f2f34e516809fc172ae8969fe6a25d0b10bf68"
EXPECTED_DF_SHA="0fd34729642bc53983e9b8a21b9e409245ec5a083770ce86d454d9ddb9c751fd"
MODE="${1:-deploy}"
SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)"
RUN_ID="$(date +%Y%m%d-%H%M%S)-$$"
RUN_DIR="$BASE/deployments/$RUN_ID"
CANARY_DATA="$RUN_DIR/canary-data"
IMG="pulse-edge:gf3820-hidden-$RUN_ID"
CANARY="pulse-edge-gf3820-canary-$RUN_ID"
LIVE="pulse-edge-gf3820-live-$RUN_ID"
OLD=""
OLD_RUNNING=false
OLD_RESTART="no"
CUTOVER=0
...
fixed={
'PULSE_RELEASE':'3.8.20_SCORE_DIAGNOSTICS','PULSE_FEED_ENABLED':'true','PULSE_DATA_DIR':'/app/data/pulse_edge',
...
import json,sys
q=json.load(open(sys.argv[1])); assert q['release']=='3.8.20_SCORE_DIAGNOSTICS' and q['ready'] is True and q['feed_enabled'] is False
PY
  fi
...
import json,sys
q=json.load(open(sys.argv[1])); assert q.get('release')=='3.8.20_SCORE_DIAGNOSTICS' and isinstance(q.get('rows'),list),q
...
import json,sys
h=json.load(open(sys.argv[1])); assert h.get('runtime_safety_release')=='3.8.20_SCORE_DIAGNOSTICS'
PY
public=0
