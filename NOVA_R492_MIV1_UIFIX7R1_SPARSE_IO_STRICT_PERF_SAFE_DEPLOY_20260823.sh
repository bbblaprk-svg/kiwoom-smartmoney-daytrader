...
EXPECTED_DOCKERFILE_SHA="888515017963068d4ed467bb7bbaf5cb316236cda91c1a3c5269dc2717399701"
EXPECTED_EMBEDDED_SOURCE_SHA="aebbce1b54dfab77ea833b990b402c0e8649b0281fd12c7a333492ee0437f624"
EXPECTED_VERSION="NOVA-3.3.5-R492-MARKET-INDEX-VERIFY1"
BASELINE_UIFIX6_LABEL="FULL_EDGE_REACCEL_ALERTS_CLOSED_LOOP_VERIFY"
EXPECTED_UIFIX7_LABEL="CLOSE_FROZEN_PERSIST_TRISTATE_SPARSE_IO_R1"
DEPLOY_SCRIPT_REV="UIFIX7R1_SPARSE_IO_STRICT_PERF_GATE"
BASELINE_VERSION="NOVA-3.3.5-R492-MARKET-INDEX-VERIFY1"
IMAGE="quant-nova:3.3.5-r492-miv1-uifix7r1-sparse-io-local"
STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP="${APP}-pre-uifix7r1-${STAMP}"
CANDIDATE="${APP}-candidate-uifix7r1-${STAMP}"
WORK="$(mktemp -d /tmp/nova-r492-miv1-uifix7r1-sparse-io.XXXXXX)"
DOCKERFILE="$WORK/Dockerfile"
ENVFILE="$WORK/current.env"
CAND_DATA="$WORK/candidate-data"
LOG="/tmp/nova-r492-miv1-uifix7r1-sparse-io-deploy-${STAMP}.log"
LOCKFILE="/tmp/nova-r492-miv1-uifix7r1-sparse-io-deploy.lock"
OLD_WAS_RUNNING=0
OLD_STOPPED=0
OLD_RENAMED=0
NEW_STARTED=0
...
exec 9>"$LOCKFILE"
if ! flock -n 9; then
  echo "INFO: R492 MIV1 UIFIX7R1 SPARSE IO deploy is already running. Do not start it twice."
  exit 0
fi
...
strict_rotation_image_gate(){
  local pass=0 attempt=0
  say "3A/9 STRICT MARKET ROTATION IMAGE GATE — SAME p95 < 25ms THRESHOLD"
  echo "PERF_POLICY=UNCHANGED_TEST d5951f873d9f97e2d4f7c1c50bb1e02f3c20e5d1ac4556de872c57670a7586a9 · REQUIRE=2_CONSECUTIVE_PASS · candidate_web_server=OFF" | tee -a "$LOG"
  while (( attempt < 4 && pass < 2 )); do
    attempt=$((attempt+1))
    sleep 10
    echo "STRICT_PERF_ATTEMPT=$attempt" | tee -a "$LOG"
    if dc run --rm --network none --memory 384m --memory-swap 384m --pids-limit 128 --env-file "$ENVFILE" -e NOVA_CANDIDATE_MODE=1 -e NOVA_OFFLINE=1 --entrypoint python "$IMAGE" /app/scripts/market_rotation_acceptance.py 2>&1 | tee -a "$LOG"; then
      pass=$((pass+1))
      echo "STRICT_PERF_PASS_STREAK=$pass/2" | tee -a "$LOG"
    else
      pass=0
      echo "STRICT_PERF_RETRY=post-build CPU contention suspected; threshold NOT relaxed" | tee -a "$LOG"
    fi
  done
  ...
  echo "STRICT_MARKET_ROTATION_GATE=PASS threshold_lt_25ms=UNCHANGED consecutive_passes=2" | tee -a "$LOG"
}
...
'app/addons/decision_panels.py':'c2eef19d71b020ba6bb54c31ba7ccdf4eb1a167db824acb39e2be0c78d273382',
...
print('UIFIX7R1_CANDIDATE=PASS close_persist=1 sparse_write_300s=1 tristate=1 reaccel_fake_fill=0 price_path_unchanged=1')
PY
