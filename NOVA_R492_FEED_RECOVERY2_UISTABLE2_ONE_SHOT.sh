...
DOCKERFILE="QUANT_NOVA_R492_FEED_RECOVERY2_UISTABLE2.Dockerfile"
EXPECTED_SHA="45935023fb2f1e0f0b8d97e1672816a4d76e94d4b699f5f4f1dba777a49e5f30"
EXPECTED_VERSION="NOVA-3.3.5-R492-FEED-RECOVERY2-UISTABLE2"
IMAGE="quant-nova:r492-feed-recovery2-uistable2"
STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP="${APP}-pre-uistable2-${STAMP}"
FAILED="${APP}-failed-uistable2-${STAMP}"
WORK="$(mktemp -d /tmp/nova-uistable2.XXXXXX)"
...
dc run --rm --entrypoint sh "$IMAGE" -lc '
  grep -q "build:'"'"'UISTABLE2'"'"'" /app/static/stable-board.js &&
  ...
  grep -q "height:58px;min-height:58px;max-height:58px" /app/static/decision-panels.css &&
  grep -q -- "-webkit-line-clamp:3" /app/static/decision-panels.css &&
  grep -q "sig-badge" /app/static/decision-panels.css &&
  grep -q "v=335-r492-uistable2" /app/static/index.html &&
  grep -q "RELEASE_CACHE='"'"'nova-shell-335-r492-uistable2'"'"'" /app/static/sw.js &&
  grep -q "fetch(request,{cache:'"'"'no-store'"'"'})" /app/static/sw.js
' || { echo "FAIL: UISTABLE2 contract missing"; exit 2; }
...
SB="$(curl -fsS --max-time 8 "${AUTH[@]}" -H 'Cache-Control: no-cache' 'http://127.0.0.1:3200/static/stable-board.js?v=335-r492-uistable2')"
DP="$(curl -fsS --max-time 8 "${AUTH[@]}" -H 'Cache-Control: no-cache' 'http://127.0.0.1:3200/static/decision-panels.js?v=335-r492-uistable2')"
CSS="$(curl -fsS --max-time 8 "${AUTH[@]}" -H 'Cache-Control: no-cache' 'http://127.0.0.1:3200/static/decision-panels.css?v=335-r492-uistable2')"
SW="$(curl -fsS --max-time 8 "${AUTH[@]}" -H 'Cache-Control: no-cache' 'http://127.0.0.1:3200/sw.js?v=335-r492-uistable2')"
grep -q 'v=335-r492-uistable2' <<<"$INDEX"
grep -q "build:'UISTABLE2'" <<<"$SB"
grep -q 'sig-badge' <<<"$DP"
grep -q 'height:58px;min-height:58px;max-height:58px' <<<"$CSS"
grep -q "RELEASE_CACHE='nova-shell-335-r492-uistable2'" <<<"$SW"
...
trap - ERR INT TERM
cleanup
echo "RESULT=SUCCESS CURRENT=$APP BACKUP=$BACKUP VERSION=$EXPECTED_VERSION FRONTEND=UISTABLE2"
