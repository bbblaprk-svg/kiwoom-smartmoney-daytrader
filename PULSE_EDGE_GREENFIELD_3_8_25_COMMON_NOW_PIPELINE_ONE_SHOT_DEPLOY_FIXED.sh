...
RELEASE="3.8.25_COMMON_NOW_PIPELINE"
...
SRC_NAME="PULSE_EDGE_GREENFIELD_3_8_25_COMMON_NOW_PIPELINE_SOURCE.tar.gz"
EXPECTED_SRC_SHA="16bec6168d639320fc633eeb622b6a3853caaba674c563533a16c633063fb2af"
EXPECTED_DF_SHA="9334a278513ba6bd5003b2d2f462ef4f1cd001f587ada13b78155df919cd9797"
MODE="${1:-deploy}"
SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)"
RUN_ID="$(date +%Y%m%d-%H%M%S)-$$"
RUN_DIR="$BASE/deployments/$RUN_ID"
CANARY_DATA="$RUN_DIR/canary-data"
IMG="pulse-edge:gf3825-hidden-$RUN_ID"
CANARY="pulse-edge-gf3825-canary-$RUN_ID"
LIVE="pulse-edge-gf3825-live-$RUN_ID"
OLD=""
OLD_RUNNING=false
OLD_RESTART="no"
CUTOVER=0
...
fixed={
'PULSE_RELEASE':'3.8.25_COMMON_NOW_PIPELINE','PULSE_FEED_ENABLED':'true','PULSE_DATA_DIR':'/app/data/pulse_edge',
...
import json,sys
q=json.load(open(sys.argv[1])); assert q['release']=='3.8.25_COMMON_NOW_PIPELINE' and q['ready'] is True and q['feed_enabled'] is False
PY
  fi
...
import json,sys
q=json.load(open(sys.argv[1])); assert q.get('release')=='3.8.25_COMMON_NOW_PIPELINE' and isinstance(q.get('rows'),list),q
...
import json,sys
h=json.load(open(sys.argv[1])); assert h.get('runtime_safety_release')=='3.8.25_COMMON_NOW_PIPELINE'
PY
...
import json,sys
h=json.load(open(sys.argv[1])); assert h.get('runtime_safety_release')=='3.8.25_COMMON_NOW_PIPELINE',h
