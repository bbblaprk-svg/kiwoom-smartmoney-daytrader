...
ZIP_NAME="QUANT_NOVA_2.4.20_RS_LEADERS_TOP10_TRAFFIC_SAFE_FINAL_FIX5.zip"
ROOT_NAME="nova2420"
EXPECTED_ZIP_SHA256="df4128f71563f66a4ddc4572d4fc5d78eceafc5c864bab1dc783ae51d4d9ed5a"
APP_DIR="$HOME/quant-nova"
IMAGE="quant-nova:2.4.20"
...
rollback(){
  local ec=$?
  [[ $ec -eq 0 ]] && return
  echo "[자동복구] NOVA 2.4.20 적용 실패 — 이전 NOVA 복구" >&2
...
echo "=== START QUANT NOVA 2.4.20 DIRECT DEPLOY ==="
...
say "2/7 NOVA 2.4.20 DIRECT RAW 다운로드 + SHA/manifest 검증"
for c in unzip python3 node curl docker sha256sum; do command -v "$c" >/dev/null 2>&1 || fail "필수 명령 없음: $c"; done
ZIP_URL="https://raw.githubusercontent.com/bbblaprk-svg/kiwoom-smartmoney-daytrader/main/${ZIP_NAME}"
echo "DOWNLOAD=$ZIP_URL"
if ! curl -fL --retry 3 --retry-delay 2 --connect-timeout 10 --max-time 180 "$ZIP_URL" -o "$TMP/$ZIP_NAME"; then
  fail "ZIP 직접 다운로드 실패 — GitHub 최상위 파일명/업로드 여부를 확인하세요. 저장소가 Private이면 GITHUB_TOKEN 방식 updater가 필요합니다."
fi
[[ -s "$TMP/$ZIP_NAME" ]] || fail "다운로드된 ZIP이 비어 있습니다."
...
grep -Fq "APP_VERSION='NOVA-2.4.20'" "$ROOT/app/main.py" || fail "버전 동기화 누락"
...
grep -Fq "MARKET / SECTOR RELATIVE STRENGTH LEADERS · 2.4.20" "$ROOT/static/index.html" || fail "RS Leaders TOP10 UI 누락"
...
grep -Fq "PROSPECTIVE EVIDENCE TRUTH + SHADOW VALIDATION · 2.4.20" "$ROOT/static/index.html" || fail "2.4.20 Shadow Evidence UI 동기화 누락"
...
grep -Fq "SMART MONEY CROSS-VENUE · 2.4.20" "$ROOT/static/index.html" || fail "2.4.20 스마트머니 TOP10 UI 누락"
...
grep -Fq "/static/nova.js?v=2.4.20-rsleaders1" "$ROOT/static/index.html" || fail "알림 UI cache-bust 누락"
...
[[ -n "$HEALTH" ]] || { sudo docker logs --tail 300 "$CONTAINER" || true; fail "NOVA 2.4.20 health 실패"; }
printf '%s' "$HEALTH" > "$TMP/health.json"
python3 - "$TMP/health.json" <<'PYH'
import json,sys
h=json.load(open(sys.argv[1]))
assert h.get('ok') is True,h
assert h.get('version')=='NOVA-2.4.20',h.get('version')
...
rsl=h.get('rs_leader_table') or {}; assert rsl.get('version')=='RS_LEADER_TABLE_V1' and int(rsl.get('kiwoom_extra_requests') or 0)==0 and rsl.get('buy_effect') is False,rsl
...
curl -fsS --max-time 5 http://127.0.0.1:${HOST_PORT}/api/nova | python3 -c 'import sys,json;j=json.load(sys.stdin); assert j.get("version")=="NOVA-2.4.20",(j.get("version"),j.get("snapshot_source_version")); print("NOVA_API=OK version=",j.get("version"),"snapshot_source_version=",j.get("snapshot_source_version"),"rows=",len(j.get("rows") or []))'
echo "CHECK_API=/api/rs-leaders"
curl -fsS --max-time 5 http://127.0.0.1:${HOST_PORT}/api/rs-leaders | python3 -c 'import sys,json;j=json.load(sys.stdin); assert j.get("version")=="NOVA-2.4.20" and int(j.get("kiwoom_extra_requests_per_poll") or 0)==0; tr=j.get("traffic") or {}; assert int(tr.get("market_data_extra_requests") or 0)==0; print("RS_LEADERS_API=OK rows=",j.get("count"),"payload=",tr.get("payload_bytes"),"B build=",j.get("build_ms"),"ms")'
echo "CHECK_API=/api/screen-state"
curl -fsS --max-time 5 http://127.0.0.1:${HOST_PORT}/api/screen-state | python3 -c 'import sys,json;j=json.load(sys.stdin); assert j.get("version")=="NOVA-2.4.20"; assert "runtime_awake" in j and "screen_hold" in j; h=j.get("screen_hold") or {}; assert "07:30" in str(h.get("policy") or ""); print("SCREEN_STATE_API=OK awake=",j.get("runtime_awake")," hold=",h.get("active")," display_day=",j.get("display_day"))'
echo "CHECK_API=/api/ws-contract"
curl -fsS --max-time 5 http://127.0.0.1:${HOST_PORT}/api/ws-contract | python3 -c 'import sys,json;j=json.load(sys.stdin); assert j.get("version")=="NOVA-2.4.20"; c=j.get("contract") or {}; assert c.get("trade_type")=="0B" and c.get("program_type")=="0w" and c.get("orderbook_type")=="0D"; print("WS_CONTRACT_API=OK",c)'
echo "CHECK_API=/api/nxt-signal-table"
curl -fsS --max-time 5 http://127.0.0.1:${HOST_PORT}/api/nxt-signal-table | python3 -c 'import sys,json;j=json.load(sys.stdin); assert j.get("version")=="NOVA-2.4.20" and "rows" in j; print("NXT_SIGNAL_TABLE_API=OK rows=",len(j.get("rows") or []))'
echo "CHECK_API=/api/close-picks"
curl -fsS --max-time 5 http://127.0.0.1:${HOST_PORT}/api/close-picks | python3 -c 'import sys,json;j=json.load(sys.stdin); assert j.get("version")=="NOVA-2.4.20" and "visible" in j; print("CLOSE_PICKS_API=OK visible=",j.get("visible"),"count=",j.get("count"))'
echo "CHECK_API=/api/close-smart-money"
curl -fsS --max-time 5 http://127.0.0.1:${HOST_PORT}/api/close-smart-money | python3 -c 'import sys,json;j=json.load(sys.stdin); assert j.get("version")=="NOVA-2.4.20" and "rows" in j and "sectors" in j; print("CLOSE_SMART_MONEY_API=OK visible=",j.get("visible"),"count=",j.get("count"),"dual=",j.get("dual_confirmed_count"))'
echo "CHECK_API=/api/position-manager"
curl -fsS --max-time 5 http://127.0.0.1:${HOST_PORT}/api/position-manager | python3 -c 'import sys,json;j=json.load(sys.stdin); assert j.get("version")=="NOVA-2.4.20"; p=j.get("plans") or {}; assert p.get("SWING",{}).get("ELITE",{}).get("runner_pct")==65.0; print("POSITION_MANAGER_API=OK open=",j.get("open_count"))'
echo "CHECK_API=/api/exit-dna"
curl -fsS --max-time 5 http://127.0.0.1:${HOST_PORT}/api/exit-dna | python3 -c 'import sys,json;j=json.load(sys.stdin); assert j.get("version")=="NOVA-2.4.20"; print("EXIT_DNA_API=OK")'
echo "CHECK_API=/api/nxt-alerts"
curl -fsS --max-time 5 http://127.0.0.1:${HOST_PORT}/api/nxt-alerts | python3 -c 'import sys,json;j=json.load(sys.stdin); assert j.get("version")=="NOVA-2.4.20"; print("NXT_ALERT_API=OK count=",j.get("count"))'

# evidence_report.json is a derived cache and may have been written by an older NOVA version.
# Rebuild only this derived report from preserved source ledgers before asserting the 2.4.20 schema.
