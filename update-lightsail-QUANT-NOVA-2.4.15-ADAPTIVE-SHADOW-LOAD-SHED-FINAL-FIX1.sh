...
grep -Fq "PROSPECTIVE EVIDENCE TRUTH + SHADOW VALIDATION · 2.4.15" "$ROOT/static/index.html" || fail "2.4.15 Shadow Evidence UI 동기화 누락"
...
grep -Fq "SMART MONEY CROSS-VENUE · 2.4.15" "$ROOT/static/index.html" || fail "2.4.15 스마트머니 TOP10 UI 누락"
...
assert b.get('version')=='NOVA-2.4.15',b
...
assert h.get('version')=='NOVA-2.4.15',h.get('version')
...
curl -fsS --max-time 5 http://127.0.0.1:${HOST_PORT}/api/nova | python3 -c 'import sys,json;j=json.load(sys.stdin); assert j.get("version")=="NOVA-2.4.15"; print("NOVA_API=OK rows=",len(j.get("rows") or []))'
curl -fsS --max-time 5 http://127.0.0.1:${HOST_PORT}/api/screen-state | python3 -c 'import sys,json;j=json.load(sys.stdin); assert j.get("version")=="NOVA-2.4.15"; assert "runtime_awake" in j and "screen_hold" in j; h=j.get("screen_hold") or {}; assert "07:30" in str(h.get("policy") or ""); print("SCREEN_STATE_API=OK awake=",j.get("runtime_awake")," hold=",h.get("active")," display_day=",j.get("display_day"))'
curl -fsS --max-time 5 http://127.0.0.1:${HOST_PORT}/api/ws-contract | python3 -c 'import sys,json;j=json.load(sys.stdin); assert j.get("version")=="NOVA-2.4.15"; c=j.get("contract") or {}; assert c.get("trade_type")=="0B" and c.get("program_type")=="0w" and c.get("orderbook_type")=="0D"; print("WS_CONTRACT_API=OK",c)'
curl -fsS --max-time 5 http://127.0.0.1:${HOST_PORT}/api/nxt-signal-table | python3 -c 'import sys,json;j=json.load(sys.stdin); assert j.get("version")=="NOVA-2.4.15" and "rows" in j; print("NXT_SIGNAL_TABLE_API=OK rows=",len(j.get("rows") or []))'
curl -fsS --max-time 5 http://127.0.0.1:${HOST_PORT}/api/close-picks | python3 -c 'import sys,json;j=json.load(sys.stdin); assert j.get("version")=="NOVA-2.4.15" and "visible" in j; print("CLOSE_PICKS_API=OK visible=",j.get("visible"),"count=",j.get("count"))'
curl -fsS --max-time 5 http://127.0.0.1:${HOST_PORT}/api/close-smart-money | python3 -c 'import sys,json;j=json.load(sys.stdin); assert j.get("version")=="NOVA-2.4.15" and "rows" in j and "sectors" in j; print("CLOSE_SMART_MONEY_API=OK visible=",j.get("visible"),"count=",j.get("count"),"dual=",j.get("dual_confirmed_count"))'
curl -fsS --max-time 5 http://127.0.0.1:${HOST_PORT}/api/position-manager | python3 -c 'import sys,json;j=json.load(sys.stdin); assert j.get("version")=="NOVA-2.4.15"; p=j.get("plans") or {}; assert p.get("SWING",{}).get("ELITE",{}).get("runner_pct")==65.0; print("POSITION_MANAGER_API=OK open=",j.get("open_count"))'
curl -fsS --max-time 5 http://127.0.0.1:${HOST_PORT}/api/exit-dna | python3 -c 'import sys,json;j=json.load(sys.stdin); assert j.get("version")=="NOVA-2.4.15"; print("EXIT_DNA_API=OK")'
curl -fsS --max-time 5 http://127.0.0.1:${HOST_PORT}/api/nxt-alerts | python3 -c 'import sys,json;j=json.load(sys.stdin); assert j.get("version")=="NOVA-2.4.15"; print("NXT_ALERT_API=OK count=",j.get("count"))'

curl -fsS --max-time 8 http://127.0.0.1:${HOST_PORT}/api/evidence | python3 -c 'import sys,json;j=json.load(sys.stdin); assert j.get("ok") is True and j.get("version")=="NOVA-2.4.15"; r=j.get("report") or {}; assert r.get("version")=="EVIDENCE_ENGINE_V3_HORIZON_COVERAGE"; p=r.get("prospective") or {}; assert "coverage" in p and "market_top1_audit" in p; sh=r.get("shadow_oos") or {}; assert sh.get("auto_apply") is False; t=p.get("top1_target_audit") or {}; assert int(t.get("min_oos_samples") or 300)==300; assert t.get("top1_target_precision_pct") is None or int(t.get("samples") or 0)>=300; print("EVIDENCE_API=OK 30M_COVERAGE=",(p.get("coverage") or {}).get("30m")," MARKET_TOP1=",(p.get("market_top1_audit") or {}).get("status"))'
curl -fsS --max-time 5 http://127.0.0.1:${HOST_PORT}/api/backtest/status | python3 -c 'import sys,json;j=json.load(sys.stdin); assert j.get("ok") is True and j.get("version")=="NOVA-2.4.15" and j.get("auto_apply") is False; assert "observation" in j.get("command",""); print("BACKTEST_STATUS_API=OK auto_apply=",j.get("auto_apply"))'
