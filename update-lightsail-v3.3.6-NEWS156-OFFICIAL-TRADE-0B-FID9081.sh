...
ZIP_NAME="kiwoom-unified-edge-v3.3.6-NEWS156-OFFICIAL-TRADE-0B-FID9081.zip"
SOURCE_DIR_NAME="kiwoom-unified-edge-v3.3.6"
EXPECTED_ZIP_SHA256="186e1aeb8574ec01bd4c872dd07208b96fa3532ce08134344e67a7549b3dceca"
OLD_APP_DIR="$HOME/kiwoom-smartmoney-daytrader"
NEW_APP_DIR="$HOME/kiwoom-unified-edge"
TRADER_IMAGE="kiwoom-unified:v3.3.6"
...
rollback(){
  local ec=$?
  [[ $ec -eq 0 ]] && return
  printf '\n[자동복구] v3.3.6 OFFICIAL DOMESTIC REG 적용 실패 — 기존 컨테이너로 복구합니다.\n' >&2
...
say "v3.3.6 REALTIME FOUNDATION + OFFICIAL DOMESTIC REG + News 1.5.6 파일 받기"
...
[[ "$TRADER_VERSION" == "3.3.6" ]] || fail "trader 버전 불일치: $TRADER_VERSION"
...
grep -Fq "entry.type === '0B'" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "주식체결 0B 판정 누락"
grep -Fq "case '0B':" "$ROOT/trader/lib/realtimeStore.js" || fail "realtimeStore 0B 체결 처리 누락"
! grep -Fq "DOMESTIC_TRADE_0A" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "잘못된 0A 체결 REG 잔존"
grep -Fq "KIWOOM_OFFICIAL_DOMESTIC_BARE_REG_V4" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "v3.3.6 공식 국내 WebSocket 등록모드 누락"
