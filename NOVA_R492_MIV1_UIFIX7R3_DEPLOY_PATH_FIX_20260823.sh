...
DEPLOY_SCRIPT_REV="UIFIX7R3_STRICT_PERF_PYTHONPATH_AND_REUSE_FIX"
BASELINE_VERSION="NOVA-3.3.5-R492-MARKET-INDEX-VERIFY1"
IMAGE="quant-nova:3.3.5-r492-miv1-uifix7r2-sparse-io-local"
STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP="${APP}-pre-uifix7r3-${STAMP}"
CANDIDATE="${APP}-candidate-uifix7r3-${STAMP}"
WORK="$(mktemp -d /tmp/nova-r492-miv1-uifix7r3-pathfix.XXXXXX)"
DOCKERFILE="$WORK/Dockerfile"
ENVFILE="$WORK/current.env"
CAND_DATA="$WORK/candidate-data"
LOG="/tmp/nova-r492-miv1-uifix7r3-pathfix-deploy-${STAMP}.log"
LOCKFILE="/tmp/nova-r492-miv1-uifix7r3-pathfix-deploy.lock"
OLD_WAS_RUNNING=0
OLD_STOPPED=0
OLD_RENAMED=0
NEW_STARTED=0
...
exec 9>"$LOCKFILE"
if ! flock -n 9; then
  echo "INFO: R492 MIV1 UIFIX7R3 deploy-path-fix is already running. Do not start it twice."
  exit 0
fi
...
strict_rotation_image_gate(){
  ...
  dc run --rm --network none --memory 384m --memory-swap 384m --pids-limit 128 \
    --env-file "$ENVFILE" -e NOVA_CANDIDATE_MODE=1 -e NOVA_OFFLINE=1 -e PYTHONPATH=/app \
