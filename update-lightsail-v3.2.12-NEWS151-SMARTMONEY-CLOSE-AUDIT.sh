...
ZIP_NAME="kiwoom-unified-edge-v3.2.12-NEWS151-SMARTMONEY-CLOSE-AUDIT.zip"
SOURCE_DIR_NAME="kiwoom-unified-edge-v3.2.12"
EXPECTED_ZIP_SHA256="5afcfd890117861091a02b6b4905242c6fdb78ed919f5625b7dbf94ac875ba40"
OLD_APP_DIR="$HOME/kiwoom-smartmoney-daytrader"
NEW_APP_DIR="$HOME/kiwoom-unified-edge"
TRADER_IMAGE="kiwoom-unified:v3.2.12"
...
rollback() {
  ...
  printf '\n[자동 복구] v3.2.12 SMART MONEY CLOSE AUDIT 적용 실패 — 기존 앱으로 복구합니다.\n' >&2
...
say "v3.2.12 SMART MONEY CLOSE AUDIT 파일 받기"
...
# v3.2.12 compatibility check: 기존 trader와 동일하게 APP_ACCESS_TOKEN 최소 길이를 8자로 통일한다.
...
[[ "$TRADER_VERSION" == "3.2.12" ]] || fail "trader package 버전이 3.2.12가 아닙니다: $TRADER_VERSION"
...
# v3.2.12 추천용 시장발굴 + KRX/NXT dual-feed + close-audit guard. Kiwoom 실시간 WebSocket/TR/FID 수신 설정은 변경하지 않는다.
...
# v3.2.12 SMART MONEY CLOSE AUDIT dual-feed 장애방어 기본값. 기존 사용자가 명시한 값은 보존한다.
...
# v3.2.12에서는 15분 전체 스냅샷 환경값을 사용하지 않는다. 오래된 값은 제거해 혼선을 방지한다.
...
say "v3.2.12 SMART MONEY CLOSE AUDIT 공개앱 실행"
sudo docker run -d \
  --name "$APP_CONTAINER" \
  --restart unless-stopped \
  --network "$DOCKER_NETWORK" \
...
[[ "$RUNNING_VERSION" == "3.2.12" ]] || fail "실행 버전 불일치: $RUNNING_VERSION"
...
say "SMART MONEY CLOSE AUDIT 구조 확인"
CLOSE_AUDIT_OK="$(sudo docker exec "$APP_CONTAINER" node -e "fetch('http://127.0.0.1:3000/api/health').then(r=>r.json()).then(h=>{const ok=String(h.version||'').startsWith('3.2.12')&&Number.isFinite(Number(h.smartMoneyCheckpoints))&&Number.isFinite(Number(h.smartMoneyCheckpointFull))&&Number.isFinite(Number(h.smartMoneyCheckpointPartial))&&typeof h.closeAuditMode==='boolean'&&Number.isFinite(Number(h.closeAuditFullCount));console.log(ok?'OK':'BAD',JSON.stringify({version:h.version,checkpoints:h.smartMoneyCheckpoints,full:h.smartMoneyCheckpointFull,partial:h.smartMoneyCheckpointPartial,closeAuditMode:h.closeAuditMode,closeAuditFullCount:h.closeAuditFullCount}));process.exit(ok?0:1)}).catch(e=>{console.error(e.message);process.exit(1)})" 2>&1 || true)"
[[ "$CLOSE_AUDIT_OK" == OK* ]] || fail "SMART MONEY CLOSE AUDIT 구조 확인 실패: $CLOSE_AUDIT_OK"
printf '%s\n' "$CLOSE_AUDIT_OK"
...
printf '\n============================================================\n'
printf 'KIWOOM UNIFIED EDGE v3.2.12 SMART MONEY CLOSE AUDIT 설치 완료\n'
printf '공개 앱     : https://%s\n' "$DOMAIN"
printf '공개 컨테이너: kiwoom-app v3.2.11\n'
...
printf '\nSafari/PWA를 완전히 닫았다가 다시 열고 상단 v3.2.12 SMART MONEY CLOSE AUDIT와 통합레이더 탭을 확인하세요.\n'
