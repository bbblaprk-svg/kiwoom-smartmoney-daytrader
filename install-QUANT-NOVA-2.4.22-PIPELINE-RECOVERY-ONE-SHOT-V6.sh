...
REV="QUANT-NOVA-2.4.22-PIPELINE-RECOVERY-ONE-SHOT-V6"
APP="quant-nova"
STAMP="$(date +%Y%m%d%H%M%S)"
WORK="/tmp/nova-v6-$STAMP"
BK="$HOME/quant-nova/pipeline-recovery-v6-backups"
...
if "WS_QUOTA_COOLDOWN_V6" not in fn:
    ...
    new="""                            msg=str(m.get('return_msg') or f"{kind} registry failed")[:180]\n                            if '허용된 요청 건수를 초과' in msg or '요청 건수' in msg:\n                                # WS_QUOTA_COOLDOWN_V6: preserve healthy socket and successful registrations.\n                                blocked_until[kind]=time.time()+60\n                                STATE.ws_status.update({'quota_error':msg,'quota_error_at':time.time(),'quota_guard':'SESSION_AWARE_HARD_CAP_COOLDOWN_V6','subscription_budget':WS_REG_ITEM_BUDGET,'quota_cooldown_kind':kind,'quota_cooldown_until':blocked_until[kind]})\n                                continue\n                            raise RuntimeError(msg)"""
    if old in fn:
        fn=fn.replace(old,new,1)
...
print("KA90003_REQUIRED_FIELDS_GATE=PASS")
...
print("ARCHIVE_STATUS",(post.get('archive') or {}).get('seal_ready'),(post.get('archive') or {}).get('seal_waiting'))
PY
...
TAG="quant-nova:2.4.22-pipeline-recovery-v6-$STAMP"
