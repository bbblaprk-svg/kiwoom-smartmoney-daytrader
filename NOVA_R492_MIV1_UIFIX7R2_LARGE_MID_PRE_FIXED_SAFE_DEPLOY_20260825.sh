...
EXPECTED_DOCKERFILE_SHA="0801d26de8bbb0a741051d4ac0b7bdeca20e400fa7c9e9eae4b4bc6fc325a358"
EXPECTED_SOURCE_SHA="0d2f300af673c7187ae51b3ec9acc0e13e1dfaa6211d7b76a4f721130df632fc"
...
BASE_INDEX_SHA="d44261de26a5ed3f8f2fd6febfd999d762d7d654ec7bd8dd851fc38f99e870c1"
IMAGE="quant-nova:3.3.5-r492-miv1-uifix7r2-lmpre-fixed20-v1"
STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP="${APP}-pre-lmpre-${STAMP}"
FAILED="${APP}-failed-lmpre-${STAMP}"
CANDIDATE="${APP}-lmpre-candidate-${STAMP}"
WORK="$(mktemp -d /tmp/nova-lmpre-deploy.XXXXXX)"
DOCKERFILE="$WORK/Dockerfile"
ENVFILE="$WORK/current.env"
CAND_DATA="$WORK/candidate-data"
LOG="/tmp/nova-lmpre-deploy-${STAMP}.log"
...
rollback(){
  ...
  trap - ERR INT TERM
  say "AUTO ROLLBACK -> EXACT PHOTO BASELINE UIFIX7R2"
  ...
  if [ "$OLD_RENAMED" -eq 1 ] && dc inspect "$BACKUP" >/dev/null 2>&1; then
    if [ -n "$BASELINE_IMAGE_ID" ] && [ "$(image_id "$BACKUP")" != "$BASELINE_IMAGE_ID" ]; then
      echo "CRITICAL: photo baseline image id changed; refusing non-exact rollback." | tee -a "$LOG"
      cleanup; exit "$rc"
    fi
...
with tempfile.TemporaryDirectory(prefix='nova-lmpre-preflight-') as td:
    ...
    with tarfile.open(fileobj=io.BytesIO(raw),mode='r:gz') as tf:
        ...
        tf.extractall(root)
    required={'app/addons/large_mid_pre.py','static/large-mid-pre.js','tests/test_r492_large_mid_pre_fixed.py','scripts/r492_large_mid_pre_fixed_acceptance.py','R492_LARGE_MID_PRE_FIXED_INVARIANTS.md','SOURCE_MANIFEST.sha256'}
...
print(f'PREFLIGHT=PASS chunks={len(parts)} gzip_bytes={len(raw)} source_sha={actual} fixed_pre20=1 fixed_lmpre20=1 protected_core=9 new_ws=0 rest_max=2')
PY
}
...
say "1/8 FETCH APPROVED FIXED20 DOCKERFILE + SHA LOCK"
fetch_dockerfile
...
say "2/8 VERIFY EXACT PHOTO BASELINE OR ALREADY DEPLOYED"
...
echo "current_version=${CURRENT_VERSION:-UNKNOWN} uifix7=${CURRENT_UIFIX7:-NONE} patch=${CURRENT_PATCH:-NONE} source=${CURRENT_SOURCE:-NONE}" | tee -a "$LOG"
if [ "$CURRENT_VERSION" = "$EXPECTED_VERSION" ] && [ "$CURRENT_UIFIX7" = "$EXPECTED_UIFIX7" ] && [ "$CURRENT_PATCH" = "$EXPECTED_PATCH" ] && [ "$CURRENT_SOURCE" = "$EXPECTED_SOURCE_SHA" ]; then
  wait_health "$APP"; runtime_gate "$APP"
  trap - ERR INT TERM; cleanup
  echo "RESULT=ALREADY_DEPLOYED VERSION=$EXPECTED_VERSION PATCH=$EXPECTED_PATCH CURRENT=$APP LOG=$LOG"
  exit 0
fi
[ "$CURRENT_VERSION" = "$EXPECTED_VERSION" ] || { echo "FAIL: current version is not photo baseline MIV1" | tee -a "$LOG"; false; }
[ "$CURRENT_UIFIX7" = "$EXPECTED_UIFIX7" ] || { echo "FAIL: current UIFIX7 is not exact R2" | tee -a "$LOG"; false; }
[ "$CURRENT_SOURCE" = "$BASE_SOURCE_SHA" ] || { echo "FAIL: current embedded source is not exact photo baseline" | tee -a "$LOG"; false; }
CURRENT_INDEX_SHA="$(dc exec "$APP" sha256sum /app/static/index.html | awk '{print $1}')"
[ "$CURRENT_INDEX_SHA" = "$BASE_INDEX_SHA" ] || { echo "FAIL: current index.html is not exact photo baseline" | tee -a "$LOG"; false; }
...
say "5/8 ATOMIC CUTOVER — KEEP EXACT PHOTO BASELINE BACKUP"
...
trap - ERR INT TERM
cleanup
printf 'RESULT=DEPLOYED CURRENT=%s VERSION=%s PATCH=%s BACKUP=%s LOG=%s\n' "$APP" "$EXPECTED_VERSION" "$EXPECTED_PATCH" "$BACKUP" "$LOG"
