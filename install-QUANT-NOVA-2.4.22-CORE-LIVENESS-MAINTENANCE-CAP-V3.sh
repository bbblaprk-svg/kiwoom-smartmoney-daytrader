#!/usr/bin/env bash
set -Eeuo pipefail
REV="QUANT-NOVA-2.4.22-CORE-LIVENESS-MAINTENANCE-CAP-V3"
APP="quant-nova"
...
for i in $(seq 1 20); do
  ...
  curl -sS --max-time 2 -o /dev/null -w '%{http_code}' http://127.0.0.1:3200/api/ws-diagnostics > /tmp/v3c & C=$!
  wait "$A" || true; wait "$B" || true; wait "$C" || true
  RA="$(cat /tmp/v3a 2>/dev/null || true)"; RB="$(cat /tmp/v3b 2>/dev/null || true)"; RC="$(cat /tmp/v3c 2>/dev/null || true)"
  echo "V3_CONCURRENCY $i health=$RA nova=$RB ws=$RC"
