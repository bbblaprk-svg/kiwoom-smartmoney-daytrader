...
VERSION="3.1.1"
REV="MASTER-GROUNDUP-9A-PULL-ONLY"
APP_VERSION="NOVA-3.1.1-MASTER-GROUNDUP-9A"
IMAGE="ghcr.io/bbblaprk-svg/kiwoom-smartmoney-daytrader:nova-3.1.1-master-groundup-9a"
EXPECTED_SOURCE_ZIP_SHA256="74d4f3defbe544c92f4fd083fddbbe7fa8557056e6115761b047daeae33c5f7b"
...
DEPLOY_DIR="$APP_DIR/deployments/nova3110-master-groundup9a-$STAMP"
BACKUP="quant-nova-backup-master-groundup9a-$STAMP"
CANDIDATE="quant-nova-candidate-master-groundup9a-$STAMP"
...
rollback(){
  ...
  echo "[AUTO-ROLLBACK] G9 미완료 — 새 컨테이너 제거 및 기존 immutable 상태 복원" >&2
  ...
  if [[ $ACTIVE_DATA_MUTATED -eq 1 && -d "$BASE_DATA/nova30" ]]; then
    ...
    echo "[AUTO-ROLLBACK] G9 NOVA30 state restored; failed attempt preserved at $FAILED_DATA" >&2
  fi
...
sudo docker exec "$CANDIDATE" python -c "import json,urllib.request;j=json.load(urllib.request.urlopen('http://127.0.0.1:8000/api/readyz',timeout=2));assert j.get('ok') and j.get('version')=='NOVA-3.1.1-MASTER-GROUNDUP-9A',j;print('CANDIDATE_READYZ=PASS',j)"
...
curl -fsS --max-time 3 "http://127.0.0.1:${HOST_PORT}/api/livez" | python3 -c 'import json,sys;j=json.load(sys.stdin);assert j.get("ok") and j.get("version")=="NOVA-3.1.1-MASTER-GROUNDUP-9A",j;print("ACTIVE_LIVEZ=PASS",j)'
...
sudo docker exec "$CONTAINER" python -c "import json,urllib.request;j=json.load(urllib.request.urlopen('http://127.0.0.1:8000/api/readyz',timeout=2));assert j.get('ok') and j.get('version')=='NOVA-3.1.1-MASTER-GROUNDUP-9A',j;print('ACTIVE_READYZ=PASS',j)"
...
if [[ -d "$GUARD_STATIC" ]] && sudo docker inspect nova-http-guard >/dev/null 2>&1; then
  mkdir -p "$GUARD_BK" "$TMP/g9a-static"
  ...
  [[ "$CLEAN_GUARD_SHA" == "62a05451d955a187b49dde3d2481de22b1453e27b7ddc9f8effa78b6297bae1f" ]] || die "clean guard SHA mismatch: $CLEAN_GUARD_SHA"
  ...
  for f in index.html nova.js nova.css manifest.webmanifest sw.js; do
    sudo docker cp "$CONTAINER:/app/static/$f" "$TMP/g9a-static/$f"
  done
  grep -Fq "h['Authorization']='Bearer '+token" "$TMP/g9a-static/nova.js" || die "G9A source-level Authorization UI 없음"
  grep -Fq "startLivePoll" "$TMP/g9a-static/nova.js" || die "G9A permanent UI poll 없음"
  ! grep -Eq '__NOVA_INLINE_AUTH_V3__|__NOVA_ALWAYS_ON_POLL_R7__|NOVA_R6|NOVA_R7' "$TMP/g9a-static/nova.js" || die "debug JS lineage 발견"
  ...
  for f in index.html nova.js nova.css manifest.webmanifest sw.js; do
    sudo cp "$TMP/g9a-static/$f" "$GUARD_STATIC/.$f.nova311.tmp"
    sudo chmod 644 "$GUARD_STATIC/.$f.nova311.tmp"
    sudo mv "$GUARD_STATIC/.$f.nova311.tmp" "$GUARD_STATIC/$f"
  done
  sudo rm -f "$GUARD_CACHE"/* >/dev/null 2>&1 || true
...
PYGUARD
  ...
  curl -ksS --max-time 5 "https://3-38-25-20.nip.io/static/nova.js?g9a=$STAMP" -o "$PUBLIC_JS"
  grep -Fq "h['Authorization']='Bearer '+token" "$PUBLIC_JS" || die "public JS auth contract 실패"
  ...
  curl -ksS --max-time 5 "https://3-38-25-20.nip.io/api/auth/check?g9a0=$STAMP" -o "$A0"
  if [[ -n "$AUTH_TOKEN" ]]; then
    curl -ksS --max-time 5 -H "Authorization: Bearer $AUTH_TOKEN" "https://3-38-25-20.nip.io/api/auth/check?g9a1=$STAMP" -o "$A1"
    curl -ksS --max-time 5 "https://3-38-25-20.nip.io/api/auth/check?g9a2=$STAMP" -o "$A2"
    python3 - "$A0" "$A1" "$A2" <<'PYAUTH'
import json,sys
a0,a1,a2=[json.load(open(x,encoding='utf-8')) for x in sys.argv[1:]]
...
print('PUBLIC_AUTH_NO_CACHE=PASS')
PYAUTH
  else
    python3 - "$A0" <<'PYAUTH'
import json,sys
...
rm -rf "$TMP"
trap - EXIT
echo "=== QUANT NOVA 3.1.1 MASTER GROUND-UP 9A PULL-ONLY DEPLOY PASS ==="
