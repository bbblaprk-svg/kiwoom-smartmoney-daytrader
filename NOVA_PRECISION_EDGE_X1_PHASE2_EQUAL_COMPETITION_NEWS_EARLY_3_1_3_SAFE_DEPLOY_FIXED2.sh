#!/usr/bin/env bash
# NOVA PRECISION EDGE X1 PHASE2 · EQUAL COMPETITION + NEWS EARLY · 3.1.3
# Safe build/canary/cutover with automatic rollback. No runtime source patching.
set -Eeuo pipefail

PRODUCT='NOVA PRECISION EDGE X1 PHASE2 · EQUAL COMPETITION + NEWS EARLY · 3.1.3'
VERSION='NOVA-PRECISION-EDGE-X1-PHASE2-EQUAL-COMPETITION-NEWS-EARLY-3.1.3'
CURRENT_CONTAINER="${NOVA_CURRENT_CONTAINER:-quant-nova}"
DOCKERFILE_NAME='NOVA_PRECISION_EDGE_X1_PHASE2_EQUAL_COMPETITION_NEWS_EARLY_3_1_3.Dockerfile'
EXPECTED_DOCKERFILE_SHA256='0741f522334154d58d0b1319351c0b96600e97b6568abfc8ef59c641385934f0'
...
IMAGE_TAG="quant-nova:equal-news-3-1-3-${STAMP}"
BACKUP_CONTAINER="quant-nova-backup-before-equal-news-3-1-3-${STAMP}"
CANARY_CONTAINER="quant-nova-canary-equal-news-3-1-3-${STAMP}"
BUILD_CONTEXT=''
CUTOVER_STARTED=0
...
[[ "$(label io.quantnova.signal_occurrence)" == 'DISTINCT_IGNITION_CYCLES_NOT_STAGE_EVENTS' ]] || fail 'signal occurrence contract mismatch'
[[ "$(label io.quantnova.signal_visibility)" == 'DO_NOT_ERASE_VALID_ROWS_ON_TIME_GATE_OR_TRANSIENT_GAP' ]] || fail 'signal visibility contract mismatch'
[[ "$(label io.quantnova.intraday_snapshot)" == 'SAME_DAY_PRE_PREBUY_PULLBACK_DISPLAY_ONLY' ]] || fail 'intraday snapshot contract mismatch'
...
assert edge.get('version')=='NOVA-PRECISION-EDGE-X1-PHASE2-EQUAL-COMPETITION-NEWS-EARLY-3.1.3', edge.get('version')
...
say "SIGNAL_OCCURRENCE=DISTINCT_IGNITION_CYCLES"
say "SIGNAL_DISPLAY_CONTINUITY=DO_NOT_ERASE_VALID_ROWS_ON_TIME_GATE_OR_TRANSIENT_GAP"
