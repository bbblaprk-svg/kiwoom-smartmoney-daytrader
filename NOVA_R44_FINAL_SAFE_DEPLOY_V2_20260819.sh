...
IMAGE="quant-nova:3.3.4-r44-final-v2-local"
...
LOG="/tmp/nova-r44-final-v2-deploy-${STAMP}.log"
OLD_WAS_RUNNING=0
OLD_STOPPED=0
OLD_RENAMED=0
NEW_STARTED=0
...
recover_previous_if_needed(){
  ...
  latest="$(dc ps -a --format '{{.Names}}' | grep -E "^${APP}-pre-r44-[0-9]{8}-[0-9]{6}$" | sort | tail -n 1 || true)"
  if [ -n "$latest" ]; then
