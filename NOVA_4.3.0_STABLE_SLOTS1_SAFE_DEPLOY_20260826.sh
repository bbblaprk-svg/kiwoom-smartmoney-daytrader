...
DOCKERFILE_NAME="QUANT_NOVA_4.3.0_STABLE_SLOTS1_20260826.Dockerfile"
EXPECTED_SHA256="4b4d438d134871365c52bccbe1fd7762f37cf21010cf3e9928858083a5efece6"
APP_DIR="$HOME/quant-nova"
DATA_DIR="$APP_DIR/data"
ENV_FILE="$APP_DIR/.env"
IMAGE="quant-nova:4.3.0-stable-slots1"
CONTAINER="quant-nova"
NETWORK="kiwoom-net"
HOST_PORT="3200"
TMP="$(mktemp -d)"
STAMP="$(date +%Y%m%d%H%M%S)"
BACKUP="${CONTAINER}-pre-stable-slots-${STAMP}"
STATE_FILE="$APP_DIR/.stable_slots_previous_container"
OLD_RENAMED=0
NEW_STARTED=0
...
restore(){
  ...
  echo "[자동복구] STABLE-SLOTS 적용 실패 — 직전 NOVA를 복구합니다." >&2
...
printf '\n==> 1/8 새 STABLE-SLOTS 파일 취득 및 SHA256 검사\n'
...
printf '\n==> 6/8 livez/stable-slots 응답 검증\n'
OK=0
for i in $(seq 1 40); do
  if curl -fsS --max-time 4 "http://127.0.0.1:${HOST_PORT}/api/livez" >/tmp/nova_livez.json 2>/dev/null && \
     curl -fsS --max-time 4 "http://127.0.0.1:${HOST_PORT}/api/clean-rebuild" >/tmp/nova_stable_slots.json 2>/dev/null; then
    OK=1; break
  fi
  sleep 3
done
...
python3 - <<'PY'
import json
j=json.load(open('/tmp/nova_stable_slots.json'))
assert j.get('ok') is True, j
assert j.get('version')=='NOVA-4.3.0-STABLE-SLOTS1', j.get('version')
assert j.get('engine')=='ACTION_FUNNEL_EXCLUSIVE_STAGE_AFTER_CLOSE_STABLE_SLOTS', j.get('engine')
rules=j.get('rules') or {}
assert rules.get('exclusive_symbol_per_action_board') is True, rules
assert rules.get('fixed_ui_slots')==10, rules
assert rules.get('slot_height_stable') is True, rules
...
if j.get('close_mode'):
    ...
    print('STABLE_SLOTS_API=PASS source='+str(j.get('source'))+' boards='+str({k:len(v or []) for k,v in cb.items()}))
else:
    ...
    print('STABLE_SLOTS_API=PASS source='+str(j.get('source'))+' action_codes='+str(len(codes)))
PY
...
STABLE="$(sudo docker inspect -f '{{index .Config.Labels "io.quantnova.stable_slots"}}' "$CONTAINER")"
SLOTS="$(sudo docker inspect -f '{{index .Config.Labels "io.quantnova.fixed_ui_slots"}}' "$CONTAINER")"
[[ "$LABEL" == "NOVA-4.3.0-STABLE-SLOTS1" ]]
...
echo "화면 안정화: 30초 순위확정 + 60초 재확인 유예 + 활성 보드 10개 고정슬롯"
echo "장후 NXT: 강한 증거는 조건부 비중, 약한 보존증거는 재검증/0%"
