...
VERSION="3.0.0"
REV="MASTER-GROUNDUP-8.1-PULL-ONLY"
...
DEPLOY_DIR="$APP_DIR/deployments/nova3000-master-groundup8-1-$STAMP"
BACKUP="quant-nova-backup-master-groundup8-1-$STAMP"
CANDIDATE="quant-nova-candidate-master-groundup8-1-$STAMP"
...
rollback(){
  ...
  echo "[AUTO-ROLLBACK] G8.1 미완료 — 새 컨테이너 제거 및 기존 immutable 상태 복원" >&2
  ...
  if [[ $ACTIVE_DATA_MUTATED -eq 1 && -d "$BASE_DATA/nova30" ]]; then
    ...
    echo "[AUTO-ROLLBACK] G8.1 NOVA30 state restored; failed attempt preserved at $FAILED_DATA" >&2
  fi
...
rm -rf "$TMP"
trap - EXIT
echo "=== QUANT NOVA 3.0.0 MASTER GROUND-UP 8.1 PULL-ONLY DEPLOY PASS ==="
