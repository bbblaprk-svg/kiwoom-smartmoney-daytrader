...
ZIP_NAME="kiwoom-unified-edge-v3.3.1-NEWS156-REALTIME-FOUNDATION-CLOSE-FRESHNESS.zip"
SOURCE_DIR_NAME="kiwoom-unified-edge-v3.3.1"
EXPECTED_ZIP_SHA256="4f1df67ff38a64336479653ac1a0f803a2b8018367bdb1c329c627bf16ee2653"
OLD_APP_DIR="$HOME/kiwoom-smartmoney-daytrader"
NEW_APP_DIR="$HOME/kiwoom-unified-edge"
TRADER_IMAGE="kiwoom-unified:v3.3.1"
...
rollback(){
  local ec=$?
  [[ $ec -eq 0 ]] && return
  printf '\n[자동복구] v3.3.1 CLOSE FRESHNESS 적용 실패 — 기존 컨테이너로 복구합니다.\n' >&2
...
say "v3.3.1 REALTIME FOUNDATION + CLOSE FRESHNESS + News 1.5.6 파일 받기"
...
[[ "$TRADER_VERSION" == "3.3.1" ]] || fail "trader 버전 불일치: $TRADER_VERSION"
...
[[ "$RUNNING_VERSION" == "3.3.1" ]] || fail "실행 버전 불일치: $RUNNING_VERSION"
