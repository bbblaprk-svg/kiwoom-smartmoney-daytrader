...
ZIP_NAME="kiwoom-unified-edge-v3.3.7-NEWS156-NXT-DUAL-REG-0B-FID9081.zip"
SOURCE_DIR_NAME="kiwoom-unified-edge-v3.3.7"
EXPECTED_ZIP_SHA256="a1a39d93eda6d1eaa3aec7468bbe53a4f7f3ee1ac59405c39de28c2a3b766097"
OLD_APP_DIR="$HOME/kiwoom-smartmoney-daytrader"
NEW_APP_DIR="$HOME/kiwoom-unified-edge"
TRADER_IMAGE="kiwoom-unified:v3.3.7"
...
rollback(){
  ...
  printf '\n[자동복구] v3.3.7 NXT DUAL REG 검증 실패 — 기존 컨테이너로 복구합니다.\n' >&2
...
[[ "$TRADER_VERSION" == "3.3.7" ]] || fail "trader 버전 불일치: $TRADER_VERSION"
...
grep -Fq "KRX_BARE_PLUS_NXT_SUFFIX_0B_V337" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "v3.3.7 REG mode 누락"
...
[[ "$RUNNING_VERSION" == "3.3.7" ]] || fail "실행 버전 불일치: $RUNNING_VERSION"
...
if [[ $RUNTIME_OK -ne 1 ]]; then
  echo "=== V3.3.7 FAILURE DIAGNOSTIC ==="
  ...
  fail "NXT _NX REG 또는 실제 FID9081=NXT 체결 검증 실패 — v3.3.6으로 자동 롤백합니다."
fi
