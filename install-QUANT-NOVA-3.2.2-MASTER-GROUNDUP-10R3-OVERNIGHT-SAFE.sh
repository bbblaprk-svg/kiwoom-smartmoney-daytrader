...
IMAGE="ghcr.io/bbblaprk-svg/kiwoom-smartmoney-daytrader:nova-3.2.2-master-groundup-10r3-overnight-safe"
SOURCE_SHA="e58b231d101a181de4f3d716dacd7f6b3e02866d83222fc75f0450bde43227d0"
RELEASE_TAG="nova-3.2.2-master-groundup-10r3-overnight-safe"
ASSET="quant-nova-3.2.2-master-groundup-10r3-overnight-safe-image.tar.gz"
...
[[ "$(sudo docker image inspect -f '{{index .Config.Labels "io.quantnova.deploy_model"}}' "$IMAGE")" == "PUBLIC_RELEASE_ASSET" ]] || die "deploy model mismatch"
...
sudo docker run --rm --entrypoint sh "$IMAGE" -lc "grep -q EOD_FROZEN_LAST_ACTIVE /app/app/runtime/display.py && grep -q EOD_FROZEN_LAST_TRADE /app/app/runtime/projection.py && grep -q 's >= 20\*3600 or s < 8\*3600' /app/app/runtime/clock.py" || die "overnight source contract missing"
echo "IMAGE_IDENTITY=PASS DELIVERY=PUBLIC_RELEASE_ASSET OVERNIGHT_CONTRACT=PASS"
...
from app.runtime.clock import sessions,lifecycle_phase,sessions_at,KST
from datetime import datetime
if not sessions()['active']:
    j=json.load(urllib.request.urlopen(B+'/api/live-dashboard',timeout=3))
    assert lifecycle_phase()=='PRIOR_CLOSE_FROZEN',lifecycle_phase()
    assert j.get('phase')=='PRIOR_CLOSE_FROZEN',j.get('phase')
    assert (j.get('delivery') or {}).get('market_active') is False,j.get('delivery')
    for h in j.get('signal_history') or []:
        if h.get('state') in ('TOP5_ENTER','TOP5_REENTER','TOP5_EXIT'):
            d=datetime.fromtimestamp(float(h.get('at') or 0),KST);sec=d.hour*3600+d.minute*60+d.second
            assert sessions_at(d.date(),sec).get('active'),h
    before=[(x.get('at'),x.get('state'),x.get('code')) for x in j.get('signal_history') or []]
    time.sleep(1.3)
    j2=json.load(urllib.request.urlopen(B+'/api/live-dashboard',timeout=3))
    after=[(x.get('at'),x.get('state'),x.get('code')) for x in j2.get('signal_history') or []]
    assert before==after,(before[:3],after[:3])
    print('OVERNIGHT_FROZEN_DISPLAY=PASS history_stable',len(after))
PY
...
rm -rf "$TMP"
echo
echo "===== QUANT NOVA 3.2.2 G10R3 OVERNIGHT-SAFE DEPLOY PASS ====="
