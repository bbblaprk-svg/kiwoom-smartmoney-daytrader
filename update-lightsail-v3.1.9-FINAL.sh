...
ZIP_NAME="kiwoom-smartmoney-daytrader-v3.1.9-FINAL.zip"
SOURCE_DIR_NAME="kiwoom-smartmoney-daytrader-v3.1.9"
IMAGE_NAME="kiwoom-smartmoney:v3.1.9"
...
rollback() {
  ...
  printf '\n[자동 복구] v3.1.9 FINAL 적용 실패 — 기존 앱으로 되돌립니다.\n' >&2
...
say "v3.1.9 FINAL 파일 받기"
git clone --depth 1 "$REPO_URL" "$TMP_ROOT/repo"
cd "$TMP_ROOT/repo"
[[ -f "$ZIP_NAME" ]] || fail "GitHub 최상위에 $ZIP_NAME 파일이 없습니다."
EXPECTED_ZIP_SHA256="1376dcbdd0a1b616f65dccd7c665f9cb79ee28a50d2cbd69644b53cd7996531e"
...
COPY kiwoom-smartmoney-daytrader-v3.1.9-FINAL.zip /tmp/app.zip
RUN mkdir -p /tmp/source \
    && unzip -q /tmp/app.zip -d /tmp/source \
    && cp -a /tmp/source/kiwoom-smartmoney-daytrader-v3.1.9/. /app/ \
...
say "v3.1.9 FINAL Docker 이미지 빌드 — 검사·부하검사·Next 빌드 포함"
...
printf '\nSafari 탭을 완전히 닫았다가 다시 열고 v3.1.9, 장전 사전점검, RISK/EARLY_SELL, 실제체결시장, 아이폰 푸시 테스트를 확인하세요.\n'
