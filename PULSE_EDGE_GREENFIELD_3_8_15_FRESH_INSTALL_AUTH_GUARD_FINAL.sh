...
RELEASE="3.8.15_NAME_FIRST_DISPLAY"
...
SRC_NAME="PULSE_EDGE_GREENFIELD_3_8_15_NAME_FIRST_DISPLAY_SOURCE.tar.gz"
EXPECTED_SRC_SHA="0ffcd22fa2e71631950d996e435bcba96c365cb2a6934e661ff194c05e97eafe"
EXPECTED_DF_SHA="92a1c5900851811ec7c72583696deabca7b75de94b9973d21490e7ca478a595e"
MODE="${1:-deploy}"
SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)"
RUN_ID="$(date +%Y%m%d-%H%M%S)-$$"
RUN_DIR="$BASE/deployments/$RUN_ID"
CANARY_DATA="$RUN_DIR/canary-data"
IMG="pulse-edge:gf3815-name-$RUN_ID"
CANARY="pulse-edge-gf3815-canary-$RUN_ID"
LIVE="pulse-edge-gf3815-live-$RUN_ID"
OLD=""
OLD_RUNNING=false
OLD_RESTART="no"
CUTOVER=0
...
raw=src.read_bytes()
# Preserve selected source env lines byte-for-byte. In particular, Kiwoom
# credentials are never decoded/re-serialized, so their value bytes cannot be
# altered by trimming, quoting or normalization.
keep={
 b'PULSE_KIWOOM_APPKEY',b'PULSE_KIWOOM_SECRETKEY',b'PULSE_KIWOOM_REST_BASE',b'PULSE_KIWOOM_WS_URL',
 b'PULSE_VAPID_PUBLIC_KEY',b'PULSE_VAPID_PRIVATE_KEY',b'PULSE_VAPID_SUBJECT',b'PULSE_NEWS_WEBHOOK_SECRET',b'PULSE_KRX_CLOSED_DATES'
}
selected=[]; values={}
for physical in raw.splitlines(keepends=True):
    core=physical.rstrip(b'\r\n')
    ...
    if key in keep and key not in values:
        # Preserve exact physical line bytes from trusted credential source.
        selected.append((key, physical if physical.endswith((b'\n',b'\r')) else physical+b'\n'))
        values[key]=val
for req in (b'PULSE_KIWOOM_APPKEY',b'PULSE_KIWOOM_SECRETKEY'):
    if req not in values or not values[req].strip():
        raise SystemExit('Kiwoom credentials missing')
fixed={
'PULSE_RELEASE':'3.8.15_NAME_FIRST_DISPLAY','PULSE_FEED_ENABLED':'true','PULSE_DATA_DIR':'/app/data/pulse_edge',
...
# Only non-secret runtime settings are serialized here.
out=b''.join(line for _,line in selected)
# Supply VAPID subject only if absent/localhost, without touching Kiwoom creds.
sub=values.get(b'PULSE_VAPID_SUBJECT',b'').strip()
if not sub or b'localhost' in sub:
    out=b''.join(line for k,line in selected if k!=b'PULSE_VAPID_SUBJECT')
    out+=('PULSE_VAPID_SUBJECT='+pub+'\n').encode()
out+=''.join(f'{k}={v}\n' for k,v in fixed.items()).encode()
tmp=dst.with_name(dst.name+'.tmp-'+str(os.getpid()))
tmp.write_bytes(out); tmp.chmod(0o600); os.replace(tmp,dst)
# Prove Kiwoom credential lines copied byte-for-byte without revealing value/hash.
def exact_line(blob,key):
    for physical in blob.splitlines(keepends=True):
        core=physical.rstrip(b'\r\n'); stripped=core.lstrip()
        if b'=' in stripped and stripped.split(b'=',1)[0].strip()==key:
            return physical if physical.endswith((b'\n',b'\r')) else physical+b'\n'
    return None
newraw=dst.read_bytes()
for key in (b'PULSE_KIWOOM_APPKEY',b'PULSE_KIWOOM_SECRETKEY'):
    if exact_line(raw,key)!=exact_line(newraw,key):
        raise SystemExit('credential byte preservation failed')
print('ENV_PASS')
print('KIWOOM_CREDENTIAL_COPY_EXACT=PASS')
PY
...
docker run --rm -i --network "$NET" --env-file "$ENV_FILE" "$IMG" python - <<'PYKIWOOM'
import asyncio, sys
import httpx
...
import json,sys
q=json.load(open(sys.argv[1])); assert q['release']=='3.8.15_NAME_FIRST_DISPLAY' and q['ready'] is True and q['feed_enabled'] is False
PY
  fi
...
import json,sys
q=json.load(open(sys.argv[1])); assert q.get('release')=='3.8.15_NAME_FIRST_DISPLAY' and isinstance(q.get('rows'),list),q
...
import json,sys
h=json.load(open(sys.argv[1])); assert h.get('runtime_safety_release')=='3.8.15_NAME_FIRST_DISPLAY'
PY
public=0
