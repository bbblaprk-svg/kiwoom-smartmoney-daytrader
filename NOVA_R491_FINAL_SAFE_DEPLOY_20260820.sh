...
EXPECTED_DOCKERFILE_SHA="ac10ac92fed28288b0bf1c720b4e6e386348180b20513ba4873451de4377d219"
EXPECTED_VERSION="NOVA-3.3.4-MARKET-WIDE-ROTATION-VERIFY"
IMAGE="quant-nova:3.3.4-r491-final-local"
STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP="${APP}-pre-r491-${STAMP}"
FAILED="${APP}-failed-r491-${STAMP}"
CANDIDATE="${APP}-candidate-r491-${STAMP}"
WORK="$(mktemp -d /tmp/nova-r491-final.XXXXXX)"
DOCKERFILE="$WORK/Dockerfile"
ENVFILE="$WORK/current.env"
CAND_DATA="$WORK/candidate-data"
LOG="/tmp/nova-r491-final-deploy-${STAMP}.log"
OLD_WAS_RUNNING=0
OLD_STOPPED=0
OLD_RENAMED=0
NEW_STARTED=0
...
recover_previous_if_needed(){
  ...
  latest="$(dc ps -a --format '{{.Names}}' | grep -E "^${APP}-pre-r(491|49|48|47)-[0-9]{8}-[0-9]{6}$" | sort | tail -n 1 || true)"
  if [ -n "$latest" ]; then
...
fetch_dockerfile(){
  local remote="QUANT_NOVA_3.3.4_R491_PRE_SIGNAL_LIVE_EVIDENCE_FINAL_20260820.Dockerfile"
...
say "1/8 FETCH R491 FINAL DOCKERFILE + SHA LOCK"
fetch_dockerfile
...
[ "$(dc image inspect "$IMAGE" --format '{{index .Config.Labels "io.quantnova.ui_patch"}}')" = "R491_PRE_SIGNAL_LIVE_EVIDENCE_10M" ]
...
print(json.dumps({'ok':True,'version':expected,'verify':'PASS','rotation':'PASS','signal_accel':'TOP20_PLUS_LIVE_RANK30_DWELL60_GAP3','live_evidence_episode_sec':600,'entry_target':'DISPLAY_ONLY','swap_mb':0},ensure_ascii=False))
PY
verify_protected
