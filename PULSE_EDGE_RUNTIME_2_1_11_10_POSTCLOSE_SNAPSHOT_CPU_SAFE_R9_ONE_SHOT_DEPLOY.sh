#!/usr/bin/env bash
set -Eeuo pipefail
# PULSE EDGE 2.1.11.10 R9 — deterministic route-owner recovery + CPU-safe scout-funnel deployment.
# R9 preserves the last valid KRX/NXT operator snapshots across CLOSED and restarts; deployment keeps R8 deterministic route-owner recovery.
...
EXPECTED_DF_SHA=3dec3f959f5264cbb917542416683b48b11144342ac0b99b822fa6b6258d3e40
EXPECTED_SRC_SHA=bfc3a229f18b884292287bf0c0d297044f351915afc8f847af84bba001db35c5
SRC_NAME=PULSE_EDGE_CORE_2_1_3_RUNTIME_2_1_11_10_POSTCLOSE_SNAPSHOT_CPU_SAFE_R9_SOURCE.tar.gz
    ...
    for root in roots:
        for p in sorted(root.glob(pattern)):
            if p.is_file() and hashlib.sha256(p.read_bytes()).hexdigest()==expected:
                found.append(p)
    if not found: raise SystemExit(f'{label}: matching 2.1.11.10 R9 file missing or modified; put all 3 files together')
...
IMG="pulse-edge:runtime-2.1.11.10-$RUN_ID"
CANARY="pulse-edge-canary-runtime-2.1.11.10-$RUN_ID"
LIVE="pulse-edge-live-runtime-2.1.11.10-$RUN_ID"
save_state
