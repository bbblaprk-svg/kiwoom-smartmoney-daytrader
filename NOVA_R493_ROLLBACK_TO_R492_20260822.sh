...
LOG="/tmp/nova-r493-to-r492-rollback-${STAMP}.log"
...
while IFS= read -r cand; do
  ...
  if [ "$ver" = "$EXPECTED_R492_VERSION" ]; then BACKUP="$cand"; break; fi
done < <(dc ps -a --format '{{.Names}}' | grep -E "^${APP}-pre-r493-[0-9]{8}-[0-9]{6}$" | sort -r)
[ -n "$BACKUP" ] || { echo "FAIL: pre-r493 백업 중 승인 R492를 찾지 못했습니다. 현재 앱은 변경하지 않습니다."; exit 1; }
echo "ROLLBACK_SOURCE=$BACKUP EXPECTED_VERSION=$EXPECTED_R492_VERSION" | tee -a "$LOG"
...
if ! wait_health "$APP"; then
  echo "FAIL: R492 health check 실패. 현재 복구 컨테이너=$APP, R493 보존=$FAILED" | tee -a "$LOG"
  exit 1
fi
...
PY
echo "RESULT=R492_RESTORED CURRENT=$APP FAILED_R493=$FAILED LOG=$LOG" | tee -a "$LOG"
