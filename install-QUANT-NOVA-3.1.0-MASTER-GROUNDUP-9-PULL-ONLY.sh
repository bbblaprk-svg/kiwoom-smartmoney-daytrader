...
VERSION="3.1.0"
REV="MASTER-GROUNDUP-9-PULL-ONLY"
APP_VERSION="NOVA-3.1.0-MASTER-GROUNDUP-9"
IMAGE="ghcr.io/bbblaprk-svg/kiwoom-smartmoney-daytrader:nova-3.1.0-master-groundup-9"
EXPECTED_SOURCE_ZIP_SHA256="1f3161d4e9e12614c19dd164cc8969095de4dd38c564ca08e40862748f87a568"
...
DEPLOY_DIR="$APP_DIR/deployments/nova3100-master-groundup9-$STAMP"
BACKUP="quant-nova-backup-master-groundup9-$STAMP"
CANDIDATE="quant-nova-candidate-master-groundup9-$STAMP"
...
sudo docker exec "$CANDIDATE" python -c "import json,urllib.request;j=json.load(urllib.request.urlopen('http://127.0.0.1:8000/api/readyz',timeout=2));assert j.get('ok') and j.get('version')=='NOVA-3.1.0-MASTER-GROUNDUP-9',j;print('CANDIDATE_READYZ=PASS',j)"
...
curl -fsS --max-time 3 "http://127.0.0.1:${HOST_PORT}/api/livez" | python3 -c 'import json,sys;j=json.load(sys.stdin);assert j.get("ok") and j.get("version")=="NOVA-3.1.0-MASTER-GROUNDUP-9",j;print("ACTIVE_LIVEZ=PASS",j)'
...
sudo docker exec "$CONTAINER" python -c "import json,urllib.request;j=json.load(urllib.request.urlopen('http://127.0.0.1:8000/api/readyz',timeout=2));assert j.get('ok') and j.get('version')=='NOVA-3.1.0-MASTER-GROUNDUP-9',j;print('ACTIVE_READYZ=PASS',j)"
...
if [[ -d "$GUARD_STATIC" ]] && sudo docker inspect nova-http-guard >/dev/null 2>&1; then
  mkdir -p "$GUARD_BK" "$TMP/g9-static"
  ...
  [[ "$CLEAN_GUARD_SHA" == "3d075f640773a30fcde2598f90402c3e3ac1b079757109d10cfce3375da7fd71" ]] || die "clean guard SHA mismatch: $CLEAN_GUARD_SHA"
  ...
  for f in index.html nova.js nova.css manifest.webmanifest sw.js; do
    sudo docker cp "$CONTAINER:/app/static/$f" "$TMP/g9-static/$f"
  done
  grep -Fq "h['Authorization']='Bearer '+token" "$TMP/g9-static/nova.js" || die "G9 source-level Authorization UI 없음"
  grep -Fq "startLivePoll" "$TMP/g9-static/nova.js" || die "G9 permanent UI poll 없음"
  ! grep -Eq '__NOVA_INLINE_AUTH_V3__|__NOVA_ALWAYS_ON_POLL_R7__|NOVA_R6|NOVA_R7' "$TMP/g9-static/nova.js" || die "debug JS lineage 발견"
  ...
  for f in index.html nova.js nova.css manifest.webmanifest sw.js; do
    sudo cp "$TMP/g9-static/$f" "$GUARD_STATIC/.$f.nova31.tmp"
    sudo chmod 644 "$GUARD_STATIC/.$f.nova31.tmp"
    sudo mv "$GUARD_STATIC/.$f.nova31.tmp" "$GUARD_STATIC/$f"
  done
  sudo rm -f "$GUARD_CACHE"/* >/dev/null 2>&1 || true
...
PYGUARD
  ...
  curl -ksS --max-time 5 "https://3-38-25-20.nip.io/static/nova.js?g9=$STAMP" -o "$PUBLIC_JS"
...
rm -rf "$TMP"
trap - EXIT
echo "=== QUANT NOVA 3.1.0 MASTER GROUND-UP 9 PULL-ONLY DEPLOY PASS ==="
