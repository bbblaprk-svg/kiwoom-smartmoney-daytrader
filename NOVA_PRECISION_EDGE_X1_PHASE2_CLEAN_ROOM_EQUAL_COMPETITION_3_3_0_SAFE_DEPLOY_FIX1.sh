#!/usr/bin/env bash
# NOVA PRECISION EDGE X1 PHASE2 · CLEAN ROOM EQUAL COMPETITION · 3.3.0 · CURRENT-DAY ONLY
# Safe build/canary/cutover with automatic rollback. No runtime source patching.
set -Eeuo pipefail

PRODUCT='NOVA PRECISION EDGE X1 PHASE2 · CLEAN ROOM EQUAL COMPETITION · 3.3.0'
VERSION='NOVA-PRECISION-EDGE-X1-PHASE2-CLEAN-ROOM-EQUAL-COMPETITION-3.3.0'
CURRENT_CONTAINER="${NOVA_CURRENT_CONTAINER:-quant-nova}"
DOCKERFILE_NAME='NOVA_PRECISION_EDGE_X1_PHASE2_CLEAN_ROOM_EQUAL_COMPETITION_3_3_0_FIX1.Dockerfile'
EXPECTED_DOCKERFILE_SHA256='10ed815235edf788537cd456efb262cd4c5c6ea971268b967c6d31625991ffd9'
...
IMAGE_TAG="quant-nova:clean-room-equal-3-3-0-${STAMP}"
BACKUP_CONTAINER="quant-nova-backup-before-clean-room-equal-3-3-0-${STAMP}"
CANARY_CONTAINER="quant-nova-canary-clean-room-equal-3-3-0-${STAMP}"
BUILD_CONTEXT=''
CUTOVER_STARTED=0
...
BUILD_CONTEXT="$(mktemp -d -t nova-clean-room-330-build.XXXXXX)"
...
[[ "$(label io.quantnova.clean_room)" == 'TRUE' ]] || fail 'clean room contract missing'
[[ "$(label io.quantnova.live_candidate_boot)" == 'EMPTY_EACH_PROCESS_AND_TRADING_DAY' ]] || fail 'live candidate boot must be empty on process/day start'
[[ "$(label io.quantnova.live_candidate_source)" == 'CURRENT_DAY_BROKER_RANK_SCANNERS_ONLY' ]] || fail 'live source must be current broker rank scanners only'
[[ "$(label io.quantnova.prior_close_to_candidate)" == 'ZERO' ]] || fail 'prior close candidate materialization must be zero'
[[ "$(label io.quantnova.history_to_candidate)" == 'ZERO' ]] || fail 'historical signal candidate materialization must be zero'
[[ "$(label io.quantnova.signal_reservation_membership_privilege)" == 'ZERO' ]] || fail 'signal reservation membership privilege must be zero'
[[ "$(label io.quantnova.snapshot_to_live_membership)" == 'ZERO' ]] || fail 'display snapshot backfill must be zero'
[[ "$(label io.quantnova.retired_pin_followup_state)" == 'REMOVED' ]] || fail 'legacy pin/followup state must be removed'
[[ "$(label io.quantnova.sparse_board_policy)" == 'SIGNAL_WAIT_NOT_OLD_SYMBOL' ]] || fail 'sparse board must remain SIGNAL WAIT'
[[ "$(label io.quantnova.rank_delivery)" == 'CURRENT_EVIDENCE_EQUAL_COMPETITION_FIXED10_30S_COMMIT' ]] || fail 'equal competition rank contract mismatch'
[[ "$(label io.quantnova.candidate_factory)" == 'UPSERT_RANK_HIT_ONLY_TWO_CALLSITES' ]] || fail 'candidate factory boundary mismatch'
[[ "$(label io.quantnova.unknown_ws_to_candidate)" == 'ZERO' ]] || fail 'unknown WS must not materialize candidates'
[[ "$(label io.quantnova.history_to_candidate)" == 'ZERO' ]] || fail 'history must not materialize candidates'
[[ "$(label io.quantnova.performance_to_candidate)" == 'ZERO' ]] || fail 'performance must not materialize candidates'
[[ "$(label io.quantnova.buy_signal_membership_privilege)" == 'ZERO' ]] || fail 'BUY signal must have zero membership privilege'
[[ "$(label io.quantnova.performance_scope)" == 'HISTORY_ONLY_NO_DISCOVERY_WS' ]] || fail 'performance separation mismatch'
...
assert edge.get('version')=='NOVA-PRECISION-EDGE-X1-PHASE2-CLEAN-ROOM-EQUAL-COMPETITION-3.3.0', edge.get('version')
...
say "LIVE_CANDIDATE_SOURCE=CURRENT_DAY_BROKER_RANK_SCANNERS_ONLY"
say "PRIOR_CLOSE_CANDIDATE_MATERIALIZATION=ZERO"
say "HISTORICAL_SIGNAL_CANDIDATE_MATERIALIZATION=ZERO"
say "FORMAL_SIGNAL_PIN_PRIVILEGE=ZERO"
say "SNAPSHOT_TO_LIVE_MEMBERSHIP=ZERO"
say "CORE_FRESH_ANCHOR_RESERVATION=ZERO"
say "SPARSE_BOARD=SIGNAL_WAIT_NOT_OLD_SYMBOL"
say "CANDIDATE_FACTORY=UPSERT_RANK_HIT_ONLY_TWO_CALLSITES"
...
say "RETIRED_PIN_FOLLOWUP_STATE=REMOVED"
