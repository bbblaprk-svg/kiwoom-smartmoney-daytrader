...
LOG="/tmp/nova-r493-full-to-r491-${STAMP}.log"
...
if [ "$CURRENT" = "$EXPECTED" ]; then wait_health "$APP"; echo "RESULT=ALREADY_R491 CURRENT=$APP VERSION=$CURRENT"; exit 0; fi
BACKUP=""
...
[ -n "$BACKUP" ] || { echo "FAIL: 승인 R491 백업을 찾지 못했습니다. 현재 앱은 변경하지 않습니다."; exit 1; }
