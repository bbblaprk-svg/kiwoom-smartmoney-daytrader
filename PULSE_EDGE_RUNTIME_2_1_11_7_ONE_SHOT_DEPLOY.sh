#!/usr/bin/env bash
set -Eeuo pipefail
# PULSE EDGE 2.1.11.7 — deployment repair. Run with bash, never source this file.
...
RELEASE=2.1.11.7_CPU_GUARD
EXPECTED_DF_SHA=1a4c2911220467dea525abc84a59808423ddeac8bfbd2d6e0b750fa312dbb9ec
EXPECTED_SRC_SHA=b0bef7bc8165d5ba1577f4df62344986e900b8fcd471ca3fac0b71e9426eae95
SRC_NAME=PULSE_EDGE_CORE_2_1_3_RUNTIME_2_1_11_7_CPU_GUARD_SOURCE.tar.gz
    ...
    for root in roots:
        for p in sorted(root.glob(pattern)):
            if p.is_file() and hashlib.sha256(p.read_bytes()).hexdigest()==expected:
                found.append(p)
    if not found: raise SystemExit(f'{label}: matching 2.1.11.7 file missing or modified; put all 3 files together')
...
IMG="pulse-edge:runtime-2.1.11.7-$RUN_ID"
CANARY="pulse-edge-canary-runtime-2.1.11.7-$RUN_ID"
LIVE="pulse-edge-live-runtime-2.1.11.7-$RUN_ID"
save_state
