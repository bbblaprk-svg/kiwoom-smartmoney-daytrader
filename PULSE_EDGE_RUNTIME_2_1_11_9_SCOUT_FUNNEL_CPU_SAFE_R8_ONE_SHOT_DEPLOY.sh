#!/usr/bin/env bash
set -Eeuo pipefail
# PULSE EDGE 2.1.11.9 R8 — deterministic route-owner recovery + CPU-safe scout-funnel deployment.
# R8 adds an explicit healthy preferred-baseline arbitration path for legacy failed 2.1.11.9 live containers that Docker may resurrect without usable run metadata.
...
EXPECTED_DF_SHA=6559cb4da652834206cf41a7bcf6ecda266c20d66360b104df5b8bbead95faeb
EXPECTED_SRC_SHA=12a3065c7ed866bd51d3b3c254fb030d962e02064eb227945e40151c87c1849c
SRC_NAME=PULSE_EDGE_CORE_2_1_3_RUNTIME_2_1_11_9_SCOUT_FUNNEL_CPU_SAFE_R8_SOURCE.tar.gz
...
reconcile_route_holders(){
  ...
  while IFS= read -r c; do
    ...
    # Any managed 2.1.11.9 live name with a known run directory but no deploy-passed marker is objectively a failed/orphan run.
    # Do not depend on labels: early R2/R3 containers may not carry the newer project/role labels, yet Docker can resurrect them.
    if [[ "$c" = pulse-edge-live-runtime-2.1.11.9-* ]] && [ -n "$rid" ] && [ -d "$BASE/deployments/$rid" ] && [ ! -f "$BASE/deployments/$rid/deploy-passed" ]; then
  ...
  # Second pass: deterministic arbitration for historical failed 2.1.11.9 lives whose
  ...
  if [ "${#holders_now[@]}" -gt 1 ]; then
    ...
    if printf '%s\n' "${holders_now[@]}" | grep -Fxq "$preferred"; then
      ...
      for c in "${holders_now[@]}"; do
        ...
        case "$c" in
          pulse-edge-live-runtime-2.1.11.9-*|pulse-edge-canary-runtime-2.1.11.9-*)
