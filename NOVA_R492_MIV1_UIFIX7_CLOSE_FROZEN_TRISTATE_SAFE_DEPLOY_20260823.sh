...
EXPECTED_DOCKERFILE_SHA="8a0848b81d3cfc3f7e7d5a8b43a734a797f08ab4c3d5f5b87672a2033e9dbee5"
EXPECTED_EMBEDDED_SOURCE_SHA="7dd2687364cbe514cbf08ab4ca1734e58bf4bc35b07e6364897183110011c898"
EXPECTED_VERSION="NOVA-3.3.5-R492-MARKET-INDEX-VERIFY1"
BASELINE_UIFIX6_LABEL="FULL_EDGE_REACCEL_ALERTS_CLOSED_LOOP_VERIFY"
EXPECTED_UIFIX7_LABEL="CLOSE_FROZEN_PERSIST_TRISTATE_DISPLAY_ONLY"
DEPLOY_SCRIPT_REV="UIFIX7_V1_CLOSE_FROZEN_TRISTATE_SAFE"
BASELINE_VERSION="NOVA-3.3.5-R492-MARKET-INDEX-VERIFY1"
IMAGE="quant-nova:3.3.5-r492-miv1-uifix7-close-frozen-local"
STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP="${APP}-pre-uifix7-${STAMP}"
CANDIDATE="${APP}-candidate-uifix7-${STAMP}"
WORK="$(mktemp -d /tmp/nova-r492-miv1-uifix7-close-frozen.XXXXXX)"
DOCKERFILE="$WORK/Dockerfile"
ENVFILE="$WORK/current.env"
CAND_DATA="$WORK/candidate-data"
LOG="/tmp/nova-r492-miv1-uifix7-close-frozen-deploy-${STAMP}.log"
LOCKFILE="/tmp/nova-r492-miv1-uifix7-close-frozen-deploy.lock"
OLD_WAS_RUNNING=0
OLD_STOPPED=0
OLD_RENAMED=0
NEW_STARTED=0
...
exec 9>"$LOCKFILE"
if ! flock -n 9; then
  echo "INFO: R492 MIV1 UIFIX7 CLOSE FROZEN deploy is already running. Do not start it twice."
  exit 0
fi
...
'static/index.html':'5808cfdfae0caf0d5a34aaa5f8d38f832246bd94f4ef5f84e47c9fc12f94bc1f',
...
'app/addons/decision_panels.py':'b0e85f40bba0e057ee23b8e2e61dc90417170d880f0cbcaccc0bd487a17e7703',
...
'static/sw.js':'53dc9d3a9e47a26884a7799733b3aab57c78809277611a453cb59d0757945442',
'scripts/r492_uifix7_acceptance.py':'551fbb219b8d70df96124427c618993bf8e67e8f32cc9cceb5da01cad084a017',
'R492_UIFIX7_INVARIANTS.md':'c877eb7242940eee14ee11124f4e2a35c3f3e6fa6a063c846ad8bed1ff1627aa',
}
...
print(f'PREFLIGHT_UIFIX7=PASS chunks={len(parts)} gzip_bytes={len(raw)} gzip_sha256={actual} manifest=PASS frozen_core=7 main_dom=UNCHANGED close_persist=1 tristate=1 price_path_unchanged=1 new_rest_type=0 new_ws_type=0')
PY
...
say "4A/9 UNIT TESTS — TEST PROCESS ONLY, CANDIDATE SERVER REMAINS ISOLATED"
echo "UNIT_TEST_ENV=NOVA_CANDIDATE_MODE=0 NOVA_OFFLINE=0 NOVA_MARKET_INDEX_VERIFY_ENABLED=1 NETWORK=container:none" | tee -a "$LOG"
dc exec "$CANDIDATE" env PYTHONPATH=/app NOVA_CANDIDATE_MODE=0 NOVA_OFFLINE=0 NOVA_MARKET_INDEX_VERIFY_ENABLED=1 python -m unittest discover -s tests -p 'test_*.py' -q | tee -a "$LOG"
...
assert 'largecap_close_snapshot_persisted' in dp and 'FROZEN_LAST_LIVE' in djs
print('UIFIX7_CANDIDATE=PASS close_persist=1 tristate=1 reaccel_fake_fill=0 price_path_unchanged=1')
PY
...
dpc=dp.get('contracts') or {}; assert dpc.get('largecap_close_snapshot_persisted') is True and dpc.get('largecap_empty_state_explicit') is True and dpc.get('price_source_or_scale_changed') is False,dpc
