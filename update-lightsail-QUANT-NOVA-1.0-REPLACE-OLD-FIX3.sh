...
# Caddyfile은 컨테이너 내부에 read-only bind/volume mount일 수 있으므로
# 컨테이너 안에서 덮어쓰지 않고 실제 HOST mount source를 찾아 원본을 교체한다.
CADDY_DEST="/etc/caddy/Caddyfile"
CADDY_MOUNT_TYPE="$(sudo docker inspect "$CADDY" --format '{{range .Mounts}}{{if eq .Destination "/etc/caddy/Caddyfile"}}{{.Type}}{{end}}{{end}}')"
CADDY_MOUNT_SOURCE="$(sudo docker inspect "$CADDY" --format '{{range .Mounts}}{{if eq .Destination "/etc/caddy/Caddyfile"}}{{.Source}}{{end}}{{end}}')"
[[ -n "$CADDY_MOUNT_SOURCE" ]] || fail "Caddyfile host mount source를 찾지 못했습니다. 기존 앱은 삭제하지 않았습니다."

# 기존 설정은 앱 폴더와 원본 옆에 이중 백업
sudo cat "$CADDY_MOUNT_SOURCE" > "$APP_DIR/Caddyfile.pre-nova.$STAMP"
sudo cp "$CADDY_MOUNT_SOURCE" "${CADDY_MOUNT_SOURCE}.pre-nova.${STAMP}"
...
# 실제 host-side mount source 교체. read-only는 컨테이너 관점일 뿐 host에서는 변경 가능하다.
sudo cp "$TMP/Caddyfile" "$CADDY_MOUNT_SOURCE"
sudo docker exec "$CADDY" caddy validate --config "$CADDY_DEST" --adapter caddyfile >/dev/null
sudo docker exec "$CADDY" caddy reload --config "$CADDY_DEST" --adapter caddyfile
...
if [[ $HTTPS_OK -ne 1 ]]; then
  echo "[복구] HTTPS 확인 실패 — Caddy 원본 복원" >&2
  sudo cp "${CADDY_MOUNT_SOURCE}.pre-nova.${STAMP}" "$CADDY_MOUNT_SOURCE" || true
  sudo docker exec "$CADDY" caddy reload --config "$CADDY_DEST" --adapter caddyfile >/dev/null 2>&1 || true
  fail "NOVA HTTPS 전환 확인 실패. 기존 앱은 삭제하지 않았습니다."
fi
