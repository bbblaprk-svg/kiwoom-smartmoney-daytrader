...
static_image_acceptance(){
  ...
  dc run --rm -w /app -e PYTHONPATH=/app --entrypoint python "$IMAGE" /app/scripts/market_rotation_acceptance.py | tee -a "$LOG"
...
dc exec -e PYTHONPATH=/app -w /app "$APP" python /app/scripts/market_rotation_acceptance.py | tee -a "$LOG"
