#!/usr/bin/env bash
# Revision: PHASE2-CHART-FIX9-CLOSE4-3_0_4
set -Eeuo pipefail

PRODUCT='NOVA PRECISION EDGE X1 PHASE2 · CHART FIX9 CLOSE4'
VERSION='NOVA-PRECISION-EDGE-X1-PHASE2-CHART-FIX9-CLOSE4-3.0.4'
CURRENT_CONTAINER="${NOVA_CURRENT_CONTAINER:-quant-nova}"
DOCKERFILE_NAME='QUANT_NOVA_PRECISION_EDGE_X1_PHASE2_CHART_FIX9_CLOSE4.Dockerfile'
EXPECTED_DOCKERFILE_SHA256='ba60447332e0c762b6221ec6741842dcf299909121a037285a1ce9a46af2d167'
...
IMAGE_TAG="quant-nova:precision-edge-x1-phase2-chart-fix9-close4-${STAMP}"
BACKUP_CONTAINER="quant-nova-backup-precision-edge-x1-phase2-chart-fix9-close4-${STAMP}"
CANARY_CONTAINER="quant-nova-canary-precision-edge-x1-phase2-chart-fix9-close4-${STAMP}"
BUILD_CONTEXT=''
CUTOVER_STARTED=0
...
BUILD_CONTEXT="$(mktemp -d -t nova-precision-x1-phase2-chart-fix9-close4-build.XXXXXX)"
...
[[ "$(docker image inspect --format '{{index .Config.Labels "io.quantnova.chart_close_preserve"}}' "${IMAGE_TAG}")" == 'UNTIL_NEXT_TRADING_SESSION_FRESH_COMPLETE20_HANDOFF' ]] ||
...
snapshot={"ok":True,"version":"NOVA-CHART-EDGE-LS1925-FIX9-1.0.0","active":True,"mode":"LIVE_VERIFY","generatedAt":1800000000.0,
...
with open(os.path.join(root,"chart_edge_close.json"),"w",encoding="utf-8") as stream:json.dump(snapshot,stream)
print("LEGACY_FROZEN_FIXTURE=WRITTEN")
'
...
[[ "${CANARY_HEALTH}" == 'healthy' ]] || die "legacy-frozen restart canary health timeout: ${CANARY_HEALTH}"
docker exec "${CANARY_CONTAINER}" python -c '
...
assert edge.get("version")=="NOVA-PRECISION-EDGE-X1-PHASE2-CHART-FIX9-CLOSE4-3.0.4",edge
...
print("PHASE2_CHART_FIX9_CLOSE4_LEGACY_FROZEN_RESTART_CANARY=PASS")
'
...
assert j.get("ok") and j.get("version")=="NOVA-PRECISION-EDGE-X1-PHASE2-CHART-FIX9-CLOSE4-3.0.4",j
...
assert lc.get("maxSignalCycles")==1000 and lc.get("maxStageEvents")==5000,lc
assert lc.get("outcomeWriteFlushSec")==12 and not lc.get("repeatedPacketsCreateEvents"),lc
print("PRECISION_CHART_FIX9_CLOSE4_API_CONTRACT=PASS PHASE="+str(j.get("phase")))
'
...
done < <(docker images 'quant-nova' --format '{{.Repository}}:{{.Tag}}' | awk '/^quant-nova:precision-edge-x1-phase2-chart-fix9-close4-[0-9]{8}-[0-9]{6}$/')
...
say "MANUAL_ROLLBACK=./NOVA_PRECISION_EDGE_X1_PHASE2_CHART_FIX9_CLOSE4_MANUAL_ROLLBACK.sh ${BACKUP_CONTAINER}"
