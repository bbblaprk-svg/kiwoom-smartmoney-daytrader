...
EXPECTED_DOCKERFILE_SHA="0499f4a53ed0e4bd6aecced239ea3288f44af94d49177189d2a4b65926c97d30"
EXPECTED_VERSION="NOVA-3.3.4-MARKET-WIDE-ROTATION-VERIFY"
IMAGE="quant-nova:3.3.4-r49-final-local"
STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP="${APP}-pre-r49-${STAMP}"
FAILED="${APP}-failed-r49-${STAMP}"
CANDIDATE="${APP}-candidate-r49-${STAMP}"
WORK="$(mktemp -d /tmp/nova-r49-final.XXXXXX)"
DOCKERFILE="$WORK/Dockerfile"
ENVFILE="$WORK/current.env"
CAND_DATA="$WORK/candidate-data"
LOG="/tmp/nova-r49-final-deploy-${STAMP}.log"
OLD_WAS_RUNNING=0
OLD_STOPPED=0
OLD_RENAMED=0
NEW_STARTED=0
...
recover_previous_if_needed(){
  ...
  latest="$(dc ps -a --format '{{.Names}}' | grep -E "^${APP}-pre-r(49|48|47)-[0-9]{8}-[0-9]{6}$" | sort | tail -n 1 || true)"
  if [ -n "$latest" ]; then
...
fetch_dockerfile(){
  local remote="QUANT_NOVA_3.3.4_R49_SIGNAL_ACCELERATION_TOP20_FINAL_20260820.Dockerfile"
...
say "1/8 FETCH R49 FINAL DOCKERFILE + SHA LOCK"
fetch_dockerfile
...
[ "$(dc image inspect "$IMAGE" --format '{{index .Config.Labels "io.quantnova.ui_patch"}}')" = "R49_SIGNAL_ACCELERATION_TOP20_ENTRY_TARGET" ]
...
[ "$(dc image inspect "$IMAGE" --format '{{index .Config.Labels "io.quantnova.signal_acceleration"}}')" = "TOP20_5M15M30M_RANK30_DWELL60_GAP3" ]
[ "$(dc image inspect "$IMAGE" --format '{{index .Config.Labels "io.quantnova.entry_candidate"}}')" = "DISPLAY_ONLY_4OF5_NO_ENTRY_V18_MUTATION" ]
[ "$(dc image inspect "$IMAGE" --format '{{index .Config.Labels "io.quantnova.pre_antistickiness"}}')" = "STALE_SIGNAL_PIN_15M_EXCLUDED_UNLESS_FAST_SCOUT" ]
...
assert 'DISPLAY ONLY' in str(pc.get('entry_candidate_policy') or ''),pc
assert len(panels.get('preignition') or [])<=20
print(json.dumps({'ok':True,'version':expected,'verify':'PASS','rotation':'PASS','signal_accel':'TOP20_RANK30_DWELL60_GAP3','entry_target':'DISPLAY_ONLY','swap_mb':0},ensure_ascii=False))
PY
verify_protected
