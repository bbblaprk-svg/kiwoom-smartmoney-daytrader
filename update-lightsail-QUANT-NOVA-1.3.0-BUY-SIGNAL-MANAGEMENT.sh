...
ZIP_NAME="QUANT_NOVA_1.3.0_BUY_SIGNAL_MANAGEMENT.zip"
ROOT_NAME="nova130"
APP_DIR="$HOME/quant-nova"
IMAGE="quant-nova:1.3.0"
...
rollback(){
  local ec=$?
  [[ $ec -eq 0 ]] && return
  echo "[자동복구] NOVA 1.3.0 적용 실패 — 이전 NOVA 복구" >&2
...
grep -Fq "APP_VERSION='NOVA-1.3.0'" "$ROOT/app/main.py" || fail "버전 동기화 누락"
...
[[ -n "$HEALTH" ]] || { sudo docker logs --tail 200 "$CONTAINER" || true; fail "NOVA 1.3.0 health 실패"; }
printf '%s' "$HEALTH" > "$TMP/health.json"
python3 - "$TMP/health.json" <<'PY'
import json,sys
h=json.load(open(sys.argv[1]))
assert h.get('ok') is True,h
assert h.get('version')=='NOVA-1.3.0',h.get('version')
...
PY
curl -fsS --max-time 5 http://127.0.0.1:${HOST_PORT}/api/nova | python3 -c 'import sys,json;j=json.load(sys.stdin); h=j.get("health") or {}; print("NOVA_API=OK rows=",len(j.get("rows") or []),"smart=",h.get("smart_status"),"learning=",h.get("learning"))'
curl -fsS --max-time 5 http://127.0.0.1:${HOST_PORT}/api/buy-signals | python3 -c 'import sys,json;j=json.load(sys.stdin); assert j.get("version")=="NOVA-1.3.0"; print("BUY_SIGNAL_API=OK rows=",len(j.get("rows") or []),"events=",j.get("total_events"))'

say "7/7 완료"
if [[ $OLD_RENAMED -eq 1 ]]; then sudo docker rm -f "$BACKUP" >/dev/null 2>&1 || true; OLD_RENAMED=0; fi
trap - ERR
sudo docker image prune -f >/dev/null 2>&1 || true
echo "=== QUANT NOVA 1.3.0 FINAL ==="
echo "IMAGE=$IMAGE"
