...
REV="QUANT-NOVA-2.4.22-CORE-LIVENESS-ASYNCIO-V1"
APP="quant-nova"
...
src, old_name, image = sys.argv[1:4]
obj=json.loads(subprocess.check_output(["sudo","docker","inspect",src],text=True))[0]
cfg=obj.get("Config") or {}
host=obj.get("HostConfig") or {}
nets=((obj.get("NetworkSettings") or {}).get("Networks") or {})
args=["sudo","docker","create","--name",src]
...
for n in net_names[1:]:
    subprocess.run(["sudo","docker","network","connect",n,src],check=True,stdout=subprocess.DEVNULL)
...
python3 "$CLONE_PY" "$APP" "$OLD" "$SNAP_IMAGE"
