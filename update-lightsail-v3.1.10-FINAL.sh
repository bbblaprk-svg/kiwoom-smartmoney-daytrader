...
ZIP_NAME="kiwoom-smartmoney-daytrader-v3.1.10-FINAL.zip"
SOURCE_DIR_NAME="kiwoom-smartmoney-daytrader-v3.1.10"
IMAGE_NAME="kiwoom-smartmoney:v3.1.10"
...
rollback() {
  ...
  printf '\n[자동 복구] v3.1.10 FINAL 적용 실패 — 기존 앱으로 되돌립니다.\n' >&2
...
say "v3.1.10 FINAL 파일 받기"
git clone --depth 1 "$REPO_URL" "$TMP_ROOT/repo"
cd "$TMP_ROOT/repo"
[[ -f "$ZIP_NAME" ]] || fail "GitHub 최상위에 $ZIP_NAME 파일이 없습니다."
EXPECTED_ZIP_SHA256="2d114c40bf27ee76f2c6da7b62c6dc3103dbd30dae18a8bdd032054fca06d626"
...
upsert_env KIWOOM_INVESTOR_API_ID ka10059
upsert_env KIWOOM_FOREIGN_API_ID ka10008
upsert_env KIWOOM_PROGRAM_API_ID ka90004
upsert_env KIWOOM_PROGRAM_DAILY_API_ID ka90013
...
COPY kiwoom-smartmoney-daytrader-v3.1.10-FINAL.zip /tmp/app.zip
RUN mkdir -p /tmp/source \
    && unzip -q /tmp/app.zip -d /tmp/source \
    && cp -a /tmp/source/kiwoom-smartmoney-daytrader-v3.1.10/. /app/ \
...
say "v3.1.10 FINAL Docker 이미지 빌드 — 검사·부하검사·Next 빌드 포함"
...
if grep -q '"status":"connected"' "$TMP_ROOT/health.json"; then
  ...
  printf '[주의] 앱 서버는 정상 기동했지만 키움 연결은 아직 connected가 아닙니다. 점검시간/인증/네트워크를 /api/health에서 확인하세요.\n' >&2
fi
...
printf '\nSafari 탭을 완전히 닫았다가 다시 열고 v3.1.10, 장전 사전점검, RISK/EARLY_SELL, 실제체결시장, 아이폰 푸시 테스트를 확인하세요.\n'
