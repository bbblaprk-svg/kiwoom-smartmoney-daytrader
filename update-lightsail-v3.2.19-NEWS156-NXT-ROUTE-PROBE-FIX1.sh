...
ZIP_NAME="kiwoom-unified-edge-v3.2.19-NEWS156-NXT-ROUTE-PROBE-FIX1.zip"
SOURCE_DIR_NAME="kiwoom-unified-edge-v3.2.19-FIX1"
EXPECTED_ZIP_SHA256="74b8722635d54bb0b5daa7d7bf1b9f3adcb3b8287a800a32ead1d96ef6417b02"
OLD_APP_DIR="$HOME/kiwoom-smartmoney-daytrader"
NEW_APP_DIR="$HOME/kiwoom-unified-edge"
TRADER_IMAGE="kiwoom-unified:v3.2.19"
...
rollback(){
  local ec=$?
  [[ $ec -eq 0 ]] && return
  printf '\n[자동복구] v3.2.19 FIX1 적용 실패 — 기존 컨테이너로 복구합니다.\n' >&2
...
say "v3.2.19 NXT ROUTE PROBE FIX1 + News 1.5.6 파일 받기"
...
[[ "$TRADER_VERSION" == "3.2.19" ]] || fail "trader 버전 불일치: $TRADER_VERSION"
[[ "$NEWS_VERSION" == "1.5.6" ]] || fail "news 버전 불일치: $NEWS_VERSION"
grep -Fq "type:['0A','0B']" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "0A/0B 자동탐지 CORE 등록 누락"
grep -Fq "BARE_CODE_DUAL_TYPE_AUTO_DETECT_V2" "$ROOT/trader/worker/kiwoomRealtime.js" || fail "bare-code 자동라우팅 누락"
grep -Fq "REALTIME_DEBUG_CHECK PASS" "$ROOT/trader/scripts/realtime-debug-check.js" || fail "v3.2.19 realtime debug 검사 누락"
grep -Fq "NXT_ROUTE_PROBE_ACK_FIRST_TICK_FRESH_V8" "$ROOT/trader/scripts/realtime-latency-check.js" || fail "FIX1 realtime-latency gate marker 누락"
grep -q "api/realtime-debug" "$ROOT/trader/pages/api/realtime-debug.js" || true
...
[[ "$RUNNING_VERSION" == "3.2.19" ]] || fail "실행 버전 불일치: $RUNNING_VERSION"

say "CORE REG ACK + 실제 체결 firstTick/fresh 검증 + NXT route probe (최대 75초)"
RUNTIME_OK=0
for _ in $(seq 1 25); do
  OUT="$(curl -fsS --max-time 3 http://127.0.0.1:3000/api/health 2>/dev/null || true)"
  if [[ -n "$OUT" ]]; then
    printf '%s' "$OUT" > "$TMP_ROOT/health.json"
    CHECK="$(python3 - "$TMP_ROOT/health.json" <<'PY'
import json,sys,datetime
h=json.load(open(sys.argv[1]))
r=h.get('registrationStats') or {}
core=int(r.get('coreRegAccepted') or 0)
typ=r.get('detectedTradeType')
krx=r.get('KRX') or {}; nxt=r.get('NXT') or {}; sor=r.get('SOR') or {}
fresh=sum(int((x or {}).get('fresh') or 0) for x in (krx,nxt,sor))
first=sum(int((x or {}).get('firstTick') or 0) for x in (krx,nxt,sor))
print('OK' if core>0 and typ in ('0A','0B') and first>0 and fresh>0 else 'WAIT', core, typ, first, fresh, h.get('dataStatus'))
PY
)"
    echo "$CHECK"
...
# 장외시간에는 firstTick=0이 정상일 수 있으므로 KST 활성장인지 판단 후만 강제 실패.
ACTIVE_NOW="$(python3 - <<'PY'
from datetime import datetime,timezone,timedelta
k=datetime.now(timezone.utc)+timedelta(hours=9)
sec=k.hour*3600+k.minute*60+k.second
wd=k.weekday()<5
krx=9*3600<=sec<=15*3600+30*60
nxt=(8*3600<=sec<=8*3600+50*60) or (9*3600+30<=sec<=15*3600+20*60) or (15*3600+40*60<=sec<=20*3600)
print('1' if wd and (krx or nxt) else '0')
PY
)"
if [[ "$ACTIVE_NOW" == "1" && $RUNTIME_OK -ne 1 ]]; then
  cat "$TMP_ROOT/health.json" || true
  sudo docker logs --tail 220 "$APP_CONTAINER" || true
  fail "활성장인데 KRX/NXT/SOR 전체에서 실제 WebSocket 체결 firstTick/fresh가 0입니다. 자동 롤백합니다."
fi
