...
ZIP_NAME="kiwoom-smartmoney-daytrader-v3.1.6-FINAL-R3.zip"
SOURCE_DIR_NAME="kiwoom-smartmoney-daytrader-v3.1.6"
IMAGE_NAME="kiwoom-smartmoney:v3.1.6"
...
rollback() {
  ...
  printf '\n[자동 복구] v3.1.6 FINAL 적용 실패 — 기존 앱으로 되돌립니다.\n' >&2
...
say "v3.1.6 FINAL 파일 받기"
git clone --depth 1 "$REPO_URL" "$TMP_ROOT/repo"
cd "$TMP_ROOT/repo"
[[ -f "$ZIP_NAME" ]] || fail "GitHub 최상위에 $ZIP_NAME 파일이 없습니다."
EXPECTED_ZIP_SHA256="1b82ad3659d5fb0f1f52493949465499dc4b4808de87f6f2e9dd35c40834eb0f"
...
upsert_env KIWOOM_PROGRAM_API_ID ka90003
...
COPY kiwoom-smartmoney-daytrader-v3.1.6-FINAL-R3.zip /tmp/app.zip
RUN mkdir -p /tmp/source \
    && unzip -q /tmp/app.zip -d /tmp/source \
    && cp -a /tmp/source/kiwoom-smartmoney-daytrader-v3.1.6/. /app/ \
...
say "v3.1.6 FINAL Docker 이미지 빌드 — 검사·부하검사·Next 빌드 포함"
...
printf '\nSafari 탭을 완전히 닫았다가 다시 열고 v3.1.6, 신호 한눈에, 종합기록부를 확인하세요.\n'
