...
RELEASE="3.9.1_HIDDEN_GEM_PUBLIC_READER"
...
PUB="${PULSE_DEPLOY_PUBLIC_URL:-http://3.38.25.20}"
SRC_NAME="PULSE_EDGE_GREENFIELD_3_9_1_HIDDEN_GEM_PUBLIC_READER_R1_SOURCE.tar.gz"
DF_NAME="Dockerfile"
EXPECTED_SRC_SHA="095eae9be8b129e97dbd3c1928975aa4e7b045ec06b8e5be63ad69a261edcb08"
EXPECTED_DF_SHA="7cc24689e654b3b78c4aac524e61b23013799f8cdcfa8df7911c42f9a304205a"
MODE="${1:-deploy}"
SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)"
RUN_ID="$(date +%Y%m%d-%H%M%S)-$$"
RUN_DIR="$BASE/deployments/$RUN_ID"
CANARY_DATA="$RUN_DIR/canary-data"
IMG="pulse-edge:gf391r1-hidden-$RUN_ID"
CANARY="pulse-edge-gf391r1-canary-$RUN_ID"
LIVE="pulse-edge-gf391r1-live-$RUN_ID"
OLD=""
OLD_RUNNING=false
OLD_RESTART="no"
CUTOVER=0
...
# Library/SCP downloads may append a suffix such as "(1)" or a timestamp to
# the two build artifacts. Resolve the canonical name first, then accept one
# unambiguous suffixed copy. The build context is always rewritten to the
# canonical names expected by the Dockerfile.
resolve_artifact(){
  local label="$1" canonical="$2" pattern="$3" exact="$SCRIPT_DIR/$2"
  local -a candidates=()
  if [ -f "$exact" ]; then
    printf '%s\n' "$exact"
    return 0
  fi
  mapfile -t candidates < <(find "$SCRIPT_DIR" -maxdepth 1 -type f -name "$pattern" -print | sort)
  [ "${#candidates[@]}" -eq 1 ] || die "$label missing or ambiguous; expected $canonical (or one suffixed copy), candidates=${#candidates[@]}"
  printf '%s\n' "${candidates[0]}"
}
...
_,pub=get('/api/public/hidden-gem/top?limit=3'); assert pub.get('release')==release and pub.get('read_only') is True,pub
assert isinstance(pub.get('rows'),list),pub
for row in pub.get('rows') or []:
    for key in ('symbol','name','venue','session','current_price','change_rate','stage','energy_stored','energy_released','remaining_energy','release_imminence','price_hold','data_status','reasons','invalidation','too_late'):
        assert key in row,(key,row)
    text=json.dumps(row,ensure_ascii=False).lower(); assert 'appkey' not in text and 'secretkey' not in text,row
_,c=get('/api/coiled-spring/top?limit=5'); assert c.get('release')==release and c.get('source')=='DISABLED' and (c.get('rows') or [])==[],c
...
for x in docker curl python3 sha256sum awk grep sed flock df find; do need "$x"; done
...
log "[0/13] HIDDEN-GEM PUBLIC-READER R1 CERTIFIED $RELEASE"
log "BASE=$BASE"

log "[1/13] Validate exact release artifacts and archive"
[ -n "${SCRIPT_DIR:-}" ] || die "cannot determine script directory"
DOCKERFILE_PATH="$(resolve_artifact Dockerfile "$DF_NAME" 'Dockerfile*')"
SOURCE_PATH="$(resolve_artifact SOURCE "$SRC_NAME" 'PULSE_EDGE_GREENFIELD_3_9_1_HIDDEN_GEM_PUBLIC_READER_R1_SOURCE*.gz')"
REAL_SRC="$(sha256sum "$SOURCE_PATH" | awk '{print $1}')"
REAL_DF="$(sha256sum "$DOCKERFILE_PATH" | awk '{print $1}')"
...
log "ARTIFACT_RESOLUTION_PASS dockerfile=$(basename "$DOCKERFILE_PATH") source=$(basename "$SOURCE_PATH")"
cp "$DOCKERFILE_PATH" "$RUN_DIR/build/Dockerfile"
cp "$SOURCE_PATH" "$RUN_DIR/build/$SRC_NAME"
...
for req in ['requirements-runtime.txt','pulse_edge/main.py','pulse_edge/config.py','pulse_edge/models.py','pulse_edge/feed/kiwoom.py','pulse_edge/engine/features.py','pulse_edge/engine/scorer.py','pulse_edge/storage/baseline.py','pulse_edge/storage/signals.py','pulse_edge/web/index.html','pulse_edge/web/pulse.js','pulse_edge/web/sw.js','deploy/offline_tests.py','deploy/verify.py']:
...
python3 - "$RUN_DIR/source" "$RELEASE" <<'PYREL'
import pathlib,sys
root=pathlib.Path(sys.argv[1]); release=sys.argv[2]
config=(root/'pulse_edge/config.py').read_text(encoding='utf-8')
verify=(root/'deploy/verify.py').read_text(encoding='utf-8')
main=(root/'pulse_edge/main.py').read_text(encoding='utf-8')
assert release in config, 'release marker missing from config.py'
assert release in verify, 'release marker missing from verify.py'
assert "/api/public/hidden-gem/top" in main, 'public reader endpoint missing'
for token in ('3.9.0_HIDDEN_GEM_PURE','3.9.0_HIDDEN_GEM'):
    assert token not in config and token not in verify and token not in main, 'stale release marker: '+token
print('SOURCE_RELEASE_CONSISTENCY_PASS release='+release)
PYREL
...
fixed={
'PULSE_RELEASE':'3.9.1_HIDDEN_GEM_PUBLIC_READER','PULSE_FEED_ENABLED':'true','PULSE_DATA_DIR':'/app/data/pulse_edge',
'PULSE_BASELINE_DB':'/app/data/pulse_edge/tod_baseline.sqlite3','PULSE_HISTORY_DB':'/app/data/pulse_edge/signal_history.sqlite3','PULSE_HISTORY_LIMIT':'400','PULSE_NOW_HISTORY_LIMIT':'200',
'PULSE_PUSH_ENABLED':'true','PULSE_COILED_ENABLED':'false',
'PULSE_WS_DYNAMIC_MAX_ITEMS':'34','PULSE_WS_BACKGROUND_RESERVE_ITEMS':'14','PULSE_WS_QUOTE_FOCUS_ITEMS':'4',
'PULSE_PRIORITY_IDLE_SEC':'600','PULSE_DISCOVERY_NEW_SEC':'180','PULSE_DISCOVERY_FRESH_SLOTS':'6',
'PULSE_LATENCY_SAMPLE_SIZE':'300','PULSE_EVENT_LOOP_PROBE_SEC':'0.5','PULSE_PROBE_REST_CONFIRM_MIN_SCORE':'60','PULSE_PROBE_REST_CONFIRM_COOLDOWN_SEC':'20',
'PULSE_URGENT_BASELINE_MIN_SCORE':'60','PULSE_URGENT_BASELINE_COOLDOWN_SEC':'20','PULSE_QUOTE_NEWCOMER_SLOTS':'2','PULSE_QUOTE_ROTATION_SEC':'12',
'PULSE_ZERO_TRADE_MINUTE_COVERAGE_SAMPLES':'45','PULSE_BID_ABSORPTION_MIN_SCORE':'60','PULSE_BID_ABSORPTION_WINDOW_SEC':'90',
'PULSE_BID_ABSORPTION_ABS_FLOOR_KRW':'5000000','PULSE_BID_ABSORPTION_SELL_RATIO_MIN':'0.20','PULSE_EARLY_ABSORPTION_MIN_OBSERVED_SEC':'180',
'PULSE_EARLY_HIDDEN_GEM_MIN_SCORE':'60','PULSE_EARLY_HIDDEN_GEM_CHANGE_MIN':'-1.5','PULSE_EARLY_HIDDEN_GEM_CHANGE_MAX':'2.5',
'PULSE_RANK_COMMIT_SEC':'30','PULSE_BACKGROUND_ROTATE_SEC':'4','PULSE_SCAN_INTERVAL_SEC':'20','PULSE_INVESTOR_REFRESH_SEC':'20',
'PULSE_DAILY_CONTEXT_REFRESH_SEC':'120','PULSE_DAILY_CONTEXT_SYMBOLS':'20','PULSE_BASELINE_BOOTSTRAP_INTERVAL_SEC':'60','PULSE_BASELINE_BOOTSTRAP_SYMBOLS_PER_CYCLE':'2',
'PULSE_SUPPLY_TIGHT_MIN_CONDITIONS':'8','PULSE_SUPPLY_EXHAUSTION_MIN_SIGNALS':'7','PULSE_IGNITION_TRIGGER_MIN_COUNT':'2',
'PULSE_HIDDEN_GEM_PRE_STORED_MIN':'80','PULSE_HIDDEN_GEM_PRE_REMAINING_MIN':'75','PULSE_HIDDEN_GEM_PRE_IMMINENCE_MIN':'82','PULSE_HIDDEN_GEM_PRE_RELEASED_MAX':'45',
'PULSE_HIDDEN_GEM_PRE_FLOW_MIN':'60','PULSE_HIDDEN_GEM_PRE_SELL_FAILURE_MIN':'55','PULSE_HIDDEN_GEM_PRE_SUPPLY_DECAY_MIN':'55','PULSE_HIDDEN_GEM_PRE_EXHAUSTION_SCORE_MIN':'60',
'PULSE_HIDDEN_GEM_ENTRY_IMMINENCE_MIN':'90','PULSE_ENERGY_EXTENDED_CHANGE_MAX':'7','PULSE_ENERGY_EXTENDED_REMAINING_MIN':'50','PULSE_ENERGY_EXTENDED_RELEASED_MAX':'60',
'PULSE_TOO_LATE_RELEASED_MIN':'78','PULSE_TOO_LATE_REMAINING_MAX':'28'}
...
import json,sys
q=json.load(open(sys.argv[1])); assert q['release']=='3.9.1_HIDDEN_GEM_PUBLIC_READER' and q['ready'] is True and q['feed_enabled'] is False
PY
  fi
  running "$CANARY" || { docker logs --tail 100 "$CANARY" || true; die "canary stopped"; }; sleep 1
done
[ "$ready" -eq 1 ] || die "canary readiness failed"
python3 "$RUN_DIR/source/deploy/verify.py" "$CURL" "$RELEASE"
...
# Closed session: live tick proof is intentionally not required; COILED runtime is disabled in HIDDEN GEM PURE.

log "[9/13] Caddy route and public HTTP"
...
import json,sys
h=json.load(open(sys.argv[1])); assert h.get('runtime_safety_release')=='3.9.1_HIDDEN_GEM_PUBLIC_READER'
PY

# The public Caddyfile on this server may still point to the previous ephemeral
# container name.  The deployment route contract is the stable Docker alias
# pulse-edge, so normalize the mounted Caddyfile to that alias before testing
# the public route.  This keeps rollback safe: reconnecting OLD with alias
# pulse-edge automatically restores the previous runtime without another edit.
CADDYFILE_SOURCE="$(docker inspect "$CADDY" --format '{{range .Mounts}}{{if eq .Destination "/etc/caddy/Caddyfile"}}{{.Source}}{{end}}{{end}}')"
[ -n "$CADDYFILE_SOURCE" ] || die "Caddyfile host mount not found"
[ -f "$CADDYFILE_SOURCE" ] || die "Caddyfile host source missing: $CADDYFILE_SOURCE"
cp -a "$CADDYFILE_SOURCE" "$RUN_DIR/Caddyfile.before-cutover"
python3 - "$CADDYFILE_SOURCE" <<'PYCADDY'
from pathlib import Path
import re,sys
p=Path(sys.argv[1])
s=p.read_text(encoding='utf-8')
# Rewrite only reverse_proxy upstreams that target this app's port 8000.
pat=re.compile(r'(?m)^(\s*reverse_proxy\s+)(?:https?://)?[^\s{]+:8000(\s*(?:\{)?\s*)$')
new,n=pat.subn(r'\1pulse-edge:8000\2',s)
if n < 1:
    # It may already be normalized with extra directives on the same line.
    if not re.search(r'(?m)^\s*reverse_proxy\s+(?:https?://)?pulse-edge:8000(?:\s|$)',s):
        raise SystemExit('no app reverse_proxy :8000 upstream found in Caddyfile')
    new=s
p.write_text(new,encoding='utf-8')
print('CADDY_STABLE_ALIAS_ROUTE_PASS replacements='+str(n))
PYCADDY
docker exec "$CADDY" caddy validate --config /etc/caddy/Caddyfile >/dev/null || die "Caddyfile validation failed after stable-alias rewrite"
docker restart "$CADDY" >/dev/null || die "Caddy restart failed after route rewrite"
sleep 3
# Re-check through Caddy after reload/restart, not only by direct app DNS.
docker exec "$CADDY" sh -c '
  if command -v wget >/dev/null 2>&1; then
    wget -q -T 8 -O - http://pulse-edge:8000/health
  elif command -v curl >/dev/null 2>&1; then
...
' >"$RUN_DIR/caddy-health-after-route.json" || die "Caddy lost pulse-edge route after restart"
python3 - "$RUN_DIR/caddy-health-after-route.json" <<'PY'
import json,sys
h=json.load(open(sys.argv[1])); assert h.get('runtime_safety_release')=='3.9.1_HIDDEN_GEM_PUBLIC_READER',h
print('CADDY_ROUTE_RELEASE_PASS')
PY
...
[ "$public" -eq 1 ] || die "public route did not validate"
