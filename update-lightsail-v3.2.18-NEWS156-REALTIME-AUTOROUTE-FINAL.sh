...
ZIP_NAME="kiwoom-unified-edge-v3.2.18-NEWS156-REALTIME-AUTOROUTE-FINAL.zip"
SOURCE_DIR_NAME="kiwoom-unified-edge-v3.2.18"
EXPECTED_ZIP_SHA256="c7d867e8211677a164452a6c208cd9b92e7b8483e952823c38a663806aca1186"
OLD_APP_DIR="$HOME/kiwoom-smartmoney-daytrader"
NEW_APP_DIR="$HOME/kiwoom-unified-edge"
TRADER_IMAGE="kiwoom-unified:v3.2.18"
...
rollback(){
  local ec=$?
  [[ $ec -eq 0 ]] && return
  printf '\n[자동복구] v3.2.18 적용 실패 — 기존 컨테이너로 복구합니다.\n' >&2
...
TRADER_VERSION="$(node -p "require('$ROOT/trader/package.json').version")"
NEWS_VERSION="$(node -p "require('$ROOT/news-radar/package.json').version")"
[[ "$TRADER_VERSION" == "3.2.18" ]] || fail "trader 버전 불일치: $TRADER_VERSION"
...
grep -Fq "REALTIME_ENGINE_V3218_CHECK PASS" "$ROOT/trader/scripts/realtime-engine-v3218-check.js" || fail "v3.2.18 검사 누락"
...
[[ "$RUNNING_VERSION" == "3.2.18" ]] || fail "실행 버전 불일치: $RUNNING_VERSION"

say "CORE REG ACK + 실제 체결 firstTick/fresh 검증 (최대 75초)"
RUNTIME_OK=0
...
if [[ "$ACTIVE_NOW" == "1" && $RUNTIME_OK -ne 1 ]]; then
  ...
  fail "활성장인데 실제 WebSocket 체결 firstTick/fresh를 확보하지 못했습니다. 자동 롤백합니다."
fi
