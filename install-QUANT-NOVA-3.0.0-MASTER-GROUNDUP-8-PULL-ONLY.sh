...
VERSION="3.0.0"
REV="MASTER-GROUNDUP-8-PULL-ONLY"
APP_VERSION="NOVA-3.0.0-MASTER-GROUNDUP-8"
IMAGE="ghcr.io/bbblaprk-svg/kiwoom-smartmoney-daytrader:nova-3.0.0-master-groundup-8"
EXPECTED_SOURCE_ZIP_SHA256="c7f0e3d12ea5dbb78aafc19f860f27fc72a748aedb6454107f94ed3f66affbb5"
...
DEPLOY_DIR="$APP_DIR/deployments/nova3000-master-groundup8-$STAMP"
BACKUP="quant-nova-backup-master-groundup8-$STAMP"
CANDIDATE="quant-nova-candidate-master-groundup8-$STAMP"
...
rollback(){
  ...
  echo "[AUTO-ROLLBACK] G8 미완료 — 새 컨테이너 제거 및 기존 immutable 상태 복원" >&2
  ...
  if [[ $ACTIVE_DATA_MUTATED -eq 1 && -d "$BASE_DATA/nova30" ]]; then
    ...
    echo "[AUTO-ROLLBACK] G8 NOVA30 state restored; failed attempt preserved at $FAILED_DATA" >&2
  fi
...
sudo docker exec "$CANDIDATE" python -c "import json,urllib.request;j=json.load(urllib.request.urlopen('http://127.0.0.1:8000/api/readyz',timeout=2));assert j.get('ok') and j.get('version')=='NOVA-3.0.0-MASTER-GROUNDUP-8',j;print('CANDIDATE_READYZ=PASS',j)"
...
rss=[]
for i in range(15):
    with urllib.request.urlopen('http://127.0.0.1:8000/api/realtime-health',timeout=3) as r:j=json.load(r)
...
curl -fsS --max-time 3 "http://127.0.0.1:${HOST_PORT}/api/livez" | python3 -c 'import json,sys;j=json.load(sys.stdin);assert j.get("ok") and j.get("version")=="NOVA-3.0.0-MASTER-GROUNDUP-8",j;print("ACTIVE_LIVEZ=PASS",j)'
...
sudo docker exec "$CONTAINER" python -c "import json,urllib.request;j=json.load(urllib.request.urlopen('http://127.0.0.1:8000/api/readyz',timeout=2));assert j.get('ok') and j.get('version')=='NOVA-3.0.0-MASTER-GROUNDUP-8',j;print('ACTIVE_READYZ=PASS',j)"
...
say "12/14 Static UI guard atomic sync from validated image"
if [[ -d "$GUARD_STATIC" ]]; then
  mkdir -p "$GUARD_BK" "$TMP/g8-static"
  sudo cp -a "$GUARD_STATIC"/. "$GUARD_BK"/
  GUARD_BACKUP_READY=1
  for f in index.html nova.js nova.css manifest.webmanifest sw.js; do
    sudo docker cp "$CONTAINER:/app/static/$f" "$TMP/g8-static/$f"
    sudo cp "$TMP/g8-static/$f" "$GUARD_STATIC/.$f.nova30.tmp"
...
rm -rf "$TMP"
trap - EXIT
echo "=== QUANT NOVA 3.0.0 MASTER GROUND-UP 8 PULL-ONLY DEPLOY PASS ==="
