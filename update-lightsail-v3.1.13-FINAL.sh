...
ZIP_NAME="kiwoom-smartmoney-daytrader-v3.1.13-FINAL.zip"
SOURCE_DIR_NAME="kiwoom-smartmoney-daytrader-v3.1.13"
EXPECTED_ZIP_SHA256="3d59ce80df81e315e712d2a222c8541b6ebcc197b20a7690ddf6386995df5173"
IMAGE_NAME="kiwoom-smartmoney:v3.1.13"
...
rollback() {
  ...
  printf '\n[자동 복구] v3.1.13 적용 실패 — 기존 앱으로 되돌립니다.\n' >&2
...
# v3.1.13은 v3.1.12의 자동추천/중요기록부를 보존하고 자동추천 자동적용 ON/OFF를 정식 버전으로 분리한다. 데이터 수신 경로는 보존한다.
...
COPY kiwoom-smartmoney-daytrader-v3.1.13-FINAL.zip /tmp/app.zip
RUN mkdir -p /tmp/source \
    && unzip -q /tmp/app.zip -d /tmp/source \
    && cp -a /tmp/source/kiwoom-smartmoney-daytrader-v3.1.13/. /app/ \
...
say "v3.1.13 Docker 이미지 빌드 — 전체검사·20종목 부하검사·Next 빌드 포함"
...
[[ "$RUNNING_VERSION" == "3.1.13" ]] || fail "실행 컨테이너 버전이 3.1.13이 아닙니다: $RUNNING_VERSION"
...
printf '추천      : 평일 20:15 집중8/경량8 자동추천 + 수동보호 + 자동적용 ON/OFF(기본 OFF)\n'
printf '============================================================\n'
printf '현재 건강상태: '
cat "$TMP_ROOT/health.json"
printf '\n\n최근 앱 로그:\n'
sudo docker logs --tail 60 "$APP_CONTAINER" || true
printf '\nSafari 탭을 완전히 닫았다가 다시 열고 v3.1.13 종목관리에서 자동적용 ON/OFF를 확인하세요.\n'
