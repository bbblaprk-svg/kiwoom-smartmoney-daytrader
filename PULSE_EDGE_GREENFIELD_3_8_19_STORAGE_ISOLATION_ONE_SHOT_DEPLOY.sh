...
RELEASE="3.8.19_STORAGE_ISOLATION"
...
SRC_NAME="PULSE_EDGE_GREENFIELD_3_8_19_STORAGE_ISOLATION_SOURCE.tar.gz"
EXPECTED_SRC_SHA="ac1e3c382de9096f9d091a36fb77e26ed7999b1c6ca99af9fd8c12e10c20b8fa"
EXPECTED_DF_SHA="46ac93ea83aa409469bb6227443b693dd05e82562015687dd42f29b820018665"
MODE="${1:-deploy}"
SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)"
RUN_ID="$(date +%Y%m%d-%H%M%S)-$$"
RUN_DIR="$BASE/deployments/$RUN_ID"
CANARY_DATA="$RUN_DIR/canary-data"
IMG="pulse-edge:gf3819-hidden-$RUN_ID"
CANARY="pulse-edge-gf3819-canary-$RUN_ID"
LIVE="pulse-edge-gf3819-live-$RUN_ID"
OLD=""
OLD_RUNNING=false
OLD_RESTART="no"
CUTOVER=0
...
fixed={
'PULSE_RELEASE':'3.8.19_STORAGE_ISOLATION','PULSE_FEED_ENABLED':'true','PULSE_DATA_DIR':'/app/data/pulse_edge',
...
import json,sys
q=json.load(open(sys.argv[1])); assert q['release']=='3.8.19_STORAGE_ISOLATION' and q['ready'] is True and q['feed_enabled'] is False
PY
  fi
...
import json,sys
q=json.load(open(sys.argv[1])); assert q.get('release')=='3.8.19_STORAGE_ISOLATION' and isinstance(q.get('rows'),list),q
...
import json,sys
h=json.load(open(sys.argv[1])); assert h.get('runtime_safety_release')=='3.8.19_STORAGE_ISOLATION'
PY
public=0
