...
WORK="$(mktemp -d /tmp/nova-r492-freshrot.XXXXXX)"
ENVFILE="$WORK/current.env"
BUILDCTX="$WORK/buildctx"
LOG="/tmp/nova-r492-freshrot-${STAMP}.log"
NEW_STARTED=0
OLD_RENAMED=0
OLD_STOPPED=0
...
say(){ printf '\n===== %s =====\n' "$*" | tee -a "$LOG"; }
cleanup(){
  if [ -d "$WORK" ] && [[ "$WORK" == /tmp/nova-r492-freshrot.* ]]; then
...
# A prior R1 cutover may have stopped/renamed quant-nova before failing on an empty network name.
# Recover that exact pre-cutover container first if needed, then arm rollback for this run.
recover_previous_interrupted_cutover
trap rollback ERR INT TERM

say "1. CURRENT CONTAINER + DOCKERFILE PRECHECK — DEPLOY R2 NETWORK-SAFE"
...
say "2. BUILD SELF-CONTAINED IMAGE — RUNNING APP UNTOUCHED"
dc build --no-cache --pull -t "$IMAGE" "$BUILDCTX" | tee -a "$LOG"
LABEL_VERSION="$(dc image inspect "$IMAGE" --format '{{index .Config.Labels "org.opencontainers.image.version"}}')"
LABEL_FIX="$(dc image inspect "$IMAGE" --format '{{index .Config.Labels "io.quantnova.fresh_rotation_fix"}}')"
LABEL_EMBED="$(dc image inspect "$IMAGE" --format '{{index .Config.Labels "io.quantnova.embedded_source_sha256"}}')"
[ "$LABEL_VERSION" = "$EXPECTED_VERSION" ]
[ "$LABEL_FIX" = "$EXPECTED_FIX" ]
[ "$LABEL_EMBED" = "$EXPECTED_EMBEDDED_SHA" ]
...
PY
# Verify the protected runtime files again inside the actually running container.
dc exec "$APP" sh -lc '
