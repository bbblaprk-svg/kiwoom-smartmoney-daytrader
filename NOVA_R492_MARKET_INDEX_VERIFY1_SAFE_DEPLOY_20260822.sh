...
REMOTE_DOCKERFILE="QUANT_NOVA_3.3.5_R492_MARKET_INDEX_VERIFY1_20260822.Dockerfile"
...
fetch_dockerfile(){
  ...
  if curl -fsSL --max-time 35 "$raw" -o "$DOCKERFILE" 2>/dev/null; then return 0; fi
  token="${GH_TOKEN:-${GITHUB_TOKEN:-}}"
  if [ -z "$token" ]; then
    printf '\nGitHub 저장소가 Private이면 Contents: Read 권한 token을 입력하세요.\n' >&2
