...
ZIP_NAME="QUANT_NOVA_1.2.0_ADAPTIVE_SMART_MONEY.zip"
ROOT_NAME="nova120"
APP_DIR="$HOME/quant-nova"
IMAGE="quant-nova:1.2.0"
...
rollback(){
  local ec=$?
  [[ $ec -eq 0 ]] && return
  echo "[자동복구] NOVA 1.2.0 적용 실패 — 이전 NOVA 복구" >&2
...
grep -Fq "APP_VERSION='NOVA-1.2.0'" "$ROOT/app/main.py" || fail "버전 동기화 누락"
...
[[ -n "$HEALTH" ]] || { sudo docker logs --tail 200 "$CONTAINER" || true; fail "NOVA 1.2.0 health 실패"; }
printf '%s' "$HEALTH" > "$TMP/health.json"
python3 - "$TMP/health.json" <<'PY'
import json,sys
h=json.load(open(sys.argv[1]))
assert h.get('ok') is True,h
assert h.get('version')=='NOVA-1.2.0',h.get('version')
