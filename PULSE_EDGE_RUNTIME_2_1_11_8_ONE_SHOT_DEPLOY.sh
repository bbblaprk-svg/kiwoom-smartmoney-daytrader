#!/usr/bin/env bash
set -Eeuo pipefail
# PULSE EDGE 2.1.11.8 — deployment repair. Run with bash, never source this file.
...
RELEASE=2.1.11.8_CPU_FIX
EXPECTED_DF_SHA=91ea69a91c0f1be4515d43063b3c6af531578d853a7c4b6e099bb55c5cfe3a4e
EXPECTED_SRC_SHA=edbdac11e30d536f379fc83d281977147e3085122932ac97fec67820a2f2f58c
SRC_NAME=PULSE_EDGE_CORE_2_1_3_RUNTIME_2_1_11_8_CPU_FIX_SOURCE.tar.gz
...
for c in docker curl python3 sha256sum flock awk df cp tee seq stat date; do need "$c"; done
...
if [ "$MODE" = --background ]; then
  LOG="$BASE/deploy-latest.log"
  nohup bash "$SELF" deploy >"$LOG" 2>&1 < /dev/null &
  log "배포 실행 PID=$! | 로그: $LOG"
  log "확인: sudo tail -n 40 $LOG"
  exit 0
fi
...
public_check(){
  ...
  for i in $(seq 1 30); do
    if curl -fsS --connect-timeout 3 --max-time 8 "$PUB/health" >"$RUN_DIR/public-health.json" &&
       ...
       { [ -z "$expected" ] || [ "$(json_path "$RUN_DIR/public-health.json" runtime_safety_release)" = "$expected" ]; } &&
...
[ "$MODE" = deploy ] || [ "$MODE" = --preflight ] || die 'usage: bash SCRIPT [--background | --preflight | --rollback /opt/pulse-edge/deployments/RUN]'
    ...
    for root in roots:
        for p in sorted(root.glob(pattern)):
            if p.is_file() and hashlib.sha256(p.read_bytes()).hexdigest()==expected:
                found.append(p)
    if not found: raise SystemExit(f'{label}: matching 2.1.11.8 file missing or modified; put all 3 files together')
...
with tarfile.open(run/'build'/sys.argv[7],'r:gz') as t:
    ...
    for f in ['requirements-runtime.txt','pulse_edge/main.py','pulse_edge/web/pulse.js','pulse_edge/web/pulse.css','deploy/prepare_env.py','deploy/verify.py','deploy/burst_test.py','deploy/offline_tests.py','deploy/soak_check.py','deploy/UNCHANGED_SHA256.json','deploy/data_check.py','deploy/runtime_tests.py','deploy/credential_conflicts.py','deploy/resource_probe.py','pulse_edge/features/series.py']:
...
python3 "$RUN_DIR/source/deploy/resource_probe.py" >"$RUN_DIR/host-before.json"
docker stats --no-stream --format '{{.Name}} CPU={{.CPUPerc}} MEMORY={{.MemUsage}}' >"$RUN_DIR/containers-before.txt"
...
# Protect host capacity without hiding application overload from health checks.
read -r RUN_CPUS RUN_MEMORY_MB RUN_SWAP_MB < <(python3 - <<'PYRES'
import os
m={l.split(':')[0]:int(l.split()[1]) for l in open('/proc/meminfo') if ':' in l}
total=m['MemTotal']//1024
if total < 768: raise SystemExit('at least 768MiB host RAM is required')
cpus=min(1.5,max(0.5,(os.cpu_count() or 1)*0.75))
ram=min(1024,max(512,total-256))
print(cpus,ram,ram+min(512,m.get('SwapTotal',0)//1024))
PYRES
)
[[ "$RUN_MEMORY_MB" =~ ^[0-9]+$ ]] || die 'could not size runtime capacity'
RESOURCE_ARGS=(--cpus "$RUN_CPUS" --memory "${RUN_MEMORY_MB}m" --memory-swap "${RUN_SWAP_MB}m" --pids-limit 128 --log-driver json-file --log-opt max-size=10m --log-opt max-file=3)
log "RUNTIME_LIMITS cpus=$RUN_CPUS memory_mb=$RUN_MEMORY_MB memory_plus_swap_mb=$RUN_SWAP_MB"
log '[3/12] Build archive-aware image and run offline semantics/config checks' 
IMG="pulse-edge:runtime-2.1.11.8-$RUN_ID"
CANARY="pulse-edge-canary-runtime-2.1.11.8-$RUN_ID"
LIVE="pulse-edge-live-runtime-2.1.11.8-$RUN_ID"
save_state
cp "$SELF" "$RUN_DIR/deploy.sh"
docker build -t "$IMG" "$RUN_DIR/build"
docker run --rm "${RESOURCE_ARGS[@]}" --network none -e PULSE_FEED_ENABLED=false --entrypoint python "$IMG" -m deploy.burst_test
docker run --rm "${RESOURCE_ARGS[@]}" --network none -e PULSE_FEED_ENABLED=false --entrypoint python "$IMG" -m deploy.offline_tests
docker run --rm "${RESOURCE_ARGS[@]}" --network none -e PULSE_FEED_ENABLED=false --entrypoint python "$IMG" -m deploy.runtime_tests
# Normalize quotes/export/CRLF once. The raw credential file is never printed.
docker run --rm "${RESOURCE_ARGS[@]}" --network none -e PULSE_FEED_ENABLED=false \
...
mapfile -t ALL_RUNNING_IDS < <(docker ps -q)
if [ "${#ALL_RUNNING_IDS[@]}" -gt 0 ]; then
  docker inspect "${ALL_RUNNING_IDS[@]}" | python3 "$RUN_DIR/source/deploy/credential_conflicts.py" "$RUN_DIR/runtime.env"
fi
...
log '[5/12] Feed-OFF canary, isolated ports, pages/assets/APIs'
# Docker allocates and reserves the host port atomically; no ss/probe race.
docker run -d "${RESOURCE_ARGS[@]}" --name "$CANARY" --restart=no \
  ...
  -p 127.0.0.1::8000 "$IMG" >/dev/null
CANARY_PORT="$(docker port "$CANARY" 8000/tcp | awk -F: 'NR==1{print $NF}')"
wait_local(){
  local c="$1" port="$2" feed="$3" i
  for i in $(seq 1 90); do
    running "$c" || { log "$c stopped during startup"; return 1; }
    if curl -fsS --max-time 3 "http://127.0.0.1:$port/health" >"$RUN_DIR/ready.json" 2>/dev/null; then
...
wait_local "$CANARY" "$CANARY_PORT" false || die 'canary not ready (inspect docker logs)'
python3 "$RUN_DIR/source/deploy/verify.py" "http://127.0.0.1:$CANARY_PORT" false

log '[6/12] Two-minute Feed-OFF responsiveness check'
for i in $(seq 1 24); do
  curl -fsS --max-time 4 "http://127.0.0.1:$CANARY_PORT/health" >/dev/null
...
docker run -d "${RESOURCE_ARGS[@]}" --name "$LIVE" --restart=unless-stopped \
  ...
  --network "$NET" --network-alias pulse-edge -p 127.0.0.1::8000 "$IMG" >/dev/null
LIVE_PORT="$(docker port "$LIVE" 8000/tcp | awk -F: 'NR==1{print $NF}')"
wait_local "$LIVE" "$LIVE_PORT" true || die 'new live not ready (inspect docker logs)'
...
public_check "$RELEASE" || die 'public HTTPS health/page/API did not recover'
python3 "$RUN_DIR/source/deploy/verify.py" "http://127.0.0.1:$LIVE_PORT" true
...
for i in $(seq 1 20); do
  ...
  for entry in 'health:health' 'session:api/session' 'truth:api/feed-truth' 'runtime:api/runtime-truth'; do
    ...
    curl -fsS --max-time 6 "http://127.0.0.1:$LIVE_PORT/$endpoint" >"$RUN_DIR/$name.json" || BAD=1
  done
  curl -fsS --max-time 8 "$PUB/" >/dev/null || BAD=1
  curl -fsS --max-time 8 "$PUB/api/presentation/current-decision" >/dev/null || BAD=1
  curl -fsS --max-time 8 "http://127.0.0.1:$LIVE_PORT/api/smart-money/top?limit=10" >/dev/null || BAD=1
  ...
  sole_feed || BAD=1
  python3 "$RUN_DIR/source/deploy/resource_probe.py" >"$RUN_DIR/host-latest.json"
...
ln -sfn "$RUN_DIR" "$BASE/last-successful-deployment"
log "ACTIVE=$LIVE"
log "PUBLIC=$PUB"
