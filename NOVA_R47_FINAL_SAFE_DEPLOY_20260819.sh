...
EXPECTED_DOCKERFILE_SHA="5fcf7adab0251030667ec18082b9395b4f2d3f2709119e0bca6b5dbd4c454846"
EXPECTED_VERSION="NOVA-3.3.4-MARKET-WIDE-ROTATION-VERIFY"
IMAGE="quant-nova:3.3.4-r47-final-local"
STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP="${APP}-pre-r47-${STAMP}"
FAILED="${APP}-failed-r47-${STAMP}"
CANDIDATE="${APP}-candidate-r47-${STAMP}"
WORK="$(mktemp -d /tmp/nova-r47-final.XXXXXX)"
DOCKERFILE="$WORK/Dockerfile"
ENVFILE="$WORK/current.env"
CAND_DATA="$WORK/candidate-data"
LOG="/tmp/nova-r47-final-deploy-${STAMP}.log"
OLD_WAS_RUNNING=0
OLD_STOPPED=0
OLD_RENAMED=0
NEW_STARTED=0
...
recover_previous_if_needed(){
  ...
  latest="$(dc ps -a --format '{{.Names}}' | grep -E "^${APP}-pre-r47-[0-9]{8}-[0-9]{6}$" | sort | tail -n 1 || true)"
  if [ -n "$latest" ]; then
...
fetch_dockerfile(){
  local remote="QUANT_NOVA_3.3.4_R46_SESSION_CLOSE_HARD_FREEZE_FINAL_20260819.Dockerfile"
...
say "1/8 FETCH R46 FINAL DOCKERFILE + SHA LOCK"
fetch_dockerfile
...
[ "$(dc image inspect "$IMAGE" --format '{{index .Config.Labels "io.quantnova.ui_patch"}}')" = "R47_SESSION_CLOSE_QUEUE_DROP_HARD_FREEZE" ]
[ "$(dc image inspect "$IMAGE" --format '{{index .Config.Labels "io.quantnova.session_close_policy"}}')" = "HARD_FREEZE_NO_RANK_NO_EVENT_WRITERS" ]
[ "$(dc image inspect "$IMAGE" --format '{{index .Config.Labels "io.quantnova.close_signal_history"}}')" = "FROZEN_LAST_LIVE_NO_TOP10_REENTER" ]
