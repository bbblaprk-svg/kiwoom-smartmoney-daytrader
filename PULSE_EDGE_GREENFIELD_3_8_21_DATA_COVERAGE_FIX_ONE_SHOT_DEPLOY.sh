...
RELEASE="3.8.21_DATA_COVERAGE_FIX"
...
SRC_NAME="PULSE_EDGE_GREENFIELD_3_8_21_DATA_COVERAGE_FIX_SOURCE.tar.gz"
EXPECTED_SRC_SHA="2c020909e6535a5ed290214173c02e95cd8a9897e676f6ae4cfcafc56b54a3cb"
EXPECTED_DF_SHA="416b9d45aeeb76b4221ddd39bbe4b18a73063282e5b6ad8273ead80971b9ef47"
MODE="${1:-deploy}"
SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)"
RUN_ID="$(date +%Y%m%d-%H%M%S)-$$"
RUN_DIR="$BASE/deployments/$RUN_ID"
CANARY_DATA="$RUN_DIR/canary-data"
IMG="pulse-edge:gf3821-hidden-$RUN_ID"
CANARY="pulse-edge-gf3821-canary-$RUN_ID"
LIVE="pulse-edge-gf3821-live-$RUN_ID"
OLD=""
OLD_RUNNING=false
OLD_RESTART="no"
CUTOVER=0
...
fixed={
'PULSE_RELEASE':'3.8.21_DATA_COVERAGE_FIX','PULSE_FEED_ENABLED':'true','PULSE_DATA_DIR':'/app/data/pulse_edge',
...
import json,sys
q=json.load(open(sys.argv[1])); assert q['release']=='3.8.21_DATA_COVERAGE_FIX' and q['ready'] is True and q['feed_enabled'] is False
PY
  fi
...
import json,sys
q=json.load(open(sys.argv[1])); assert q.get('release')=='3.8.21_DATA_COVERAGE_FIX' and isinstance(q.get('rows'),list),q
...
import json,sys
h=json.load(open(sys.argv[1])); assert h.get('runtime_safety_release')=='3.8.21_DATA_COVERAGE_FIX'
PY
public=0
