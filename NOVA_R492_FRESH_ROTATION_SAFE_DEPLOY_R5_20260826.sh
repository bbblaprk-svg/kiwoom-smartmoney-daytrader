...
WORK="$(mktemp -d /tmp/nova-r492-freshrot-r5.XXXXXX)"
ENVFILE="$WORK/current.env"
BUILDCTX="$WORK/buildctx"
LOG="/tmp/nova-r492-freshrot-r5-${STAMP}.log"
NEW_STARTED=0
OLD_RENAMED=0
OLD_STOPPED=0
ROLLBACK_DONE=0
...
say(){ printf '\n===== %s =====\n' "$*" | tee -a "$LOG"; }
cleanup(){
  if [ -d "$WORK" ] && [[ "$WORK" == /tmp/nova-r492-freshrot-r5.* ]]; then
...
rotation_functional_probe_image(){
  dc run --rm -i -w /app -e PYTHONPATH=/app --entrypoint python "$IMAGE" - <<'PY'
...
from app.signal.early_edge import discovery_scout_score
state=RuntimeState(); base=time.time(); rows=[]
for index in range(1200):
    c=state.candidate(f'{index:06d}',f'ROTATION{index}',trade_day='20260816')
    c.source_hits['volume_surge']=base; c.rest_base=float(index%100)
    c.metrics['discovery_scout_score']=discovery_scout_score(c,base)
    rows.append(c)
rotation=MarketRotation(); selected=set(); lat=[]; sizes={}; now=base
modes=('NORMAL','NORMAL','BUSY','NORMAL','HIGH_LOAD','NORMAL','CRITICAL','NORMAL')
for cycle in range(32):
    mode=modes[cycle%len(modes)]; desired=rotation.target_for_mode(mode)
    for c in rows: c.source_hits['volume_surge']=now
    t=time.perf_counter(); result=rotation.select(rows,selected,set(),desired,'20260816',now); lat.append((time.perf_counter()-t)*1000)
    selected=result.selected; sizes.setdefault(mode,set()).add(len(selected)); assert len(selected)<=desired
    now+=24
for c in rows: c.source_hits['volume_surge']=now
normal=rotation.select(rows,selected,set(),rotation.target_for_mode('NORMAL'),'20260816',now)
state.hot_codes={f'{index:06d}' for index in range(2000,2050)}; state.exploration_codes=set(normal.selected)
ws=KiwoomWebSocket(state,None,None); _sessions,trade,program,orderbook=ws._targets()
assert len(trade)<=60; assert not state.exploration_codes.intersection(program); assert not state.exploration_codes.intersection(orderbook)
state.load_mode='CRITICAL'; _sessions,ctrade,cprogram,corderbook=ws._targets()
assert len(ctrade)==50; assert not state.exploration_codes.intersection(ctrade); assert cprogram==program and corderbook==orderbook
assert len(RANK_SPECS)==7
p95=sorted(lat)[int(.95*(len(lat)-1))]
print(json.dumps({'ok':True,'production_cached_score':True,'candidate_pool':len(rows),'cycles':len(lat),'rotation_p95_ms_info_only':round(p95,3),'mode_slots':{m:rotation.target_for_mode(m) for m in ('NORMAL','BUSY','HIGH_LOAD','CRITICAL')},'observed_sizes':{m:sorted(v) for m,v in sizes.items()},'trade_registered':len(trade),'critical_trade_registered':len(ctrade),'broker_rank_specs_unchanged':len(RANK_SPECS),'official_buy_for_exploration':False},ensure_ascii=False,sort_keys=True))
print('MARKET_ROTATION_FUNCTIONAL_ACCEPTANCE=PASS PERF_P95=INFO_ONLY')
PY
}
rotation_functional_probe_container(){
  dc exec -i -e PYTHONPATH=/app -w /app "$APP" python - <<'PY'
from __future__ import annotations
import json,time
from app.broker.discovery import RANK_SPECS
from app.broker.market_rotation import MarketRotation
...
assert len(RANK_SPECS)==7
p95=sorted(lat)[int(.95*(len(lat)-1))]
print(json.dumps({'ok':True,'production_cached_score':True,'candidate_pool':len(rows),'cycles':len(lat),'rotation_p95_ms_info_only':round(p95,3),'trade_registered':len(trade),'critical_trade_registered':len(ctrade),'official_buy_for_exploration':False},ensure_ascii=False,sort_keys=True))
print('MARKET_ROTATION_FUNCTIONAL_ACCEPTANCE=PASS PERF_P95=INFO_ONLY')
PY
}
static_image_acceptance(){
  ...
  dc run --rm -w /app -e PYTHONPATH=/app --entrypoint python "$IMAGE" /app/scripts/r42_fresh_scout_acceptance.py | tee -a "$LOG"
  rotation_functional_probe_image | tee -a "$LOG"
  dc run --rm --entrypoint sh "$IMAGE" -lc '
    echo "18cb96ef1afb9de48136d39c3e9b8216e62a94dccf0229aa027479783eb9151a  /app/app/signal/policy.py" | sha256sum -c - &&
    echo "e10d936a2540a70d8a488e0460608a3c2a1a8bfaf95dc6206e706cce3afab0d1  /app/app/broker/kiwoom.py" | sha256sum -c - &&
    ...
    echo "cd403cda76cb9d515c9228bf310287ad639bed125075fa835a3607cac2e81595  /app/app/addons/energy_path.py" | sha256sum -c - &&
    echo "28e835a781ed4a0a00bf4ab1fb1f2dcfb6c3c23ed77750c492788a168c80451b  /app/app/runtime/state.py" | sha256sum -c - &&
    echo "8f0904af264a3a8eda2d874dda25ce8086fb66d1751b592a9be0d6069b5ee1f6  /app/static/nova.js" | sha256sum -c - &&
    echo "c07332eb6951c51b433f42059133dd22b5e28a0c137f07b9f177f814dbe1ec1b  /app/ops/http_guard_v2.py" | sha256sum -c -
  '
}
...
say "1. CURRENT CONTAINER + DOCKERFILE PRECHECK — R5 REUSE-EXACT-IMAGE"
...
dc exec -e PYTHONPATH=/app -w /app "$APP" python /app/scripts/r42_fresh_scout_acceptance.py | tee -a "$LOG"
rotation_functional_probe_container | tee -a "$LOG"
dc exec -e PYTHONPATH=/app -w /app "$APP" python - "$EXPECTED_VERSION" <<'PY' | tee -a "$LOG"
...
echo "IMAGE_STRATEGY=REUSE_EXACT_IF_PRESENT BUILD_CACHE_IF_NEEDED MARKET_ROTATION_PERF=PRODUCTION_CACHED_INFO_ONLY" | tee -a "$LOG"
