#!/usr/bin/env bash
set -Eeuo pipefail
# PULSE EDGE 2.1.11.9 R5 — audited scout-funnel + CPU-safe deployment; self-heals failed prior PULSE route owners; Docker-network canary; no host-port/userland-proxy dependency. Run with bash, never source this file.
...
EXPECTED_DF_SHA=2ccbc2856c99ad1b4760ce45a2c3e96a44ed44bc95d504af6f1011e2c3d80fdd
EXPECTED_SRC_SHA=08ecfd38ea8f1cb9433dd64d585f4f01ea69654c6636a73d4cada814fd52fa29
SRC_NAME=PULSE_EDGE_CORE_2_1_3_RUNTIME_2_1_11_9_SCOUT_FUNNEL_CPU_SAFE_R5_SOURCE.tar.gz
...
reconcile_route_holders(){
  ...
  while IFS= read -r c; do
    ...
    # A managed live container from a deployment without deploy-passed is a failed/orphan run.
    # Docker restart may resurrect it because it was originally created with unless-stopped.
    if [ "$project" = PULSE_EDGE ] && [ "$role" = live ] && [ -n "$rid" ] && [ ! -f "$BASE/deployments/$rid/deploy-passed" ]; then
