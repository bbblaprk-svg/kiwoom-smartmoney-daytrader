#!/usr/bin/env bash
set -Eeuo pipefail
# PULSE EDGE 2.1.11.6 — deployment repair. Run with bash, never source this file.
...
RELEASE=2.1.11.6_DEPLOY_REPAIR
EXPECTED_DF_SHA=3c832bc9f0cacb3630526f73725d74bc239572a27c9b3dfdbce77aecbaab38cf
EXPECTED_SRC_SHA=a634896211537fd9288f2b8d569edc7da8c5268d886d447b15c683ff81ec02fd
SRC_NAME=PULSE_EDGE_CORE_2_1_3_RUNTIME_2_1_11_6_DEPLOY_REPAIR_SOURCE.tar.gz
    ...
    for root in roots:
        for p in sorted(root.glob(pattern)):
            if p.is_file() and hashlib.sha256(p.read_bytes()).hexdigest()==expected:
                found.append(p)
    if not found: raise SystemExit(f'{label}: matching 2.1.11.6 file missing or modified; put all 3 files together')
...
IMG="pulse-edge:runtime-2.1.11.6-$RUN_ID"
CANARY="pulse-edge-canary-runtime-2.1.11.6-$RUN_ID"
LIVE="pulse-edge-live-runtime-2.1.11.6-$RUN_ID"
save_state
